...
BLP		*Blp;
        ...
        /* 호가유형코드, 여기서는 지정가로 처리했다. 상/하한가로 주문낼때도 지정가로 상하한가 가격으로 내면 된다. 나머지는 전략에 따라 
            사용하면 된다. */
...
/* **************************************************************************************************** */
		...
		*/
		...
		switch (i)
		{
			...
			case 2:			// 첫번째 시장신호(ex, 채권) - 우선 test로 채권 시세 
...
Log (USR_OK, "TTT02");
	...
	if (memcmp(R_Fmt[0].Data, "500100", 6) == 0)				// 자동기동 Setting
		Set_In_Param();
	else if (memcmp(R_Fmt[0].Data, "500200", 6) == 0)				// 폐지
		Set_In_Param();
...
int	GetOrderNo( char *buf, int sz)
{
	...
	Log( USR_OK, "주문번호 채번 ... no=[%ld]", Shm_Risk[0].Order_No[0][OD_SEQ].USED_JMNO);
...
// 공통
	...
	else	// 전략 없음 --- CDC
	{
		...
		/* 1 - 채권KTS_시세수신 : pa_7102_ur */
		/* 2 - 채권KTS_M4수신   : pa_7103_ur */
		InPollCnt = 2;
...
typedef struct {
	...
	{
		Log( USR_OK, "    채권LP");
		// TEST pa_1101_ts이 이 채권 주문 송신 이니까 0으로 나가야 하는것 아닌지 o_gbn = 1;
		o_gbn = 0;
		rt = DSHM_W (TS_W1_1, (void *)W_DFmt, 1);
	}
	else											// 금융파생
	{
		Log( USR_OK, "    금융파생 Data[%s]", W_DFmt);
		o_gbn = 2;
		// rt = DSHM_W (TS_W1_1, (void *)W_DFmt, 1);
	}
...
*/
	...
	{
		Log( USR_OK, "    채권LP");
		// TEST pa_1101_ts이 이 채권 주문 송신 이니까 일로 나가야 하는것 아닌지 o_gbn = 1;
		o_gbn = 0;
		rt = DSHM_W (TS_W1_1, (void *)&W_Fmt, 1);
	}
	else											// 금융파생
