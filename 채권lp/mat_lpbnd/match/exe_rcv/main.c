...
int InitProcess( int argc, char *argv[])
{
	...
	{
		sprintf( Param->cfg_name, "%s/%s", getenv( "MAT_CFG"), "exe_rcv.cfg");
	...
	/* SMQ queue open                               */
	/************************************************/
	Mo->smq_recv = Smq_Open( Param->rq_name);
	if( Mo->smq_recv == NULL)
	{
	...
	Mo->smq_send = Smq_Open( Param->sq_name);
	if( Mo->smq_send == NULL)
	{
		LogCri( "Smq_Open error. name=[%s]", Param->rq_name);
		return -1;
...
{
	ORDER_SEND		*ord_rcv = ( ORDER_SEND *)Mo->recv;

	LogSetDump( LOG_DUMP_NONE);
	
	while( Continue)
	{
		LogDel( "Smq_GetRecord call...");
		memset( Mo->recv, 0x00, sizeof( Mo->recv));
		Mo->rsz = Smq_GetRecord( Mo->smq_recv, Mo->recv, 8192, Param->timeout);
		if( Mo->rsz < 0)
		{
			if( Mo->rsz == SMQ_TIMEOUT) 
			{
				LogDbg( "receive timeout. timeout=[%d.%d]", Param->timeout / 1000000, Param->timeout % 1000000);
				continue;
			}
			LogCri( "Smq_GetRecord error. Mo->rsz=[%d]", Mo->rsz);
			break;
		}
		Smq_Commit( Mo->smq_recv);
		LogDbg( "id=[%.24s] sym=[%.7s] type=[%.1s] stat=[%.1s]", 
			ord_rcv->ClOrdID,
			ord_rcv->Symbol,
			ord_rcv->MsgType,
			ord_rcv->OrdStatus);
		if( Param->log_level < 10)
		{
			LogDump( Mo->recv, Mo->rsz, "recv %d byte(s).", Mo->rsz);
			ORDER_SEND_Print( ord_rcv);
		}
	}
...
int ParamPrint( PARAM *param)
{
	...
	LogDbg( "param->sq_name        = [%s]", param->sq_name);
	LogDbg( "param->rq_name        = [%s]", param->rq_name);
	return 1;
}
