...
int InitProcess( int argc, char *argv[])
{
	...
	{
		sprintf( Param->cfg_name, "%s/%s", getenv( "MAT_CFG"), "mat_rcv.cfg");
...
int MainProcess( int argc, char *argv[])
{
	...
	while( 1)
	{
		...
		if( FD_ISSET( TCP_SOCK( TcpClient), &rfds))
		{
			...
			if( rtn < 0)
			{
				LogCri( "DataProcess error. close TcpClient=[%p] rtn=[%d]", TcpClient, rtn);
				Tcp_Close( TcpClient);
				TcpClient = NULL;
			}
		}
...
int DataProcess( TCP *tcp)
{
	...
	if( !strncmp( packet->comm_head.type, "DATA", 4))
	{
		...
		LogDbg( "Smq_Send ... data=[%d:%s]", sz, packet->data);
		ORDER_RECV_Print( ( ORDER_RECV *)packet->data);
...
int RecvPacket( TCP *tcp, MAT_PACKET *packet)
{
	...
	LogDbg( "recv data. data=[%04d:%.80s]", rcv_sz, ( char *)packet);
#if 0
...
int SendPacket( TCP *tcp, MAT_PACKET *packet, int sz)
{
	...
	LogDbg( "send data. data=[%04d:%.80s]", sz, ( char *)packet);
#if 0
