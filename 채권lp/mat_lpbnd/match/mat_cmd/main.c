...
"  i, ipc=<create|remove>     ipc handing\n"
	...
	while( 1)
	{
		...
		switch( opt)
		{
			...
			case 0   :
				break;
			case 'i' :
				if( optarg[ 0] == 'c')		Param->ipc_flag = 1;
				else if( optarg[ 0] == 'r')	Param->ipc_flag = 2;
				break;
			case 'c' :
				...
				break;
			case 'f' :
				sprintf( Param->map_name, optarg, strlen( optarg) +1);
...
int InitProcess( int argc, char *argv[])
{
	...
	{
		sprintf( Param->cfg_name, "%s/%s", getenv( "MAT_CFG"), "mat.cfg");
	...
	Param->color    = Cfg_GetInt( Cfg, "color");
	Param->external = Cfg_GetInt( Cfg, "external");
	rtn = Cfg_Get( Cfg, "map_name", Param->map_name, 512);
	rtn = Cfg_Get( Cfg, "map_small", Param->map_small, 512);
	rtn = Cfg_Get( Cfg, "map_index", Param->map_index, 512);
	rtn = Cfg_Get( Cfg, "map_smq", Param->map_smq, 512);
	rtn = Cfg_Get( Cfg, "map_statis", Param->map_statis, 512);
	rtn = Cfg_Get( Cfg, "map_order", Param->map_order, 512);
	...
	/************************************************/
	Param->max_record	= Cfg_GetInt( Cfg, "max_record");
	Param->max_curr		= Cfg_GetInt( Cfg, "max_curr");
	Param->max_conform	= Cfg_GetInt( Cfg, "max_conform");
	Param->ipc_key		= Cfg_GetInt( Cfg, "ipc_key");
	rtn = Cfg_Get( Cfg, "ord_pipe", Param->ord_pipe, 512);
	rtn = Cfg_Get( Cfg, "exe_pipe", Param->exe_pipe, 512);
	...
	if( Param->ipc_flag)
	{
		...
		switch( Param->ipc_flag)
		{
			case 1:		/* create */
				Mat_CreateForce();
				return 0;
			case 2:		/* remove */
				sprintf( cmd, "rm -f %s", MAT_ORD_PIPE);
				printf( "cmd=[%s]\n", cmd);
				system( cmd);
				sprintf( cmd, "rm -f %s", MAT_EXE_PIPE);
				printf( "cmd=[%s]\n", cmd);
				system( cmd);
				sprintf( cmd, "ipcrm -M 0x%08x", MAT_IPC_KEY);
				printf( "cmd=[%s]\n", cmd);
				system( cmd);
				sprintf( cmd, "ipcrm -S 0x%08x", MAT_IPC_KEY);
				printf( "cmd=[%s]\n", cmd);
				system( cmd);
				return 0;
			default:
	...
	/************************************************/
	/* Mat open - matching system                   */
	/************************************************/
	LogDbg( "매칭 엔진 OPEN ... Mat_Open()");
	Mat = Mat_Open( 1);
	if( Mat == NULL)
	{
		LogMsg( "매칭 엔진 IPC open 에러.");
		Mat = Mat_Create(); 
		if( Mat == NULL)
		{
			LogCri( "더이상 진행 할 수 없습니다.");
			return -1;
		}
		LogMsg( "매칭 엔진 IPC를 생성하였습니다.");
	}
	LogDbg( "매칭엔진 open or create success. Mat=[%p]", Mat);

	/************************************************/
	/* SMQ open - for monitoring                    */
	/************************************************/
	LogDbg( "SMQ OPEN ... Smq_Open( NULL)");
	Smq = Smq_Open( NULL);
	if( Smq == NULL)
	{
		LogCri( "Smq_Open error. smq manager[smq]에서 smq open error 원인을 파악하십시요.");
		return -1;
	}
	LogDbg( "SMQ open or create success. Smq=[%p]", Smq);
	...
	if( rtn < 0)
	{
		...
		LogMsg( "DB 관련 명령을 사용할 수 없습니다.");
	}
	else
	{
		DbCon = 1;
		LogMsg( "DB connect success.");
	}
	...
	/************************************************/
	TaskInit();
	if( Param->external) Cmd_Set( Cmd, CMD_EXTENAL_COMMAND_ON);
...
int ParamPrint( PARAM *param)
{
	...
	LogDbg( "param->map_name       = [%s]", param->map_name);
	LogDbg( "param->timeout        = [%d]", param->timeout);
	LogDbg( "param->interval       = [%d]", param->interval);

	LogDbg( "param->max_record     = [%d]", param->max_record);
	LogDbg( "param->max_curr       = [%d]", param->max_curr);
	LogDbg( "param->max_conform    = [%d]", param->max_conform);
	LogDbg( "param->ipc_key        = [0x%08x]", param->ipc_key);
	LogDbg( "param->ord_pipe       = [%s]", param->ord_pipe);
	LogDbg( "param->exe_pipe       = [%s]", param->exe_pipe);
	return 1;
}
