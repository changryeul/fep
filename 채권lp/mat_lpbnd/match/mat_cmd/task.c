...
int	CmdJangCurr( int argc, char *argv[]);
int	CmdJangPair( int argc, char *argv[]);
int	CmdJangPairDev( int argc, char *argv[]);
int	CmdMonEnd( int argc, char *argv[]);
int	CmdCancel( int argc, char *argv[]);
int	CmdSiseStop( int argc, char *argv[]);
...
CMD_TBL	CmdTable[] =
{
	...
	{	18,		"msg",			CmdMsg,			"[l] [code|msg]",		"에러메세지 조회 l=에러메세지 로드"},
	...
	{	28,		"jc",			CmdJangCurr,	"[l] or [key [key [...]]]",	"통화별 장운영 조회 / 적용"},
	{	29,		"me",			CmdMonEnd,		"[e value(0|1)] [t time]",	"월말 마감시간 변경 및 조회\n"
																		"e=월말 여부 변경 0=월말아님, 1=월말\n"
																		"t=월말 마감시간 변경 (HHMMSS:jang 시간 변경 참조)"},
	{	32,		"cancel",		CmdCancel,		"[l][s HHMMSS][e HHMMSS]",	"주문 중지 시간 설정 l=DB load"},
	{	33,		"sisestop",		CmdSiseStop,	"[0|1]",				"시세 수신 중단 1=중단 0=수신"},
	{	30,		"color",		CmdColor,		"none",					"color set for mon display"},
...
{
	int		jang_id;
	char	order_recv[ 2048] = "D23312192                                  2                         E528000948400000000000USD1000                   1           -1.140000            0.000000         1444.060000            0.0000002020250528-03:20:19.18SWPM01SUSD/KRW2025063020250630SPTSPTS120250530         1445.900000            0.000000FWDM01S220250630         1444.460000            0.0000001E5280000018         0         0   1            0.000000            0.000000            1000.000000                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  ";
	// char	order_recv[ 2048] = "D1000050001                                1                         E528000938300000000000USD1000                   1         1555.800000          100.000000         1555.800000          300.0000001020250528-00:30:12.27SPTSPTSUSD/KRW2025053020250530SPT                         0.000000            0.000000FWD                         0.000000            0.000000100000000000         0         0   1            0.000000            0.000000            1000.000000                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  ";
	ORDER	_order, *order = &_order;

	ORDER_RECV_Print( ( ORDER_RECV *)order_recv);
	Proc_OrderConvert( order, ( ORDER_RECV *)order_recv);
	jang_id = Mat_GetJangId( Mat, order, 0);

	Mat_JangCheck( Mat, order, jang_id);
...
int CmdMsg( int argc, char *argv[])
{
	...
	switch( argc)
	{
		default:
		case 2:
			if( !strncmp( argv[ 1], "l", 1 +1))
			{
				rtn = Conform( "config 파일에서 메세지를 로드합니다. 진행 하시겠습니까?");
				if( rtn != 0)
				{
					sprintf( Param->cfg_name, "%s/%s", getenv( "MAT_CFG"), "mat_bat.cfg");
					Mat_Lock( Mat);
					Proc_LoadMessage( Mat);
					Mat_Unlock( Mat);
					Mat_MessageList( Mat);
				}
				else
...
int CmdList( int argc, char *argv[])
{
	...
	*/
	...
	for( i = start; i < MAT_MAX_RECORD; i++)
	{
		...
		LogDel( "i=[%d] ord_time=[%ld] rcv_time=[%ld]", i, head->ord_time.tv_sec, head->rcv_time.tv_sec);
...
int CmdSize( int argc, char *argv[])
{
	...
	*/
	...
	printf( "sizeof( MAT_INDEX)     = [%ld]\n", sizeof( MAT_INDEX));
	printf( "sizeof( MAT_HEAD)      = [%ld]\n", sizeof( MAT_HEAD));
	printf( "sizeof( MAT_RECORD)    = [%ld]\n", sizeof( MAT_RECORD));
	printf( "sizeof( MAT_JANG)      = [%ld]\n", sizeof( MAT_JANG));
...
int CmdService( int argc, char *argv[])
{
	...
	*/
	...
	if( argc >= 2)
	{
		rtn = Conform( "서비스 상태를 변경합니다.. 진행 하시겠습니까?");
		if( rtn == 0) 
		{
			printf( "취소되었습니다.\n");
			return 0;
	...
	*/
	...
	switch( argc)
	{
		...
		case 2:
			...
			if( argv[ 1][ 0] == 's') 		
			{
				if( argc >= 3)	Mat->map->stat.start = mktime( tp);
			}
			else 
			if( argv[ 1][ 0] == 'e')	
			{
				if( argc >= 3)	Mat->map->stat.end = mktime( tp);
			}
			else 
			if( argv[ 1][ 0] == 'l')
			{
...
int CmdJangCurr( int argc, char *argv[])
{
    ...
    char	*prod_str[ 6] = { "BAR", "SPT", "FWD", "MAR", "SWP", NULL};
...
{ 
	...
	switch( argc)
	{
		...
		case 2:
			...
			{
				yn = atoi( argv[ 2]);
				Mat->map->stat.last_day = yn;
			}
			else
			{
				printf( "unknown option ... \n\n");
				rtn = Cmd_HelpCommand( Cmd, argv[ 0]);
				if( rtn < 0) return -1;
				printf( "\n");
			}
		case 1:
		case 0:
			printf( "월말=[%d] 마감시간=[%s]\n", Mat->map->stat.last_day, TtoS( Mat->map->stat.last_time));
			break;
	}
...
{ 
	...
	switch( argc)
	{
		...
		case 2:
			if( argv[ 1][ 0] == 's')
			{
				Mat->map->stat.cancel_start = set_time;
			}
			else
			if( argv[ 1][ 0] == 'e')
			{
				Mat->map->stat.cancel_end = set_time;
			}
			else
			if( argv[ 1][ 0] == 'l')
			{
				rtn = Conform( "주문중지 시간을 DB에서 LOAD 하시겠습니까?");
				if( rtn > 0)
				{
					Db_GetTbFxbCmc003D();
				}
		...
		case 1:
		case 0:
			printf( "주문중지 시작시간=[%s]\n", TtoS( Mat->map->stat.cancel_start));
			printf( "주문중지 종료시간=[%s]\n", TtoS( Mat->map->stat.cancel_end));
			break;
	}
...
{ 
	...
	printf( "Mat->map->stat.sise=[%d] (0=시세수신, 1=시세수신중단)\n", Mat->map->stat.sise);
