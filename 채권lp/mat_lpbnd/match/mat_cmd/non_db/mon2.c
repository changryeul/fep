			...
			default:
				switch( field)
				{
					case 104:
						...
						Map_CursorOff( map);
						Mon_ProcessRecord( map, mat, line, 0);
						Map_DisplayMap( map);
						break;
					case 106:
						...
						Map_CursorOff( map);
						Mon_ProcessRecord( map, mat, line, 1);
						Map_DisplayMap( map);
						break;
					case 108:
					case 109:
						...
						title = " 현재 sise ";
						Map_CursorOff( map);
						Mon_ProcessSise( map, sise, title);
						Map_DisplayMap( map);
						break;
					case 110:
					case 111:
...
**	모니터링맵 초기화
***************************************************************************** */
int Mon_MainInit2( MAP *map, MAT *mat, SMQ *smq)
{
	int			i, line = 0;
	char		name[ 32];
	MAT_INDEX	*index;
	...
	Map_SetDataPtr( map, "ordp",		&mat->map->stat.ord_pipe);
	...
	for( i = 0; i < MAT_MAX_CURR; i++)
	{
...
#elseif 0
		sprintf( name, "SiCuBi_%d", line);	Map_SetDataPtr( map, name, &index->sise_curr.bidprc);
		sprintf( name, "SiCuOf_%d", line);	Map_SetDataPtr( map, name, &index->sise_curr.offerprc);
		sprintf( name, "SiBaBi_%d", line);	Map_SetDataPtr( map, name, &index->sise_base.bidprc);
		sprintf( name, "SiBaOf_%d", line);	Map_SetDataPtr( map, name, &index->sise_base.offerprc);
		sprintf( name, "SiCoBi_%d", line);	Map_SetDataPtr( map, name, &index->sise_cont.bidprc);
		sprintf( name, "SiCoOf_%d", line);	Map_SetDataPtr( map, name, &index->sise_cont.offerprc);
#else
		sprintf( name, "SiCuBi_%d", line);	Map_SetField( map, name, Mon_Current, index);
		sprintf( name, "SiCuOf_%d", line);	Map_SetField( map, name, Mon_Current, index);
		sprintf( name, "SiBaBi_%d", line);	Map_SetField( map, name, Mon_Current, index);
		...
		sprintf( name, "SiCoOf_%d", line);	Map_SetField( map, name, Mon_Current, index);
		/*
		sprintf( name, "SiCuBi_%d", line);	Map_SetField( map, name, Mon_Current, &index->sise_curr.bidprc);
		sprintf( name, "SiCuOf_%d", line);	Map_SetField( map, name, Mon_Current, &index->sise_curr.askprc);
		sprintf( name, "SiBaBi_%d", line);	Map_SetField( map, name, Mon_Current, &index->sise_base.bidprc);
		sprintf( name, "SiBaOf_%d", line);	Map_SetField( map, name, Mon_Current, &index->sise_base.askprc);
		sprintf( name, "SiCoBi_%d", line);	Map_SetField( map, name, Mon_Current, &index->sise_cont.bidprc);
		sprintf( name, "SiCoOf_%d", line);	Map_SetField( map, name, Mon_Current, &index->sise_cont.askprc);
		*/
#endif
