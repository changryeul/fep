...
int InitProcess( int argc, char *argv[])
{
	...
	{
		sprintf( Param->cfg_name, "%s/%s", getenv( "MAT_CFG"), "smq.cfg");
	...
	if( Param->ipc_flag)
	{
		...
		{
			case 1:		/* create */
				Smq_CreateForce( Param->cfg_name);
				return 0;
			case 2:		/* remove */
				Smq_RemoveForce( Param->cfg_name);
				/*
				sprintf( cmd, "rm %s/%s.fifo", SMQ_PIPE_DIR, "smq");
				printf( "cmd=[%s]\n", cmd);
				system( cmd);
				sprintf( cmd, "ipcrm -M 0x%08x", SMQ_IPC_KEY);
				printf( "cmd=[%s]\n", cmd);
				system( cmd);
				sprintf( cmd, "ipcrm -S 0x%08x", SMQ_IPC_KEY);
				printf( "cmd=[%s]\n", cmd);
				system( cmd);
				*/
				return 0;
	...
	if( Smq == NULL)
	{
		LogMsg( "Smq_Open error. tyy \"CREATE\" Smq ... call Smq_Create.");
		...
		LogMsg( "Shard memory queue IPC를 생성하였습니다.");
	}
	LogDbg( "Smq_Open ... success");
