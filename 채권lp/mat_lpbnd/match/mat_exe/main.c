...
int InitProcess( int argc, char *argv[])
{
	...
	{
		sprintf( Param->cfg_name, "%s/%s", getenv( "MAT_CFG"), "mat_exe.cfg");
	...
	rtn = Cfg_Get( Cfg, "sq_name", Param->sq_name, 32);
	rtn = Cfg_Get( Cfg, "rq_name", Param->rq_name, 32);
	...
	rtn = fcntl( Mat->exe_fd, F_SETFL, O_NONBLOCK);
	if( rtn < 0)
	{
...
int MainProcess( int argc, char *argv[])
{
	...
	while( Continue)
	{
		...
		if( cur_time % 60 == 0)
		{
			...
			{
				LogMsg( "loop ... cnt=[%d][%d] pid=[%d]", cnt, cnt - old_cnt, getpid());
				old_time = cur_time;
				old_cnt = cnt;
			}
		}
		...
		if( pos < 0)
		{
			if( pos == MAT_TIMEOUT) 
			{
				LogDel( "Proc_Execute timeout. rtn=[%d] timeout=[%d:%d]", pos, Param->timeout / 1000000, Param->timeout % 1000000);
				continue;
			}
			break;
		}
