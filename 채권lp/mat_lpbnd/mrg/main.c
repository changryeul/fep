...
int InitProcess( int argc, char *argv[])
{
	...
	if( Param->ipc_flag)
	{
		...
		{
			case 1:		/* create */
				Mrg_Create( Param->key);
				return 0;
			case 2:		/* remove */
				sprintf( cmd, "ipcrm -M 0x%08x", Param->key);
				printf( "cmd=[%s]\n", cmd);
				system( cmd);
				return 0;
			default:
	...
	/* mrg open                                     */
	/************************************************/
	LogMsg( "Mrg_Open ... key=[0x%08x]", Param->key);
	Mrg = Mrg_Open( Param->key);
	if( Mrg == NULL)
	{
		LogCri( "Mrg_Open error. key=[0x%08x]", Param->key);
		Mrg = Mrg_Create( Param->key);
		if( Mrg == NULL)
		{
			LogCri( "Mrg_Create error. key=[0x%08x]", Param->key);
			return -1;
		}
	}
	LogMsg( "Mrg_Open ... success");
