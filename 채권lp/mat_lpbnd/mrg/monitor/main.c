...
SHM_ST			_ShmSt, *ShmSt = &_ShmSt;
E_MSG			_Emsg, *Emsg = &_Emsg;
CUSTMRGN_SHM_ST	*CustMrgn;
/** ***************************************************************************
**	PARAM
***************************************************************************** */
PARAM	ParamBuf =
...
int InitProcess( int argc, char *argv[])
{
	...
	{
		sprintf( Param->cfg_name, "%s/%s", "." /* getenv( "MAT_CFG") */, "main.cfg");
	...
	/* shm open                                     */
	/************************************************/
	ShmSt->key = CUST_MRGN_KEY;
	memcpy( Emsg->pname, argv[ 0], strlen( argv[ 0]) +1);
	f_rget_shm( ShmSt, Emsg);
	CustMrgn = ShmSt->ptr;
