...
MDS_MEM		MdsMem[ 10] =
{
	{	1,	"BST",	"BEST",	 	NULL, 0x90200000, NULL },
	{	2,	"REU",	"REUTER",	NULL, 0x90210000, NULL },
	{	3,	"KMB",	"KMB",		NULL, 0x90220000, NULL },
	{	4,	"SMB",	"SMB",		NULL, 0x90230000, NULL },
	{	5,	"EBS",	"EBS",		NULL, 0x90240000, NULL },
	{	0,	"\0",	"\0",		NULL, 0,          NULL }
};
enum MdmId { BEST = 0, REUTER, KMB, SMB, EBS };
int			MdsMemCurr = 0;
...
int InitProcess( int argc, char *argv[])
{
	...
	if( strchr( Param->argo, 'c') == NULL)
	{
		...
		LogMsg( "config file load.  Param->cfg_name = [%s]", Param->cfg_name);
	}
	...
	Param->mem_key = Cfg_GetInt( Cfg, "mem_key");
	Cfg_Get( Cfg, "main_map",	Param->main_map,	512);
	Cfg_Get( Cfg, "market_map", Param->market_map,	512);
	Cfg_Get( Cfg, "fold_map",   Param->fold_map,	512);
	Cfg_Get( Cfg, "quot_map",   Param->quot_map,	512);
	...
	{
		MdsMem[ i].mem = Mem_Open( MdsMem[ i].key);
		if( MdsMem[ i].mem == NULL)
		{
			LogErr( "MdsMem[%d] name=[%s] key=[0x%08x] Mem_Open error.", i, MdsMem[i].name, MdsMem[i].key);
			continue;
		}
		MdsMem[i].base = Mem_GetPtr( MdsMem[ i].mem);
		LogMsg( "MdsMem[%d] name=[%s] key=[0x%08x] base=[%p], Mem_Open success.", i, MdsMem[i].name, MdsMem[i].key);
	}
	...
	sprintf( Prompt, "[%s]", MdsMem[ MdsMemCurr].full_name);
...
int ParamPrint( PARAM *param)
{
	...
	LogMsg( "param->mem_key        = [0x%08x]", param->mem_key);
	LogMsg( "param->main_map       = [%s]", param->main_map);
	LogMsg( "param->market_map     = [%s]", param->market_map);
	LogMsg( "param->fold_map       = [%s]", param->fold_map);
	LogMsg( "param->quot_map       = [%s]", param->quot_map);
	return 1;
}
