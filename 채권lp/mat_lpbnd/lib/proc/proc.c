...
MEM *ProcTbl_Create( char *cfg_name)
{
	...
	key_t		key;
	char		version[ 64];
	PROC_TBL	*base, *daemon, *swapper;
	PROC_STAT	*stat;

	LogDbg( "Config file open ... name=[%s]", cfg_name);
...
{
	int				i = 0;
	PROC_TBL		*base, *rec, *daemon, *swapper;
	PROC_STAT		*stat;
...
int ProcTbl_RegiMe( MEM *mem, int cmd, char *id)
{
	...
	switch( cmd)
	{
		...
		case 1:		/* run */
			...
			curr->r_cnt++;
			time( &curr->sr_time);
			sprintf( curr->name, "%s", "proc_daemon");
			break;
		case 2:		/* pid 등록 */
			curr->pid = getpid();
			break;
...
int ProcTbl_LoadRec( MEM *mem, PROC_TBL *proc)
{
	...
	PROC_TBL	*base, *curr, *daemon, *swapper;
	PROC_STAT	*stat;
...
int Proc_Run( PROC *proc)
{
	...
	for( i = 0; i < proc->stat->cnt; i++)
	{
		...
		if( rtn) pid = Proc_RunProc( proc, curr);
...
int Proc_HealthCheck( PROC_TBL *tbl)
{
	time_t		cur_time;
	int			int_time;
	int			gap = 10;	/* 프로세스가 시작, 종료 할때까지 걸리는 시간 감안 */
...
int Proc_StopCmd( PROC *proc, char *id)
{
	...
	printf( "[%d] [%d] \n", !memcmp( id, "proc_d", 7), !memcmp( id, "all", 3));
	if( !memcmp( id, "proc_d", 7) || !memcmp( id, "all", 3))
	{
