...
MEM *MemLog_Create( key_t key, size_t sz)
{
	...
	if( mem == NULL)
	{
		LogMemErr( "malloc error. mem size=[%d]", sizeof( MEM));
		goto error;
	}
	...
	if( mem->id < 0)
	{
		LogMemErr( "shmget error. key=[0x%08x], sz=[%d]", mem->key, mem->sz);
		goto error_1;
	}
	...
	if( mem->ptr == (void *)-1)
	{
		LogMemErr( "shmat error. id=[%d]", mem->id);
		goto error_1;
	}

	LogMemDel( "shard memory created. key=[0x%08x] id=[%d] sz=[%d]", key, mem->id, sz);
	return mem;
...
int MemLog_Remove( MEM *mem)
{
	...
	if( rtn < 0)
	{
		LogMemErr( "shmdt error. ptr=[%p]", mem->ptr);
		return rtn;
	}
	...
	if( rtn < 0)
	{
		LogMemErr( "shmem remove error. id=[%d]", mem->id);
		return -1;
	}
...
int MemLog_RemoveByKey( key_t key)
{
	...
	if( id < 0)
	{
		LogMemErr( "shmget error. key=[0x%08x]", key);
		return -1;
	}
	...
	if( rtn < 0)
	{
		LogMemErr( "shmem remove error. id=[%d]", id);
		return -1;
	}
