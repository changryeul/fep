...
int ParamPrint( PARAM *param)
{
	...
	LogApp( "param                 = [%p]", param);
	LogApp( "param->argc           = [%d]", param->argc);
	for( i = 0; i < param->argc; i++)
	LogApp( "param->argv[%3d]      = [%s]", i, param->argv[i]);
	LogApp( "param->argo           = [%s]", param->argo);
	LogApp( "param->args           = [%d]", param->args);
	LogApp( "param->cfg_name       = [%s]", param->cfg_name);
	LogApp( "param->log_name       = [%s]", param->log_name);
	LogApp( "param->log_flag       = [%d]", param->log_flag);
	LogApp( "param->log_level      = [%d]", param->log_level);
	LogApp( "param->map_name       = [%s]", param->map_name);
	LogApp( "param->interval       = [%d]", param->interval);
	return 1;
}
