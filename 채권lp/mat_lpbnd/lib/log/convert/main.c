...
"  o, o_file=<out_file_name>  output file name (default:stdout)\n"
"  i, i_file=<in_file_name>   input file name (default:stdin)\n"
;
struct option LongOption[] = 
{
	{ "help",		no_argument,		0,	'h'},
	{ "config",		required_argument,	0,	'c'},
	{ "o_file",		required_argument,	0,	'o'},
...
int GetOption( int argc, char *argv[])
{
	...
	while( 1)
	{
		...
		switch( opt)
		{
			...
			case 'o' :
				Param->o_name = optarg;
				LogMsg( "Param->o_name=[%s]", Param->o_name);
				break;
			case 'i' :
				Param->i_name = optarg;
				LogMsg( "Param->i_name=[%s]", Param->i_name);
				break;
			case 'h' :
				return 0;
			default  :
...
int InitProcess( int argc, char *argv[])
{
	...
	/************************************************/
#if 0
	LogConv = LC_Open( Param->o_name, Param->i_name);
	if( LogConv == NULL)
	{
		LogCri( "LC_Open error. o_name=[%s] i_name=[%s]", Param->o_name, Param->i_name);
		return -1;
	}
#endif
...
int MainProcess( int argc, char *argv[])
{
	...
	LC_Process( Param->o_name, Param->i_name);
