...
/*--------------------------------------------------------------------------*/   
/* Z GET BYTE                                                               */   
/* Get a byte from the modem.                                               */ //??modem?¬Ý????????   
...
/*--------------------------------------------------------------------------*/   
/* Z TIMED READ                                                             */   
/* Read a character from the modem line with timeout.                       */  //??modem?¬Ø????????with timeout   
...
/*--------------------------------------------------------------------------*/   
/* Z GET BINARY HEADER                                                      */   
/* Receive a binary style header (type and position)                        */   //??????????????style header???????¥ë?a?   
/*--------------------------------------------------------------------------*/   
...
/*--------------------------------------------------------------------------*/   
/* RZ RECEIVE DATA                                                          */  //RZ????????   
/* Receive array buf of max length with ending ZDLE sequence                */  //??zdle??CRC???????¬Ý??????????????????   
...
CRCfoo:   
      switch (c) {   
        ...
        case ZZRCDO:    return c;       //No Carrier                    //u??????   
...
{ int i;   
  ...
  if ((i == ZCOMPL) || ((i == ZFILE) && ((RZ_ReceiveBatch(p)) == ZZOK))) {   
    SendHEX(1,rxbytes>>24); SendHEX(1,rxbytes>>16); SendHEX(1,rxbytes>>8); SendHEX(1,rxbytes);   
...
extern BYTE OneSramByte(DWORD add);   
