...
typedef struct _type_print_
{
	char	name[ 64];
	int		(*action)( ST_LOADER *st, struct _type_print_ *tp, STRUCT_MEMBER *member, FILE *fp, int *sum);
	int		size;
}	TYPE_PRINT;
 ...
 *
*************************************************************************************************/
int		FuncTypeChar( ST_LOADER *st, TYPE_PRINT *tp, STRUCT_MEMBER *member, FILE *fp, int *sum);
int		FuncTypeInt( ST_LOADER *st, TYPE_PRINT *tp, STRUCT_MEMBER *member, FILE *fp, int *sum);
int		FuncTypeLong( ST_LOADER *st, TYPE_PRINT *tp, STRUCT_MEMBER *member, FILE *fp, int *sum);
int		FuncTypeDouble( ST_LOADER *st, TYPE_PRINT *tp, STRUCT_MEMBER *member, FILE *fp, int *sum);
int		FuncTypeStTimeval( ST_LOADER *st, TYPE_PRINT *tp, STRUCT_MEMBER *member, FILE *fp, int *sum);
int		FuncTypeTimeT( ST_LOADER *st, TYPE_PRINT *tp, STRUCT_MEMBER *member, FILE *fp, int *sum);
int		FuncTypeSizeT( ST_LOADER *st, TYPE_PRINT *tp, STRUCT_MEMBER *member, FILE *fp, int *sum);
int		FuncTypeKeyT( ST_LOADER *st, TYPE_PRINT *tp, STRUCT_MEMBER *member, FILE *fp, int *sum);
/************************************************************************************************
 *
************************************************************************************************/
...
char FuncName[ 64] = "LogRaw";

TYPE_PRINT	TypePrint[ 128] =
{
	{ "char",				FuncTypeChar,		0 },
	{ "int",				FuncTypeInt,		sizeof( int) },
	{ "long",				FuncTypeLong,		sizeof( long) },
	{ "double",				FuncTypeDouble,		sizeof( double) },
	{ "struct timeval",		FuncTypeStTimeval,	sizeof( struct timeval) },
	{ "time_t",				FuncTypeTimeT,		sizeof( time_t) },
	{ "key_t",				FuncTypeKeyT,		sizeof( key_t) },
	{ "size_t",				FuncTypeSizeT,		sizeof( size_t) },
	{ "\0",					NULL,				-1 }
};
	...
	while( 1)
	{
		...
		switch( st->rec[ st->col])
		{
			...
			case ' ' :
				...
				{
					case 0:		/* type */
						member->type = realloc( member->type, st->w_cnt +1);
						if( member->type == NULL)
						{
						...
						if( !memcmp( member->type, "struct", 6) && st->w_cnt == 6)
						{
							st->word[ st->w_cnt++] = ' ';
						}
						else
			...
			case '*' :
				...
				else
				{
					...
					else
					{
						...
						LogDel( "member type=[%s] name=[%s] member=[%p]", member->type, member->name, member);
...
int StLoader_PrintFile( ST_LOADER *st, FILE *fp)
{
	...
	while( form != NULL)
	{
		...
		while( member != NULL)
		{
			for( i = 0; TypePrint[ i].action != NULL; i++)
			{
#if 0
				if( !memcmp( member->type, TypePrint[ i].name, strlen( member->type)))
#else
				if( !memcmp( member->type, TypePrint[ i].name, strlen( TypePrint[ i].name)))
#endif
				{
					rtn = TypePrint[ i].action( st, &TypePrint[ i], member, fp, &sum);
					if( rtn < 0)
					{
						break;
					}
			...
			if( TypePrint[ i].action == NULL)
			{
				/* printf( "unknown type. type=[%s]\n", member->type); */
				if( member->type[ 0] == '#')
				{
				...
				else
				{
					...
					if( ptr != NULL)	/* pointer type */
					{
						*ptr = 0;
						printf( "    %s_Print( ptr->%s);\n", member->type, member->name);
					}
					else
					{
						printf( "    %s_Print( ptr->%s);\n", member->type, member->name);
...
int FuncTypeChar( ST_LOADER *st, TYPE_PRINT *tp, STRUCT_MEMBER *member, FILE *fp, int *sum)
{
	...
	if( m_sz)
	{
		...
		fprintf( fp, "%s( \"%s%-20s %3d %4d = %s\\n\", \tptr->%s);", 
				FuncName, cmt_buf, member->name, m_sz, *sum, prt_buf, member->name);
		*sum += m_sz;
		fprintf( fp, "\n");
	}
	else
...
int FuncTypeStTimeval( ST_LOADER *st, TYPE_PRINT *tp, STRUCT_MEMBER *member, FILE *fp, int *sum)
{
	...
	fprintf( fp, "    ");
	sprintf( cmt_buf, "%-30.28s", StLoader_CommentTrim( st, member->comment));
	sprintf( prt_buf, "[%%s:%%06d]");
	fprintf( fp, "%s( \"%s%-20s %3d %4d = %s\\n\",  TtoS( ptr->%s.tv_sec), ptr->%s.tv_usec);", 
			FuncName, cmt_buf, member->name, m_sz, *sum, prt_buf, member->name, member->name);
	*sum += sizeof( struct timeval);
	/* *sum += tp->size; */
	fprintf( fp, "\n");
...
#if 0
int FuncType( ST_LOADER *st, STRUCT_MEMBER *member, FILE *fp, int *sum)
{
	int		m_sz; 
	char	cmt_buf[ 512];
	char	prt_buf[ 512];
