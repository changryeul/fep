...
char	*OptStr = "hc:";
char	Usage[] = 
"Usage: %s [-h] [-c config_file_name] [-f tcp_name]\n"
"Option: -o, --option                \n"
"  h, help                    this help message\n"
"  c, config=<cfg_name>       config file name (default:./main.cfg)\n"
"  f, file=<tcp_name>         tcp file name (default:./main.tcp)\n"
;
...
int InitProcess( int argc, char *argv[])
{
	...
	rtn = Cfg_Get( Cfg, "addr", Param->addr, 512);
	...
	/* tcp client open                              */
	/************************************************/
	Tcp = Tcp_OpenClient( Param->addr, Param->port);
	if( Tcp == NULL)
	{
		LogCri( "Tcp_Open error. addr=[%s] port=[%d]", Param->addr, Param->port);
		return -1;
	}
