...
struct _FX0067_DB_FORM {
    int     CheGyeolNo;                     /* 체결번호         */
    char    CheGyeolDate[SIZE_DATE+1];      /* 체결일자         */
    double  GeumAek;                        /* 금액             */
    double  SeonMulHwanYul;                 /* 선물 환율        */
    double  HyeonMulHwanYul;                /* 현물 환율        */
    char    FixingDate[SIZE_DATE+1];        /* Fixing Date      */
    char    ValueDate[SIZE_DATE+1];         /* Value Date       */
};
...
struct _FX0068_DB_FORM {
    ...
    char    CheGyeolDate[SIZE_DATE+1];      /* 체결일자         */
    double  GeumAek;                        /* 금액             */
    char    NearValueDate[SIZE_DATE+1];     /* Near value date  */
    char    FarValueDate[SIZE_DATE+1];      /* Far value date   */
    int     GiGan;                          /* 기간(일수)       */
    double  MaJin;                          /* 마진(전)         */
};
...
struct _FX0069_DB_FORM {
    char    Good_Fg[1+1];                   /* 상품구분         */
    int     CheGyeolNo;                     /* 체결번호         */
    char    CheGyeolDate[SIZE_DATE+1];      /* 체결일자         */
    double  GeumAek;                        /* 금액             */
    char    SiJakDate[SIZE_DATE+1];         /* 시작일           */
    char    JongRyoDate[SIZE_DATE+1];       /* 종료일           */
    int     GiGan;                          /* 기간(일수)       */
    double  GoJeongGeumLi;                  /* 고정금리         */
    double  ByeondongGeumLi;                /* 변동금리         */
    char    MaeIpEopCheCode[4+1];           /* 매입 업체 코드   */
    char    MaeDoEopCheCode[4+1];           /* 매도 업체 코드   */
};
...
struct _FX0073_DB_FORM {
    int     CheGyeolNo;                     /* 체결번호         */
    char    CheGyeolDate[SIZE_DATE+1];      /* 체결일자         */
    char    GiJunTongHwaCode[3+1];          /* 기준통화 코드    */
    char    SangDaeTongHwaCode[3+1];        /* 상대통화 코드    */
    char    SangPumCode[1+1];               /* 상품 코드        */
    char    GiilCode[2+1];                  /* 기일 코드        */
    char    CheGyeolTime[6+1];              /* 체결 시각        */
    char    MaeIpEopCheCode[4+1];           /* 매입 업체 코드   */
    char    MaeDoEopCheCode[4+1];           /* 매도 업체 코드   */
    double  MaeMaeYul;                      /* 매매율           */
    double  CheGyeolGeumAek;                /* 체결 금액        */
    double  HyeonMulHwanYul;                /* 현물 환율        */
    char    GyeolJeDate[SIZE_DATE+1];       /* 결제 일자        */
    char    ManGiDate[SIZE_DATE+1];         /* 만기 일자        */
    char    JangNaeOeFlag[1+1];             /* 장내/장외 구분   */
    char    NearValueDate[SIZE_DATE+1];     /* Near value date  */
    char    FarValueDate[SIZE_DATE+1];      /* Far value date   */
};
...
struct _FX0074_DB_FORM {
    ...
    char    CheGyeolTime[6+1];              /* 체결 시각                         */
    char    JangNaeOeFlag[1+1];             /* 장내/장외 구분                    */
    char    GyeolJeDate[SIZE_DATE+1];       /* 결제일                            */
    char    SiJakDate[SIZE_DATE+1];         /* 시작일                            */
    char    JongRyoDate[SIZE_DATE+1];       /* 종료일                            */
    char    HaengSaManGiDate[SIZE_DATE+1];  /* 행사만기일                        */
    char    GiilCode[2+1];                  /* 기일 코드                         */
    ...
    double  GoJeongGeumLi;                  /* 고정금리                          */
    char    ByeondongGeumLiCode[2+1];       /* 중개사 거래 변동금리 코드         */
    double  ByeondongGeumLi;                /* 변동금리                          */
    char    PutCall[1+1];                   /* Put/Call                          */
    char    Type[1+1];                      /* Type                              */
    double  OptionGaGyeokUS;                /* 옵션가격(달러화)                  */
...
typedef struct  {
    ...
    char    CheGyeolDate[SIZE_DATE];        /* 체결일자         */
    char    GeumAek[16];                    /* 금액             */
    char    SeonMulHwanYul[12];             /* 선물 환율        */
    char    HyeonMulHwanYul[12];            /* 현물 환율        */
    char    FixingDate[SIZE_DATE];          /* Fixing Date      */
    char    ValueDate[SIZE_DATE];           /* Value Date       */
} FX0067_DATA_FORM;
...
typedef struct  {
    ...
    char    CheGyeolDate[SIZE_DATE];        /* 체결일자         */
    char    GeumAek[16];                    /* 금액             */
    char    NearValueDate[SIZE_DATE];       /* Near value date  */
    char    FarValueDate[SIZE_DATE];        /* Far value date   */
    char    GiGan[4];                       /* 기간(일수)       */
    char    MaJin[16];                      /* 마진(전)         */
} FX0068_DATA_FORM;
...
typedef struct  {
    ...
    char    CheGyeolDate[SIZE_DATE];        /* 체결일자         */
    char    GeumAek[16];                    /* 금액             */
    char    SiJakDate[SIZE_DATE];           /* 시작일           */
    char    JongRyoDate[SIZE_DATE];         /* 종료일           */
    char    GiGan[4];                       /* 기간(일수)       */
...
typedef struct  {
    ...
    char    HaengSaManGiDate[SIZE_DATE];    /* 행사만기일       */
    char    MaeIpEopCheCode[4];             /* 매입 업체 코드   */
    char    MaeDoEopCheCode[4];             /* 매도 업체 코드   */
} FX0070_DATA_FORM;
...
typedef struct  {
    ...
    char    HyeonMulHwanYul[12];            /* 현물 환율        */
    char    GyeolJeDate[SIZE_DATE];         /* 결제 일자        */
    char    ManGiDate[SIZE_DATE];           /* 만기 일자        */
...
typedef struct  {
    ...
    char    CheGyeolTime[6];                /* 체결 시각                         */
    char    JangNaeOeFlag[1];               /* 장내/장외 구분                    */
    char    GyeolJeDate[SIZE_DATE];         /* 결제일                            */
    char    SiJakDate[SIZE_DATE];           /* 시작일                            */
    char    JongRyoDate[SIZE_DATE];         /* 종료일                            */
    char    HaengSaManGiDate[SIZE_DATE];    /* 행사만기일                        */
    char    GiilCode[2];                    /* 기일 코드                         */
