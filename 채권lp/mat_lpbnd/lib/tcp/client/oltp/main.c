...
int GetOption( int argc, char *argv[])
{
	...
	while( 1)
	{
		...
		if( opt != NULL) Param->argo[ Param->args++] = opt;
	}
	argc -=optind;
	argv = &argv[ optind];
...
int InitProcess( int argc, char *argv[])
{
	...
	Client = Tcp_OpenClient( Param->addr, Param->port);
	if( Client == NULL)
	{
		LogCri( "Tcp_OpenClient( Param->addr=[%s], Param->port=[%d]) error.", Param->addr, Param->port);
		return -1;
	}
