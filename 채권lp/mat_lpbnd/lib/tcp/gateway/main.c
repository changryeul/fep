...
PARAM	ParamBuf =
{
	...
	"",										/* argo */
	0,										/* args */
	"./msg_agent.cfg",						/* cfg_name[ 512] - from 환경변수 */
	"./msg_agent.log",						/* log_name[ 512]            */
...
char	*OptStr = "hc:";
char	Usage[] = 
"usage: %s -hdmf [-c config_file_name]\n"
"\tc: config file name (default:./main.cfg)\n"
"\th: this help message\n";
...
int InitProcess( int argc, char *argv[])
{
	...
	Cfg_Get( Cfg, "addr", Param->addr, 64);
	Param->port = Cfg_GetInt( Cfg, "port");

	Cfg_Get( Cfg, "svr_addr", Param->svr_addr, 64);
	Param->svr_port = Cfg_GetInt( Cfg, "svr_port");
	...
	Server = Tcp_OpenServer( Param->addr, Param->port);
	Tcp_Listen( Server);
...
int ParamPrint( PARAM *param)
{
	...
	LogDbg( "param->addr           = [%s]", param->addr);
	LogDbg( "param->port           = [%d]", param->port);
	return 1;
}
