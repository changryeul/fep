...
int InitProcess( int argc, char *argv[])
{
	...
	Param->key = Cfg_GetInt( Cfg, "key");
	Param->oms_key = Cfg_GetInt( Cfg, "oms_key");
	...
	if( Blp == NULL)
	{
		LogCri( "Blp_Open error.");
		Blp = Blp_Create( Param->key);
		if( Blp == NULL)
		{
			LogCri( "Blp_Create error.");
			return -1;
		}
		LogMsg( "Blp_Create success. 공유 메모리를 생성 했습니다.");
	}

	Mem = Mem_Open( Param->oms_key);
	if( Mem == NULL)
	{
		LogCri( "Mem_Open error.");
		return -1;
	}
	Shm_Note = Mem->ptr;
...
int ParamPrint( PARAM *param)
{
	...
	LogMsg( "param->key            = [0x%08x]", param->key);
	return 1;
}
...
**  프로그램 수행
***************************************************************************** */
int FileProcess( char *file_name, int file_type, char *buffer)
{
	int				rtn = 0;
	int				cnt = 0;
	char			*ptr;
	...
	if( FilePtr == NULL)
	{
		...
		if( FilePtr == NULL)
		{
			LogErr( "fopen error. name=[%s]", file_name);
			return -1;
		}
		LogMsg( "File open. name=[%s] FilePtr=[%p]", file_name, FilePtr);
	}
	...
	while( Continue)
	{
		...
		if( ptr == NULL)
		{
			...
			*/
			LogMsg( "EOF ... name=[%s] FilePtr=[%p]", file_name, FilePtr);
			return 1;
		}
		DataStruct->line++;
		...
		memcpy( buffer, DataStruct->rec, DataStruct->rec_sz);
		LogDbg( "DATA[%4d:%s]", DataStruct->rec_sz, DataStruct->rec);
		break;
	}
...
int StructProcess( DATA_STRUCT *ds)
{
	...
	if( !memcmp( field, " BLP_ARG_IF ", 12 +1))
	{
		if( ds->field == 0)		/* start */
		{
			LogDel( "field start .... ");
