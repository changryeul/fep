...
#include    "key_code.h"
#include    "strategy.h"
#include    "fx.h"          /* FX wire(CO_B6FX)/주문(SMB_ST) - SHM_FX/FX_Sise 의존, 유안타 이식 */

/*------------------------------------------------------------------------
  SHM Version Marker (stored in SUB_DAEMON_INFO.process_info[32..39])
------------------------------------------------------------------------*/
#define     SHM_VERSION         20260221                /* YYYYMMDD      */
#define     SHM_MAGIC           0x46455053              /* "FEPS" ASCII  */
...
/* 시장별로 각 2억씩 잡는다 */
#define     MK01_ORDER_NO_BASE        200000000        /* 채권 주문번호 채번 사용구간 시작번호	 200,000,000 */
#define     MK02_ORDER_NO_BASE        200000000        /* 파생 주문번호 채번 사용구간 시작번호	 200,000,000 */

#define     MK01_ORDER_NO_BACK_GAP    100000000        /* 채권 주문번호 채번 사용구간 갭_2번서버 100,000,000 */
#define     MK02_ORDER_NO_BACK_GAP    100000000        /* 파생 주문번호 채번 사용구간 갭_2번서버 100,000,000 */

#define     MK01_ORDER_NO_GAP           1000000        /* 채권 주문번호 채번 사용구간별 갭 		 1,000,000	*/
#define     MK02_ORDER_NO_GAP           1000000        /* 파생 주문번호 채번 사용구간별 갭 		 1,000,000	*/
...
#define     SHM_MAX_FX               30                 /* 6. FX item cnt - yanta port */
#define     SHM_MAX_RDS01	        100000              /* 3. 국채KTS							*/
#define     SHM_MAX_DEV_FIF           1000              /* 6. 금융상품선물(국채,금리,통화)		*/
#define     SHM_MAX_DEV_FIF_N         1000              /* 6. FinFut night(_N) - yanta port */
#define     DEV_MK_FIF_N        2                       /* FinFut night(_N) mkt - yanta port */
                                                        /* Financial Instruments Futures		*/
                                                        /* 여기서는 요상품만 거래				*/

#define     SHM_MAX_PREMATCH    RISK_MK_CNT             /* 미체결용, 실사용시장 2개만 사용, MK_CNT사용하지 않고 RISK_MK_CNT로 사용, 소스포함   */
...
typedef struct {
    ...
    char        logon_id[20];       /* Logon ID                         */
    char        logon_pw[20];       /* Logon Password                   */
...
/* FX Sise - yanta_pa port (FX_SISE_FORMAT byte-exact) */
typedef struct                      
{                       
    int             auto_use;                   /* �ڵ������⵿�� �ش����� +- ����, ������ ��쿡�� ������ �����Ѵ�. */
    int             dont_trade;                 /* �ŸźҰ���������(��ħ�� �ް�, �ǽð����� �����Ȱ� �ޱ�   */
    /* **************************************************************************** */
    /* A0, Master Data */
	char			m_exch[1];					/* �ŷ��� code					*/
    char            m_item_cd[7];				/* symbol						*/
	char			complete_f[1];						/* ask�� bid�� ���� ������ ��� 2���� ���� ó���ϱ� ���� */
#if	0
    double          m_h_lmt;                        /* Hi Limit                     */
    double          m_l_lmt;                        /* Low Limit                    */
    char            m_item_stat[1];                 /* Item Status , 0:Normal, !0:Cannot*/
													/* ä���� �ŷ��������� �׸����� ������� (trade_susp_yn), Y/N */
	/* �̻�� �׸� 2025 */
//    char            item_stat[1];                   /* ������������ ����ŷ����� ����, Item Status 0:Normal, !0:Cannot  */
													/* ������������ ��� ������ �����̴�.  */
    double          m_stdard_price;                 /* Standard Price               */
                                                    /* ���ذ����, �������ذ��� �Ļ��� �ִµ� ���ذ��� ��� */
// ä���� �̻��, �Ļ��� ���
    int             m_cds_gbn_2;                    /* ������ ȣ�������ڵ� */
    int             m_cds_gbn_I;                    /* ���Ǻ������� ȣ�������ڵ� */
    int             m_cds_gbn_T;                    /* ���尡 ȣ�������ڵ�(1��� �Ļ���) */
    int             m_cds_gbn_W;                    /* ������������ ȣ�������ڵ�(X��� �Ļ���) */
// ä���� �̻��, �Ļ��� ���

    double          m_max_qty;                      /* ���Ѽ���(ä�Ǿ���)  */
    char            m_clearance_gbn[1];             /* Y:�����Ÿ�, N:�Ϲ�(ä�Ǿ���)     */
    char            m_start_price_gbn[1];           /* Y:�ð����ذ�����, N:�Ϲ�(ä�Ǿ���)  */
    // Add, 20200602, ä���� ����
    int             m_multiplier;                   /* �¼�, ������ 1�� ó��    */
	double			m_striking_price;				/* ��簡, �Ļ������,����1	17(9V8)�� 11�ڸ� �Ҽ���2�ڸ������� ��� */
    char            m_closeday[8];                  /* �ŷ�������                */
    double          basic_asset_price;              /* �����ڻ� ��������        */
#endif
    /* **************************************************************************** */
	/* �ü�����																		*/
    /* **************************************************************************** */
    ...
    double          sell_1_price;               /* Sell 1 Price         */                      
    double          buy_1_price;                /* Buy 1 Price          */                      
	char            bid_quote_id[30];               /* Quote ID - ���ѿ�    */
	char            ask_quote_id[30];               /* Quote ID - ���ѿ�    */
#if	0
    double          realtime_hprc;              /* Runnig Time Hi Limit , run_tm_h_lmt, �Ļ���,ä��X  */
    double          realtime_lprc;              /* Runnig Time Low Limit, run_tm_l_lmt, �Ļ���,ä��X  */
#endif
    /* **************************************************************************** */
}   FX_SISE_FORMAT;                       
...
typedef  struct
{
    ...
    int             check_cnt;              /* 보유여부 확인(평가금액용)    */
                                            /* 해당종목을 보유한 관리계좌가 있는지, 평가금액 확인 때문에 */
    char            auto_use[MAX_AUTO_PROC];  /* per-strategy auto_use (yanta array) - was int scalar (P4-1 A) */
...
/* FX Shared Memory - yanta_pa port (byte-exact; CO_* deps from krx_mk.h) */
typedef  struct
{
    int             total_item_cnt;         /* �����, arry 0���� �����Ѵ�. */

    int             check_cnt;              /* �������� Ȯ��(�򰡱ݾ׿�)    */
                                            /* �ش������� ������ �������°� �ִ���, �򰡱ݾ� Ȯ�� ������ */
#if	1
    char			auto_use[MAX_AUTO_PROC];/* �ڵ������⵿�� �ش����� ���� index�� 1 ����, ������ ��쿡�� ������ �����Ѵ�. */
#else
    int             auto_use;               /* �ڵ������⵿�� �ش����� +- ����    */
#endif
	int             sise_delay;             /* �ŷ��ҽð��� ���ؼ� 1���̻� ���̹߻� �� 1�� ����*//* 2026/02/12 add */
	time_t          sise_delay_tm;          /* sise_delay flag �� ������ time                    *//* 2026/02/12 add */

    int             CURR_Arry_Key;          /* �ֱ� �ü����� 30���� Key     */
                                            /* �ü����� 30�� ��￡�� ���  */
    int             Befor_CURR_Arry_Key;    /* CURR_Arry_Key ������ ���    */
#if	0
    int             HogaLastGbn;            /* 0:G7, 1:B6                   */
#endif
...
*/
    FX_SISE_FORMAT                  FX_Sise[1][SHM_MAX_FX];   /* FX Sise (yanta port), filled by pf_ */
...
------------------------------------------------------------------------*/
#define     BASE_SHM_KEY    0x41000000L                 /* base SHM key */
...
------------------------------------------------------------------------*/
#define     NOTE_SHM_KEY    0x41000001L                 /* KTS 채권		*/
#define     RDS01_SHM_KEY	0x41000002L                 /* RDS01		*/
#define     FF_SHM_KEY      0x41000003L                 /* 금융상품선물	*/
#define     FF_N_SHM_KEY    0x41000004L                 /* FinFut night(_N) - yanta port (FEP scheme) */
#define     FX_SHM_KEY      0x41000005L                 /* FX - yanta port (FEP scheme) */

#define     B_SHM_KEY       0x41000011L                 /* BackOffice   */
#define     STRRG_SHM_KEY   0x41000012L                 /* 전략SHM        */
#define     MK_PM_SHM_KEY   0x41000013L                 /* 미체결처리용   */
#define     RISK_SHM_KEY    0x41000014L                 /* 한도관리SHM  */

#define     ITEM_SHM_KEY    0x41000030L                 /* Key Sort용    */

/*------------------------------------------------------------------------
  Base SHM Accessors
  i = daemon key (D_K), j = process/element index

  INFO(i)    -> ALL_DAEMON_INFO (daemon SHM, 26 entries)
  DAEMON(i)  -> SUB_DAEMON_INFO (sub-daemon SHM header)
  PROC(i,j)  -> PROCESS_INFO
  FILEM(i,j) -> FILE_INFO
  DSHM(i,j)  -> DSHM_INFO
  TCP1(i,j)  -> TCP1_INFO
  TCP2(i,j)  -> TCP2_INFO
  UDPIP(i,j) -> UDPIP_INFO
------------------------------------------------------------------------*/
...
/* TCP1 chain (4-level expansion):
   TCP1_PORT(i,j) -> TCP1(i, PROC(i,j).l.t1.line_gubun-1).port_no
     -> Shm_Mem[i].Tcp1[Shm_Mem[i].Proc[j].l.t1.line_gubun - 1].port_no
   Usage: TCP1_PORT(D_K, P_K) -- current process TCP1 master port */
...
/* TCP2 chain (4-level expansion, k = line index):
   TCP2_PORT(i,j,k) -> TCP2(i, PROC(i,j).l.t2.l[k]-1).port_no
     -> Shm_Mem[i].Tcp2[Shm_Mem[i].Proc[j].l.t2.l[k] - 1].port_no */
...
/* UDP chain (3-level expansion):
   UDP_PORT(i,j,k) -> UDPIP(i, PROC(i,j).l.u-1).port[k]
     -> Shm_Mem[i].Udpip[Shm_Mem[i].Proc[j].l.u - 1].port[k] */
...
/* input data SHM (DSHM circular buffer)
   IDN(i,j,k) -> data_name,  IDK -> key_info,  IDW -> w_cnt,  IDR -> r_cnt
   IDO -> offset (start of circular buffer), IDM -> max_rec, IDS -> data_size */
...
/* output data SHM (DSHM circular buffer)
   ODN(i,j,k) -> data_name,  ODK -> key_info,  ODW -> w_cnt,  ODR -> r_cnt
   ODO -> offset (start of circular buffer), ODM -> max_rec, ODS -> data_size */
...
extern  SHM_FX          *Shm_FX;                /* FX (yanta port)              */
