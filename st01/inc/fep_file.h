...
#define     TS_W1_2     11              /* output file 1, write count 1 */
...
typedef struct {
	...
	char	If_Seq[8];				/* ����Ϸù�ȣ						*/
	char	ApType[8];				/* �������нĺ���					*/
	char	ResponseCode[4];   		/* �����ڵ�							*/
	char	RecvTime1[10];			/* system �ð� (nnnnnnnnnn (sec))	*/
	char	RecvTime2[12];			/* system �ð� (HH:MM:SS-mmm)		*/
...
typedef struct {
	char	Seq[8+2];				/* �Ϸù�ȣ							*/
	char	If_Seq[8+2];			/* ����Ϸù�ȣ						*/
	char	ApType[8+2];			/* �������нĺ���					*/
	char	ResponseCode[4+2];		/* �����ڵ�							*/
	char	RecvTime1[10+2];		/* system �ð� (nnnnnnnnnn (sec))	*/
	char	RecvTime2[12+2];		/* system �ð� (HH:MM:SS-mmm)		*/
	char	DataHeader[20+2];		/* data header					    */
}	FILE_RW_HEAD;

/* �ü� file header (8 + 12 = 20 bytes)	*/
typedef struct {
	char	Seq[8];
	char	RecvTime[12];
}	SISE_RW_HEAD;
...
/* KB ������(����,��� ��) */
typedef struct {
	char	BodyLength[3];			/* BodyLength */
	char	Channel[1];				/* E:equity, G:Algo */
	char	Commodity[1];			/* C:stock, A:deriv, B:bond, D:FX */
	char	FcmId[10];				/* FCM ID */
	char	RuleNo[15];				/* Algo rule */
	char	ReferenceNo[15];		/* Algo ref */
	char	GroupNo[10];			/* Algo group */
}	KB_IN_HEADER;
