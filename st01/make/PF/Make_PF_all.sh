##########################################################################
#	Module	: make shell - PF (FX venue) - 개선 빌드모델 (1 소스 = 1 바이너리)
#	File	: Make_PF_all.sh
#
...
FEP_SRC_TMP=$_PF_SRC;	export FEP_SRC_TMP
FEP_OBJ_TMP=$_PF_OBJ;	export FEP_OBJ_TMP
...
# PO 기능 플래그는 기본 없음(순수 크로스커팅). 필요 시 _PF_FEATURE_DEF로 재정의.
FEATURE_DEF="${_PF_FEATURE_DEF:--DNO_AGXPI}"
...
do
	...
	echo "$RUN_TMP"
	make -f ${_PF_MAKE}/Make_PF_c.mk
	built=`expr $built + 1`
done

if [ $built -eq 0 ]; then
	echo "PF: no sources in ${_PF_SRC} (scaffold no-op)"
fi

##########################################################################
#	End of File (Make_PF_all.sh)
##########################################################################
