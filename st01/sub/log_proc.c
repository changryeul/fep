...
/*----------------------------------------------------------------------*/
{
    ...
    rt = write(fd, p_msg, strlen(p_msg));
    if (rt < 0)
        Log(FIF_FATAL, "Log_Proc:cannot write [%d:%s]", SYS_NO, SYS_STR);
...
/*----------------------------------------------------------------------*/
{
    ...
    rt = write(fd, p_msg, strlen(p_msg));
    if (rt < 0)
        Log(FIF_FATAL, "Log_Emergency:cannot write [%d:%s]", SYS_NO, SYS_STR);
...
/*----------------------------------------------------------------------*/
{
    ...
    if (strlen(mbuf) > SHM_LOG_SIZE - SHM_LOG_HEAD_SIZE)
        sprintf(buf, "%.*s...", (int)(SHM_LOG_SIZE - SHM_LOG_HEAD_SIZE - 3), mbuf);
    else
        sprintf(buf, "%s", mbuf);
    ...
    /* 2025EDIT */
    snprintf(msg, sizeof(msg), "%04d%04d%-10.10s%02d%02d%02d%06ld%.*s",
            (int)(SHM_LOG_HEAD_SIZE + strlen(buf)), p_err_no, _Exe_Name,
            date->tm_hour, date->tm_min, date->tm_sec, tv.tv_usec,
            (int)strlen(buf), buf);
