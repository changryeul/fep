...
/*----------------------------------------------------------------------*/
{
    ...
    char            f_name[100], tmp[20], bumun[4];
    char            buf[1024];
    FILE            *fp;
    struct timeval  tv;
    ...
    if (PROC(D_K,P_K).type == TY_TRS1) {
        ...
        sprintf(buf, "%08d%08d%08d%08d%08d%08d%08d%08d%08d%08d%08d%d%d",
                IF_SEQ(D_K,P_K),
                IF_MEG_SEQ(D_K,P_K,0), IF_MEG_SEQ(D_K,P_K,1), IF_MEG_SEQ(D_K,P_K,2),
                IF_MEG_SEQ(D_K,P_K,3), IF_MEG_SEQ(D_K,P_K,4), IF_MEG_SEQ(D_K,P_K,5),
                IF_MEG_SEQ(D_K,P_K,6), IF_MEG_SEQ(D_K,P_K,7), IF_MEG_SEQ(D_K,P_K,8), IF_MEG_SEQ(D_K,P_K,9),
                START_STAT(D_K,P_K), TCP2_LINE_GUBUN(D_K,P_K));
