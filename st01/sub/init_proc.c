...
{
    int     i, j, pk, shmid, sk;
    char    fifo_name[256], bumun[4], key[32], attach_flag[MAX_DSHM_SEG];
    ...
    /* attach sise data SHM (전역 의미별 세그먼트: 전략 STRRG/미체결 MK_PM/한도 RISK/시세 NOTE·FF).
       게이트 제거(2026-08-06): sise config 파싱이 '202201' 리팩터로 주석처리되고
       SiseTr_Config_Read가 프로토타입만 남아 DAEMON.sisetr_count가 구조적 항상 0 →
       기존 `if(sisetr_count>0)` 게이트가 늘 닫혀, 미체결/한도/전략 프로세스가
       MK_PM/RISK/STRRG를 attach 못하고 Make_MiChe 등에서 NULL 역참조 크래시하던 문제.
       의미별 SHM은 pz_memory_mp가 전역 고정키(0x41000001~30)로 선행 생성하므로 무조건 attach 안전. */
    Sise_SHM();
