...
echo "[5] 실 FEP FX 다거래원 pf_1100_ts (E2E, 라우팅+fill_rule 4종+취소)"
run_one "VX-4e FX order matrix (run_vx_fx_e2e)" sh "$VEXCH/run_vx_fx_e2e.sh"
echo "[6] 실연동 안전검증: pf_1100_ts msgq 모드 + 실 큐키 + SMB_ST"
run_one "VX-4f FX msgq (run_vx_fx_msgq)" sh "$VEXCH/run_vx_fx_msgq.sh"
echo "--------------------------------------------------------"
...
echo "  실연동 안전검증 (pf_1100_ts msgq 모드 + 실 큐키 + SMB_ST):"
echo "    sh $VEXCH/run_vx_fx_msgq.sh"
