...
PIDS=""; VPORT=19100; ORDFIFO=/tmp/vxfx_ord.fifo; CLORD=FEPFX00000042
...
# --- 가상 FX 거래원 기동 ---
log "start mock_fx_venue $VPORT ..."
VX_CATALOG=$CFG/vexch.ini $INTEG/bin/mock_fx_venue $VPORT > "$RESULT/vxfx_venue.log" 2>&1 &
MPID=$!; PIDS="$PIDS $MPID"; sleep 1

# --- pf_1100_ts 기동 (거래원 접속 + 주문 FIFO 트리거) ---
log "start pf_1100_ts (venue $VPORT, fifo $ORDFIFO) ..."
( cd $BIN && ulimit -t 30 2>/dev/null;
  VX_FX_HOST=127.0.0.1 VX_FX_PORT=$VPORT VX_FX_ORDER_FIFO=$ORDFIFO VX_TEST=1 \
  exec bash -c "exec -a pf_1100_ts $BIN/pf_1100_ts" ) </dev/null > "$RESULT/vxfx_pf.log" 2>&1 &
APID=$!; PIDS="$PIDS $APID"
( sleep 20; kill -9 $APID $MPID 2>/dev/null; pkill -9 -x pf_1100_ts 2>/dev/null ) & WDPID=$!
sleep 3
pgrep -x pf_1100_ts >/dev/null || fail "pf_1100_ts 미기동(로그 $RESULT/vxfx_pf.log)"

# --- 주문요청 주입 → SMB_ST 송신 → 체결 수신 ---
log "주문 주입: $CLORD"
printf 'FXACCT0001,USD/KRW,1,1000000,1385.50,%s\n' "$CLORD" > "$ORDFIFO"
sleep 3
kill -9 $APID $MPID 2>/dev/null; pkill -9 -x pf_1100_ts 2>/dev/null; kill $WDPID 2>/dev/null; sleep 1
...
if [ -n "$PLOG" ]; then
    ...
    CORR=$(grep -c "ClOrdID=$CLORD" "$PLOG")
else CONN=0; SENT=0; EXEC=0; CORR=0; fi
CONN=${CONN:-0}; SENT=${SENT:-0}; EXEC=${EXEC:-0}; CORR=${CORR:-0}
CORES=$(ls $E2E/core.* $BIN/core.* 2>/dev/null|wc -l)
echo ""
echo "========================================"
echo "  VX-4b: FEP FX 주문(pf_1100_ts) ↔ 가상 거래원"
echo "========================================"
echo "  거래원 접속(connected)   : $CONN"
echo "  주문 송신(ORDER send)    : $SENT"
echo "  체결 수신(VX_TEST exec)  : $EXEC   (New ack + 체결 = 2 기대)"
echo "  ClOrdID 상관($CLORD): $CORR"
echo "  core dump                : $CORES"
echo "  판정 : $([ "${CONN:-0}" -ge 1 ] && [ "${SENT:-0}" -ge 1 ] && [ "${EXEC:-0}" -ge 2 ] && [ "${CORR:-0}" -ge 1 ] && [ "${CORES:-0}" -eq 0 ] && echo 'PASS (FEP FX 주문→거래원→체결 왕복, 상관 확인)' || echo 'FAIL/PENDING')"
echo "========================================"
echo "--- pf_1100_ts log (마지막 15줄) ---"; [ -n "$PLOG" ] && tail -15 "$PLOG" 2>/dev/null
echo "--- mock_fx_venue log (마지막 10줄) ---"; tail -10 "$RESULT/vxfx_venue.log" 2>/dev/null
cleanup; log "done"
