...
#             pa_7010_us.c와 동일 로직이며 접속처 수(ACC_NO_CNT)만 다름.
------------------------------------------------------------------------*/
    ...
    while (START_S != JOB_END) {
        ...
        if (rt == 1) {
            ...
            if (memcmp(Shm_Futures[0].Futures_CURR_Arry[Shm_Futures[0].CURR_Arry_Key].tot_con_qty,
                ...
                memcpy(send_data, Shm_Futures[0].Futures_CURR.tr_gbn, sizeof (SIF_G7) - 8);
            }
            ...
            if (memcmp(Shm_Options[ITEM_CODE].Options_CURR_Arry[Shm_Options[ITEM_CODE].CURR_Arry_Key].tot_con_qty,
                ...
                memcpy(send_data, Shm_Options[0].Options_CURR.tr_gbn, sizeof (SIO_G7) - 8);
            }
#endif
        }
...
/*    설정(ACCNO)에서 IP를 읽어 ACC_NO_CNT개 UDP 소켓을 생성한다.      */
/*    Return: 1=성공, 0=실패                                            */
/*----------------------------------------------------------------------*/
