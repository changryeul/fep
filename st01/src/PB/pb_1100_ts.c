...
#define     CHK_PROC        "pb_1201_tr"                    /* 체결Process가 인터페이스종료(TCHEDP99000) 수신후 종료되면 주문송신도 종료 */
...
/*----------------------------------------------------------------------*/
{
    ...
    */
    ...
    /* sfmt-overflow-fix: RecvLen이 구조체 크기를 넘으면 BSS 오버런
       (E2E에서 336B 응답으로 재현·확인) - 구조체 크기로 클램프 */
    ...
    if (FirstSeq > INT_SEQ) {  // 수신받은 Seq가 나의 INT_SEQ보다 클수없다. 오류후 Process종료.
        ...
        /* 20251223
            DATA오류를 받으면
            1. 현재 Seq를 백업받는다.
            2. 수신받은 Seq -1로 변경한다.(INT_SEQ는 변경하지 않는다)
            3. 거부 및 오류 처리 후 원복한다.  */
        /* Seq BackUp */
        ...
        /* rdcnt-underflow-guard: 되감기가 읽기커서를 음수로 만들면
           재전송 폭주 위험 → fail-safe 중단+알림 (운영 결정) */
        if (mun > RD_CNT) {
        ...
        /* Seq 원복
        RD_CNT  = back_rd_cnt;
        Log (USR_OK, "Changed back_rd_cnt[%d] RD_CNT[%d]", back_rd_cnt, RD_CNT);
        */

        /* 20251223
            소켓 종료하지 않고 그냥 처리한다.
            예, 100번 송신 & 90번 주문거부면 90번 주문거부처리 하고 101번부터 처리하면됨 */
    }
...
/*----------------------------------------------------------------------*/
{
    ...
    if (rt == 0) {  // 정상
        ...
        Device_Write();
#ifdef LAT_TRACE
        LAT_POINT("OUT", &R_Fmt[0].Data[39], 10);
#endif

        // 순서 위아래
        INT_SEQ++;
        /* F5 order-pipeline-dshm: 입력 종류에 맞는 커서 전진 */
        ...
        while (START_S != END) {
            ...
            if (memcmp(S_Fmt.Data, "0000", 4) == 0) {
                ...
                if (FirstSeq > INT_SEQ) {
                    ...
                    /* rdcnt-underflow-guard: 되감기가 읽기커서를 음수로
                       만들면(=큐에 남은 것보다 이전 주문 재전송 요구) 재전송
                       폭주 위험. 운영 결정에 따라 fail-safe 중단+알림
                       (FirstSeq>INT_SEQ 케이스와 동일 처리) */
                    if (mun > RD_CNT) {
                        TCP2_LINE_ST = OpenFlag = OFF;
...
/*----------------------------------------------------------------------*/
{
    ...
    if (cnt < 0 || cnt > 1) {
        Log(SAM_ERROR, "DSHM_R(PS_R_1)[%s] 읽기실패 거부처리 못했음_1, cnt[%d] back_rd_cnt[%d] RD_CNT[%d] IDR(D_K,P_K,0,0)[%d][%d]", IDN(D_K,P_K,0), cnt, back_rd_cnt, RD_CNT, IDR(D_K,P_K,0,0), IDR_CNT(0,0));
        return;  // 돌아가서 정리하고 죽는다.
    }
    else if (cnt == 0) {
        Log(SAM_ERROR, "DSHM_R(PS_R_1)[%s] 읽기실패 거부처리 못했음_2, cnt[%d] back_rd_cnt[%d] RD_CNT[%d] IDR(D_K,P_K,0,0)[%d][%d]", IDN(D_K,P_K,0), cnt, back_rd_cnt, RD_CNT, IDR(D_K,P_K,0,0), IDR_CNT(0,0));
        return;  // 돌아가서 정리하고 죽는다.
    }
    ...
    rt = F_W(TS_W1_1, (void *)&W_Fmt, 1);  // OFN1 = pb_1401_ts
    if (rt != 1) {
...
/*----------------------------------------------------------------------*/
{
    ...
    switch (tr_code) {
        ...
        case    TR_DATA:    /* 주문요청 + Kill Swith */
            ...
            memset(&J_Q_Fmt,           0,              sizeof (KRX_NOTE_ALL_JUMUN_Q_FMT));
...
/*----------------------------------------------------------------------*/
{
    ...
    /* F5 order-pipeline-dshm: proc.ini 설정(IDN/IFN)으로 경로 자동 선택 */
    if (PROC(D_K,P_K).in_d[0] != 0) {
    ...
    */
    ...
    for (i = 0; i < r_cnt; i ++) {
        ...
        LOAD_CNT ++;
#ifdef LAT_TRACE
        /* 주문전문 OrderNo: DataSeq(11)+TrCode(11)+Megrp(2)+MktId(3)+Board(2)+Member(5)+Branch(5) = offset 39 */
        LAT_POINT("IN", &R_Fmt[i].Data[39], 10);
#endif
        /*
        2025 암복호화때문에 로직 변경됨
        */
        ItoAf(INT_SEQ+1, J_Q_Fmt.JumunData[i].DataSeq, sizeof (J_Q_Fmt.JumunData[0].DataSeq));
        ...
        // 값 요청 때문에
        Log_Hot(USR_OK, "&R_Fmt[i].Data [%s]", &R_Fmt[i].Data);
...
/*----------------------------------------------------------------------*/
{
    ...
    // 1. 암호화 수행 (EnCtx는 핸드셰이크로 이미 초기화된 상태라고 가정)
    ...
    // enc_data는 INL 내부에서 malloc/alloc 된 버퍼
    // 호출자가 반드시 INL_Free_Buf() 또는 free()로 해제해야 함
    return enc_data;
}
#endif
