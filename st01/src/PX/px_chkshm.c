...
{
    puts("\n<SHM Size>");
    printf(" SHM_MEMORY      = %ld", sizeof (SHM_MEMORY));
    printf("\n ALL_DAEMON_INFO = %ld", sizeof (ALL_DAEMON_INFO));
    printf("\n SUB_DAEMON_INFO = %ld", sizeof (SUB_DAEMON_INFO));
    printf("\n PROCESS_INFO    = %ld", sizeof (PROCESS_INFO));
    printf("\n FILE_INFO       = %ld", sizeof (FILE_INFO));
    printf("\n DSHM_INFO       = %ld", sizeof (DSHM_INFO));
#if defined ISAM_INCL
    printf("\n CISAM_INFO      = %ld", sizeof (CISAM_INFO));
#endif
    printf("\n LINE_INFO       = %ld", sizeof (LINE_INFO));
    printf("\n TCP1_INFO       = %ld", sizeof (TCP1_INFO));
    printf("\n TCP2_INFO       = %ld", sizeof (TCP2_INFO));
    printf("\n UDPIP_INFO      = %ld", sizeof (UDPIP_INFO));
    printf("\n SISETR_INFO     = %ld", sizeof (SISETR_INFO));
...
#else
    printf("        P[%d] F[%d] DSHM[%d] T1[%d] T2[%d] U[%d] SI[%d] AC[%d]\n",
            INFO(i).process_count, INFO(i).file_count, INFO(i).dshm_count,
...
#if defined ISAM_INCL
        printf("        P[%3d,%3d,%7zu]  F[%3d,%3d,%7zu]  DSHM[%3d,%3d,%7zu] CI[%3d,%3d,%7zu]\n",
...
#else
        printf("        P[%3d,%3d,%7zu]  F[%3d,%3d,%7zu]  DSHM[%3d,%3d,%7zu]\n",
                DAEMON(i).process_count, DAEMON(i).p_count, DAEMON(i).Shmsize,
                DAEMON(i).file_count, DAEMON(i).f_count, DAEMON(i).FShmsize,
                DAEMON(i).dshm_count, DAEMON(i).d_count, DAEMON(i).DShmsize);
#endif
        printf("       T1[%3d,%3d,%7zu] T2[%3d,%3d,%7zu]\n",
                DAEMON(i).tcp1_count, DAEMON(i).t1_count, DAEMON(i).T1Shmsize,
                DAEMON(i).tcp2_count, DAEMON(i).t2_count, DAEMON(i).T2Shmsize);
        printf("        U[%3d,%3d,%7zu] SI[%3d,%3d,%7zu] AC[%3d,%3d,%7zu]\n",
...
/*----------------------------------------------------------------------*/
{
    ...
    printf("%c%c   PROC[%d,%s,%s,%d] P_SEQ[%d]\n", _System_Name[0], i + 'A',
            PROC(i,j).process_no, PROC(i,j).process_path,
            PROC(i,j).process_id, PROC(i,j).process_status, j);
    ...
    printf("         [%s,%-6.6s,%-6.6s:%-4.4s,%-6.6s:%d,%d,%s]\n",
