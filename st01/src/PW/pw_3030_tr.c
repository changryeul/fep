...
/*----------------------------------------------------------------------*/
void    JW_3030_TR(int argc, char **argv)
/*----------------------------------------------------------------------*/
{
    int     rt;
...
/*----------------------------------------------------------------------*/
{
    ...
    for (i = 0; i < PROC_MAX; i ++) {
        if (PROC_MAX == 1)
            sprintf(chk_proc, "%-4.4s101_ts", _Exe_Name);
        else
...
/*----------------------------------------------------------------------*/
{
    int             i, next, seq, rt, len, f_size, rec_size;
    ...
    rt = DSHM_W(TS_W1_1, w_buf, DataCnt);
...
/*----------------------------------------------------------------------*/
{
    ...
    SIG_WRITE_MSG("[SIGNAL] pw_3030_tr caught signal\n");
