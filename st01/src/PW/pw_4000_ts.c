...
/*----------------------------------------------------------------------*/
void    JW_4000_TS(int argc, char **argv)
/*----------------------------------------------------------------------*/
{
    int     i, rt, seq;
    ...
    if (argc != 3) {
        SLog(USR_FATAL, "invalid arguments number[%d]", argc);
        Exit_Process();
    }
    ...
    SLog(USR_OK, "Sockfd[%d] line_gubun[%d] port_type[%d]",
    ...
    if (memcmp(R_Pkt->MsgType, TCP_RSND_CD, strlen(TCP_RSND_CD)) == 0) {
        ...
        if (seq > WR_CNT || seq < 0) {
            ...
            SLog(USR_ERROR, "RSND:invalid SeqNo[%d:%d]", seq, WR_CNT);
            PktType = T_RSOK;
            Send_Packet();
            sleep(3);
            return;
        ...
        SLog(USR_OK, "received RSND request[%d]", seq);
        RD_CNT = seq;
        INT_SEQ = seq;
    }
    else
...
/*----------------------------------------------------------------------*/
{
    ...
    SLog(USR_OK, "max block count [%d]", MaxCnt);
...
/*----------------------------------------------------------------------*/
{
    ...
    if (recv_len != TCP_HEAD_LEN || recv_len != rt) {
        SLog(USR_ERROR, "invalid length[%d:%d,%d]",
    ...
    if (memcmp(R_Pkt->MsgType, TCP_POOK_CD, strlen(TCP_POOK_CD)) == 0) {
        ...
        if (seq != INT_SEQ)
            SLog(USR_WARN, "POOK:SeqNo [%d:%d]", seq, INT_SEQ);

        return (OK);
    }
    else if (memcmp(R_Pkt->MsgType, TCP_STOP_CD, strlen(TCP_STOP_CD)) == 0) {
        SLog(USR_OK, "received STOP request");
        TCP1_NET_STA = END;
        close(Sockfd);
        Exit_Process();
    }
...
/*----------------------------------------------------------------------*/
{
    ...
    if (AtoIf(R_Pkt->ResponseCode, sizeof (R_Pkt->ResponseCode)) != 0) {
        SLog(USR_ERROR, "TH.ResponseCode [%-4.4s:%04d]",
    ...
    if (memcmp(R_Pkt->MsgType, TCP_DAOK_CD, strlen(TCP_DAOK_CD)) != 0 ||
        ...
        SLog(USR_ERROR, "TH.MsgType [%.4s:%s]", R_Pkt->MsgType, TCP_DAOK_CD);
        return (NOTOK);
    }
    ...
    if (val < 0 || val > MaxCnt) {
        SLog(USR_ERROR, "TH.DataCnt [%d:%d]", val, MaxCnt);
        return (NOTOK);
    }
...
/*----------------------------------------------------------------------*/
{
    ...
    if (DataCnt < 0 || DataCnt > MaxCnt) {
        SLog(USR_FATAL, "cannot read file[%s]", IN_NAME);
        sleep(1);
        TCP1_NET_STA = OFF;
        close(Sockfd);
        Exit_Process();
...
/*----------------------------------------------------------------------*/
{
    ...
    char            t_time[12], recvtm[16], resptm[12];
    BUFF_RW_HEAD    *f_head;
    ...
    switch (PktType) {
        ...
        case    T_DATA:
            ...
            for (i = 0, next = 0; i < DataCnt; i ++) {
                ...
                if (i == 0) {
                    sprintf(resptm, "%10.10s", f_head->RecvTime1);
    ...
    if (PktType == T_DATA)
        SLog(TCP_OK, "TCP SD [%s][%s][%s](%d)<%d>",
                recvtm, resptm, SendPkt, strlen(SendPkt), INT_SEQ);
    else
...
/*----------------------------------------------------------------------*/
{
    ...
    SIG_WRITE_MSG("[SIGNAL] pw_4000_ts caught signal\n");
