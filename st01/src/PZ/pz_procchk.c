...
typedef struct Mem_Read_Data {
    char    daemon_id[20];  /* �ι� daemon�� process ID                  */
    char    start_time[6];  /* �ι� �⵿ �ð� (hhmm)                    */
    char    end_time[6];    /* �ι� ���� �ð� (hhmm)                  */
    char    pname[1000][20];/* ���� process ID                          */
    u_char  pstat[1000];    /* process status (1:run 2:stop 9:not run)  */
    short   count;          /* ���� ��� process ����                    */
    char    chk_flag;       /* �ι� daemon �⵿ ���� (0:�⵿̱ 1:�⵿)  */
}   Data_Info;
...
/*----------------------------------------------------------------------*/
{
    ...
    switch (p_signo) {
        case    SIGUSR1:
            SIG_WRITE_MSG("[SIGNAL] pz_procchk caught SIGUSR1\n");
            signal(SIGUSR1, Sig_Handler);
            break;
        case    SIGUSR2:
            SIG_WRITE_MSG("[SIGNAL] pz_procchk caught SIGUSR2\n");
            _in_signal_handler = 1;
            Exit_Process();
            break;
        default:
            SIG_WRITE_MSG("[SIGNAL] pz_procchk caught signal\n");
...
{
    int         k, rt, i, not_cnt;
    char        buf[100], path[100], tmp_file[64];
    FILE        *fp_b;
    PROCESS_T   *plp;
                ...
                if (Data[i].pstat[k] == 2)                      /* stop */
                    ...
                    if (NR > 0) {
                        ...
                        if ((fp_b = fopen(tmp_file, "r")) == NULL) {
                            Log(USR_ERROR, "cannot open:[%s]", tmp_file);
                        }
                        else {
...
/*----------------------------------------------------------------------*/
{
    ...
    for (dk = 0; dk < Process_Count; dk ++) {
        ...
        Data[dk].count = k;     /* 0���� k-1���� Proc_Id (pname) Save   */
    }
...
/*----------------------------------------------------------------------*/
{
    ...
    */
    ...
    strncpy(Dname[0], _FEP_DAT, sizeof(Dname[0]) - 1);
    Dname[0][sizeof(Dname[0]) - 1] = '\0';
    strncpy(Dname[1], _FEP_LOG, sizeof(Dname[1]) - 1);
    Dname[1][sizeof(Dname[1]) - 1] = '\0';
