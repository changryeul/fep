...
/*----------------------------------------------------------------------*/
{
    ...
    for (dk = 0; dk < Process_Count; dk ++) {
        ...
        Log(USR_OK, "Check_Sub_Daemons Check_Proc call [%s]", proc_id);
...
/*----------------------------------------------------------------------*/
{
    ...
    switch (process_no) {
        ...
        case    OK:
            ...
            Log(SYS_ERROR, "execl(%s,%s,z) failure[%s] {%d:%s}  line[%d]",
...
/*----------------------------------------------------------------------*/
{
    ...
    switch (process_no) {
        ...
        case    OK:
            ...
            Log(SYS_ERROR, "execl(%s,%s,%s,z) failure[%s] {%d:%s}  line[%d]",
    ...
    for (i = 0; i < SHM_MAX_SUB; i++) {
        ...
        switch (rt) {
            ...
            case    OK:
                ...
                /* 2025EDIT */
                snprintf(full_path, sizeof(full_path), "%s/%s", path, INFO(i).process_id);
                strncpy(path, full_path, sizeof(path));
                path[sizeof(path)-1] = '\0';

                rt = execl(path, INFO(i).process_id, (char *)NULL);
                Log(SYS_ERROR, "execl(%s,%s) failure {%d:%s}  line[%d]",
                        path, INFO(i).process_id, SYS_NO, SYS_STR, __LINE__);
                INFO(i).process_no = 0;
                exit(FAIL);
            default:
                usleep(200000);
...
/*----------------------------------------------------------------------*/
{
    ...
    switch (p_signo) {
        case    SIGTERM:
            SIG_WRITE_MSG("[SIGNAL] pz_fepp caught SIGTERM\n");
            signal(SIGTERM, Sig_Handler);
            break;
        case    SIGUSR1:
            SIG_WRITE_MSG("[SIGNAL] pz_fepp caught SIGUSR1\n");
            signal(SIGUSR1, Sig_Handler);
            break;
        case    SIGUSR2:
            SIG_WRITE_MSG("[SIGNAL] pz_fepp caught SIGUSR2\n");
            signal(SIGUSR2, Sig_Handler);
            break;
        default:
            SIG_WRITE_MSG("[SIGNAL] pz_fepp caught signal\n");
...
/*----------------------------------------------------------------------*/
{
    ...
    switch (process_no) {
        ...
        case    OK:
            ...
            Log(SYS_ERROR, "execl(%s,%s) failure[%s] {%d:%s} line[%d]",
    ...
    while (rt) {
        ...
        switch (rt) {
            ...
            case    OK:
                ...
                Log(SYS_ERROR, "execl(%s,%s) failure[%s] {%d:%s} line[%d]",
                        pname, _Exe_Name, "super daemon", SYS_NO, SYS_STR, __LINE__);
                break;
            default:
                exit(FAIL);
        }
