#define     _GLOBAL
/*------------------------------------------------------------------------
#   Module  : 전략기동/종료·한도(RISK)초기화 — OMS 코어 공유 부문(PO)
#   File    : po_9000_mp.c
#
...
#include    "fep_fepp.h"
#include    "pa_struct.h"
#include    "strategy_letter.h"     /* 전략번호→시장 letter 매핑(A안 §5.2, 걸림돌①) */
...
int     main(int argc, char *argv[]) {
    ...
    PO_9000_MP(argc);                      /* 메인 처리 루프 (argc로 초기화 여부 판단) */
...
void    Start_Client(char *p_buf) {
    ...
    /* 전략 바이너리명 생성: p{letter}_{ApType_Cd}_mp
       letter는 전략번호→시장 매핑으로 런타임 결정(A안 §5.2, 걸림돌①):
       단일시장 전략은 그 시장(채권 LP=b), 크로스마켓은 OMS 코어(차익=o). */
    memset(st_name, 0, sizeof (st_name));
    sprintf(st_name, "p%c_%4.4s_mp", Strategy_Letter(i_hd->ApType_Cd), i_hd->ApType_Cd);
