#define     _GLOBAL
/*------------------------------------------------------------------------
#   Module  : 주문접수처리(미체결내역관리) — OMS 코어 공유 부문(PO)
#   File    : po_1290_mp.c
#
#   시장 재편 Phase 3(po_ 코어 추출): pa_1290_mp.c(미체결)를 po_로 이관.
#   STATE-driven seam: 전역 미체결 SHM(MK_PREMATCH, MK_PM_SHM_KEY)을 그대로 attach
#   → 부문(pa/pb/pc)이 달라도 동일 전역 세그먼트 공유(설계 §5.1).
#
#   [이번 슬라이스 = 채권(MK_GBN=0) 경로 무변경 재배치]
#   correctness-critical 미체결 로직은 로직 변경 없이 이관(정합성 보전). `-DA1291`
#   대신 인소스 `#define A1291`로 단일 바이너리화(빌드모델 개선). 파생 경로(A2291)는
#   폐기대상 IMECO 기반이므로 제외 — **파생 미체결은 KRX TTRODP11301로 별도 신구축**
#   (pc_1200_tr 응답을 STATE-driven으로 소비, 별도 신중 단계 + shm_snap 멱등검증).
------------------------------------------------------------------------*/

/*------------------------------------------------------------------------
    Header Files
------------------------------------------------------------------------*/
#define     A1291           /* 채권 미체결(MK_GBN=0). 파생 KRX 경로는 후속 통합 */

#include    "fep_fepp.h"
#include    "pa_struct.h"
#include    "lat_trace.h"
#include    "miche_idx.h"
#include    "seam_queue.h"      /* 부문간 SEAM 스테이징 큐(pc_/pb_ 수신 → 미체결 소비) */
#include    "krx_ttrodp11301.h" /* 현·파 회원처리호가 struct(파생 미체결 등록) */
...
/*------------------------------------------------------------------------
    Function Prototypes
------------------------------------------------------------------------*/
void    PO_1290_MP(void);
void    Analyze_Data(void);
int     Make_MiChe(char *, int);
int     Make_MiChe_Deriv(char *, int);  /* 현·파 TTRODP11301 미체결 등록(MK_GBN 런타임 분기) */
...
/*----------------------------------------------------------------------*/
{
    ...
    LAT_INIT(argv[0]);
#endif
    SEAM_Init();        /* 부문간 SEAM 스테이징 큐 확보(응답 → 미체결) */
...
/*----------------------------------------------------------------------*/
{
    ...
    while (START_S != JOB_END) {
        ...
        while (START_S != JOB_END) {
            ...
            /* 부문간 전달: 파일큐 대신 전역 SEAM 응답 큐에서 소비(리더=미체결).
               SEAM 슬롯(480B)에 FILE_BUFF_FORMAT 전체 저장 → R_Fmt로 복사해
               이후 Make_MiChe 등 파싱 로직 바이트 동일. r_seq가 SHM 상주라
               재기동해도 재소비 없음(replay멱등). */
            R_Cnt = SEAM_R(SEAM_Q_RESP, SEAM_R_MICHE, Seam_Buf, READ_MAX);

            if (R_Cnt < 0) {
                Log(SAM_FATAL, "SEAM_R fail q=%d r=%d", SEAM_Q_RESP, SEAM_R_MICHE);
                sleep(1);
                Exit_Process();
            }
            else if (R_Cnt == 0)
            ...
            Log(USR_OK, "SEAM RD [q=%d r=%d][%d]", SEAM_Q_RESP, SEAM_R_MICHE, R_Cnt);
            Analyze_Data();
        }
...
#endif
            /* MK_GBN 런타임 분기(IMECO 폐기 후 전 시장 KRX form):
               현·파 회원처리호가(TTRODP113 계열) 는 파생 등록(TTRODP11301_DATA),
               그 외(채권 TTRODP4130x / TTRMOP4130x) 는 기존 채권 등록(무변경) */
            if (memcmp(&pb[4+11+11], "TTRODP113", 9) == 0)
                rt = Make_MiChe_Deriv(pb, i);
            else
                rt = Make_MiChe(pb, i);
#ifdef LAT_TRACE
...
/*************************************************************************
    Function        : . Make_MiChe_Deriv
    Parameters IN   : . p_buf : 수신 데이터(4+11 패딩 + TTRODP11301 전문)
                      . for_d : data count
    Return Code     : . int
    Comment         : 현·파 회원처리호가(TTRODP11301, 정상접수) 미체결 등록.
        채권 Make_MiChe의 접수확인(신규/정정/취소) 로직을 TTRODP11301_DATA
        필드로 이식(IMECO 폐기 후 KRX form). 거부(TTRODP11321)·자동취소
        (TTRODP11303)는 별도 전문/헤더라 이번 범위 제외. 채권LP·스프레드 제외.
        MK_GBN은 회원사용영역[35] 시장구분에서 런타임 취득(파생=1).
*************************************************************************/
/*----------------------------------------------------------------------*/
int   Make_MiChe_Deriv(char *p_buf, int for_d)
/*----------------------------------------------------------------------*/
{
    int     i, real_ord_cnt;
    int     mk_gbn, item_seq, acc_seq, trad_flag;
    int     is_normal, is_reject, is_autocxl;
    char    tr_code[12];
    long    org_cnt, edt_cnt;   /* 채권/파생 공통(double 금액) */
    double  org_gum, edt_gum;

    /* 11301(정상)/11321(거부)/11303(자동취소)는 회원처리호가 동일 레이아웃
       (docs/reference/krx-exture3 diff로 검증) → TTRODP11301_DATA 재사용. */
    TTRODP11301_DATA    *dat = (TTRODP11301_DATA *)&p_buf[4+11];

    memset(tr_code, 0, sizeof (tr_code));
    memcpy(tr_code, &p_buf[4+11 +11], 11);      /* 응답 padding 4+11 후 전문[11]=TrCode */

    is_normal  = (memcmp(tr_code, "TTRODP11301", 11) == 0);
    is_reject  = (memcmp(tr_code, "TTRODP11321", 11) == 0);
    is_autocxl = (memcmp(tr_code, "TTRODP11303", 11) == 0);
    if (!is_normal && !is_reject && !is_autocxl) {
        Log(USR_ERROR, "Make_MiChe_Deriv: 미지원 파생 응답 TR[%.11s]", tr_code);
        return (OK);
    }

    /* 회원사용영역(60B, 내부규약): [30]매체 [35]시장 [37..]종목seq [42..]계좌seq */
    mk_gbn      =   AtoIf(dat->Member_Use_Area+35, 1);  /* 시장구분(파생=1) */
    ...
    /* ===== 거부(11321)/자동취소(11303): 한도 되돌림 + 자취는 미체결 감소 =====
       (미체결 등록은 정상접수에서만 하므로 슬롯 신규등록 없음. 채권 거부/자취 로직 이식) */
    if (is_reject || is_autocxl) {
        edt_gum = AtoDf(&dat->Order_Price[2], 9);
        if (is_reject)
            edt_cnt = AtoLf(dat->Order_Quantity, 10);
        else    /* 자동취소 */
            edt_cnt = AtoLf(&dat->Real_Modify_Or_Cancel_Order_Quantity[2], 8);

        /* 신규 거부/자취 시 잡았던 한도(당일차감 매수/매도 금액) 되돌림 */
        if (memcmp(dat->Modify_Or_Cancel_Type_Code, "1", 1) == 0) {
        ...
        if (is_autocxl) {
            ...
            if (i < MAX_MICHE) {
                Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Jan_Cnt -=
                    AtoIf(dat->Real_Modify_Or_Cancel_Order_Quantity,
                          sizeof (dat->Real_Modify_Or_Cancel_Order_Quantity));
                if (Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Jan_Cnt <= 0)
                    Shm_Mk_PreMatch[0].MeChe_Cnt[mk_gbn][acc_seq]--;
                Log(USR_OK, "MiChe(파생) 자동취소 감소 JN[%10.10s] jan[%d] MK[%d]",
                        dat->Order_Identification,
                        Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Jan_Cnt, mk_gbn);
            }
            else
                Log(USR_ERROR, "자동취소 주문번호 못찾음(파생) JN[%10.10s]", dat->Order_Identification);
        }
        return (OK);
    }

    /* 내가낸 주문만 처리(A:서버자동, C:메리츠매체, T:윈웨이매체) */
    if ((memcmp(dat->Member_Use_Area+30, "A", 1) != 0)  &&
        (memcmp(dat->Member_Use_Area+30, "C", 1) != 0)  &&
        (memcmp(dat->Member_Use_Area+30, "T", 1) != 0)) {
        Log(USR_ERROR, "타매체 수신 [%30.30s] [%.10s]",
                dat->Member_Use_Area+30, dat->Order_Identification);
        return (OK);
    }

    /* ===== 신규(1)/정정(2) : 미체결 등록 ===== */
    if ((memcmp(dat->Modify_Or_Cancel_Type_Code, "1", 1) == 0)  ||
        (memcmp(dat->Modify_Or_Cancel_Type_Code, "2", 1) == 0)) {
        while (1) {     /* 만석 재시도 (채권 로직과 동일) */
        for (i = 0; i < MAX_MICHE; i++) {
            if (Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Jan_Cnt <= 0) {
                Shm_Mk_PreMatch[0].MeChe_Cnt[mk_gbn][acc_seq]++;
                ...
                /* 1. 주문잔량 */
                if (memcmp(dat->Modify_Or_Cancel_Type_Code, "1", 1) == 0)
                    Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Jan_Cnt = AtoIf(dat->Order_Quantity, 10);
                else
                    Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Jan_Cnt = AtoIf(dat->Real_Modify_Or_Cancel_Order_Quantity, 10);
                /* 2. 시장구분 */
                Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Mk_gbn   = mk_gbn;
                /* 3. 종목/계좌 일련번호 */
                Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Item_Seq = AtoIf(dat->Member_Use_Area+37, 5);
                Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Acc_Seq  = AtoIf(dat->Member_Use_Area+42, 2);
                /* 필드 복사 */
                memcpy(Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].AccountNo,       dat->Account_Number,                  12);
                memcpy(Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Item_Cd,         dat->Issue_Code_Symbol,               12);
                memcpy(Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].OrderNo,         dat->Order_Identification,            10);
                memcpy(Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].OriginalOrderNo, dat->Original_Order_Identification,   10);
                memcpy(Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].OrderFlag,       dat->Modify_Or_Cancel_Type_Code,       1);
                memcpy(Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].TradeFlag,       dat->Ask_Bid_Type_Code,                1);
                /* 주문수량 (선두 2자리 제외 8자리 — 채권/파생 동일 관례) */
                if (memcmp(dat->Modify_Or_Cancel_Type_Code, "1", 1) == 0)
                    memcpy(Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Order_Cnt,   &dat->Order_Quantity[2],                            8);
                else
                    memcpy(Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Order_Cnt,   &dat->Real_Modify_Or_Cancel_Order_Quantity[2],      8);
                /* 주문가격 (소수점2, 파생은 선두1자리 부호 — 채권/IMECO 공통 &[2],9 관례) */
                Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Order_Price_dv = AtoDf(&dat->Order_Price[2], 9);

                if (memcmp(dat->Modify_Or_Cancel_Type_Code, "2", 1) == 0) {
                    edt_gum = AtoDf(&dat->Order_Price[2], 9);
                    edt_cnt = AtoLf(Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Order_Cnt, 8);
                    (void)edt_cnt;
                }

                memcpy(Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].OrderType,      dat->Order_Type_Code,      1);
                memcpy(Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].JumunFlag,      dat->Order_Condition_Code, 1);
                memcpy(Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].MembershipItem, dat->Member_Use_Area,      60);

                Log(USR_OK, "MiChe(파생) 등록 [%d][%12.12s] acc[%d] JCNT[%10.10s] JN[%10.10s] OJN[%10.10s] MK[%d]",
                        Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Jan_Cnt,
                        Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].AccountNo, acc_seq,
                        Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Order_Cnt,
                        dat->Order_Identification, dat->Original_Order_Identification, mk_gbn);
        ...
        Log(USR_ERROR, "미체결공간 10,000모두사용 10초후 재시도 시장[%d]", mk_gbn);
        sleep(10);
        }   /* End of while (만석 재시도) */
    }   /* End of 신규 or 정정 */

    /* ===== 정정(2)/취소(3) : 원주문 잔량 조정 + 한도 가감 ===== */
    if ((memcmp(dat->Modify_Or_Cancel_Type_Code, "2", 1) == 0)  ||
        (memcmp(dat->Modify_Or_Cancel_Type_Code, "3", 1) == 0)) {
        i = Miche_Idx_Get(&Midx[acc_seq], dat->Original_Order_Identification);
        if (i >= 0 && memcmp(Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].OrderNo,
                dat->Original_Order_Identification, 10) != 0)
            i = -1;     /* 낡은 캐시 - 폴백 */
        ...
        if (i < 0) {
            ...
            if (i < MAX_MICHE)
                Miche_Idx_Put(&Midx[acc_seq], dat->Original_Order_Identification, i);
        }

        for ( ; i < MAX_MICHE; i++) {
            if (memcmp(Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].OrderNo,
                    dat->Original_Order_Identification, 10) == 0) {
                if (memcmp(Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].AccountNo,
                        dat->Account_Number, 12) != 0) {
                    Log(USR_ERROR, "주문주체와 원주문주체 계좌 상이. OJN[%10.10s]",
                            dat->Original_Order_Identification);
                    return (NOTOK);
                }

                real_ord_cnt = AtoIf(dat->Real_Modify_Or_Cancel_Order_Quantity,
                                     sizeof (dat->Real_Modify_Or_Cancel_Order_Quantity));
                org_gum = Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Order_Price_dv;
                org_cnt = AtoLf(Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Order_Cnt, 8);
                (void)org_cnt;
                ...
                if (Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Jan_Cnt - real_ord_cnt < 0) {
                    Log(USR_ERROR, "확인 수량 부적합 [%d][%d] JN[%10.10s]",
                            Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Jan_Cnt, real_ord_cnt,
                            dat->Original_Order_Identification);
                    return (NOTOK);
                }
                ...
                Log(USR_OK, "MiChe(파생) 조정 [%d][%12.12s] acc[%d] JN[%10.10s] OJN[%10.10s] MK[%d]",
                        Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Jan_Cnt,
                        Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].AccountNo, acc_seq,
                        dat->Order_Identification, dat->Original_Order_Identification, mk_gbn);
                break;
        ...
        if (i >= MAX_MICHE) {
            Log(USR_ERROR, "원주문번호 못찾음(파생). MK[%d] JN[%10.10s] OJN[%10.10s]",
                    mk_gbn, dat->Order_Identification, dat->Original_Order_Identification);
            return (NOTOK);
        }

        /* 한도(ProFit) 가감: 정정=차액*수량, 취소=수량&금액 감소 (채권 로직 동일) */
        if (memcmp(dat->Modify_Or_Cancel_Type_Code, "2", 1) == 0) {     /* 정정 */
            if (edt_gum > org_gum) {                                    /* 가격증가 */
                if (memcmp(dat->Ask_Bid_Type_Code, "2", 1) == 0)        /* 매수 */
                    Shm_Risk[0].ProFit[mk_gbn][item_seq][acc_seq].item_su_miche_gum +=
                        (double)((edt_gum-org_gum) * real_ord_cnt * Shm_Risk[0].S_Sise[mk_gbn][item_seq].m_multiplier);
    ...
    return (OK);
}   /* End of Make_MiChe_Deriv */

/*************************************************************************
    End of Program (po_1290_mp.c)
*************************************************************************/
