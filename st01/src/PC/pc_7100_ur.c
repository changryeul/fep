#define     _GLOBAL
/*------------------------------------------------------------------------
 *  Module  : 파생(선물/옵션) 시세수신 (UDP 멀티캐스트) — PC 모듈
 *  File    : pc_7100_ur.c
 *
 *  KRX로부터 UDP 멀티캐스트로 파생 시세를 수신하여
 *  TCP로 연결된 서버에 재전송하는 프로세스.
 *  시장 재편 A안(letter=시장, c=파생). IMECO 폐기 → KRX 파생시세 UDP 직접 수신.
 *  채권 시세수신(pb_7100_ur.c) 템플릿 기반, 파생 TR분류만 차별화(HA는 승계).
 *  개선 빌드모델: -DB710x 다중바이너리 없음. 1소스=1바이너리, HB포트는 env/기본값.
 *
 *  처리 흐름:
 *    1) UDP 멀티캐스트 소켓 생성 및 Join (Socket_Connect)
 *    2) TCP 서버 연결 (Device_Open)
 *    3) UDP 수신 데이터를 TR코드별 분류
 *    4) A3(체결)/G7(체결G7)/B6(우선호가)/R1(장운영TS) → TCP 전달, 기타 → LK 응답
 *
 *  네트워크 구성:
...
/* 파생(금융상품선물 16F 기준) 시세 TR코드 목록 (접미사 F, 채권은 K)
 * A301F : 173  파생체결
 * G701F : 431  파생체결G7 (우선호가 5단계) 04F/05F 제외
 * B601F : 324  파생우선호가 (우선호가 5단계) 04F/05F 제외
 * R101F : 359  파생 장운영TS + 우선호가 (단일가 세션)
 * A601F : 파생종목마감,  Q201F : 파생 동적상하한가
 * (주식선/옵 04F/05F는 10단계 G704F/B604F/R104F)
 */

/*------------------------------------------------------------------------
 *  데이터 크기 (파생 최대 G701F 431B < 700). 개선모델: -D 없이 단일 상수.
 *------------------------------------------------------------------------*/
 ...
 *------------------------------------------------------------------------*/
/*------------------------------------------------------------------------
 *  HB 포트: 파생 기본 50020 (채권 pb 50000/50001·PA-HA 50010~14와 미충돌).
 *  개선 빌드모델: -D 없이 단일 기본값 + _HA_HB_PORT env로 재정의. 인스턴스별
 *  분리가 필요하면 proc.ini/env로 base port를 달리 준다.
 *------------------------------------------------------------------------*/
#define     HB_PORT_DEFAULT 50020   /* heartbeat UDP 포트 기본값 (파생)   */
...
void PC_7100_UR(void) {
    ...
    while (START_S != JOB_END) {
        ...
        /* VX_TEST: 가상거래소 시세 수신 검증용 로그 (env VX_TEST 설정 시만) */
        if (getenv("VX_TEST") != NULL)
            Log(USR_OK, "VX_TEST recv %d bytes TrCode=[%.5s] recv_cnt=%d", rt, rbuf, my_recv_cnt);
         ...
         *------------------------------------------------------------*/
        if ((memcmp(TrCode, "A3", 2) == 0) ||    /* 파생체결 A301F */
                (memcmp(TrCode, "G7", 2) == 0) ||    /* 파생체결G7 G701F */
                (memcmp(TrCode, "B6", 2) == 0) ||    /* 파생우선호가 B601F */
                (memcmp(TrCode, "R1", 2) == 0) ) {   /* 파생장운영TS R101F (채권 M4 대체) */
            /* 시세 데이터 → TCP 서버로 전송 */
...
int HA_Init(void) {
     ...
     *----------------------------------------------------------------*/
    /*----------------------------------------------------------------
     *  heartbeat 포트 결정: _HA_HB_PORT env가 있으면 그 값, 없으면 기본값.
     *  (개선 빌드모델: -D 프로세스별 offset 제거. 인스턴스 분리는 env로.)
     *----------------------------------------------------------------*/
    {
