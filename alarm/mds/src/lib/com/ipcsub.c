...
static	char	s_io_buf[1024*4];		/* i/o buffer		*/
 ...
 * NOTE	: 투자정보의 IPC RESOURCE를 처리하는 모든 PROGRAM은 본 PROCEDURE에
  *	  의해서만 IPC ID를 얻을 수 있다.
 * IN	: ipc_name	MESSAGE QUEUE 및 SHREAD MEMORY의 ID 명.
 *	  ipc_type	'Q' = MESSAGE QUEUE, 'M' = SHARED MEMORY
 * OUT	: If an error occurs, then -1 else 0
 * RTN	: TOKEN VALUE
 ...
 * NOTE	:
 * IN	: queue_name 	message queue 명 (투자정보에서 사용하는 TRANS 전달
 *	    		queue명으로 처리 PROGRAM ID명 이다)
 ...
 * NOTE	:
 * IN	: shm_name 	shared memory id name(투자정보에서 사용하는 SHARED
 *	    		memory tag 명이다)
 * OUT	: Attached memory pointer. If error, return NULL.
 * RTN	:
 * VER	: 1.00 2009-12 Winway	initial	version.
 * AUTH.:
 ******************************************************************************/
...
/*******************************************************************************
 * NAME	: l_putmsgx()
 * DESC	: 입력된 TRANSACTION MESSAGE를 QUEUE 명에 의한 MESSAGE QUEUE로 전달
 * NOTE	:
...
/*******************************************************************************
 * NAME	: l_getmsg()
 * DESC	: 입력된 TRANSACTION MESSAGE를 qid 의 MESSAGE QUEUE로부터 수신
 * NOTE	:
