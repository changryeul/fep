...
MDFOLD	*SP=NULL, *CP=NULL, *KP=NULL, *EP=NULL, *ZP=NULL, *BP=NULL, *RP=NULL, *JP=NULL;
MDARCH	*SA=NULL, *CA=NULL, *KA=NULL, *EA=NULL, *ZA=NULL, *BA=NULL, *RA=NULL, *JA=NULL;
...
int get_mast()
{
	...
	struct EX {
		char	excode	[ 1];		// 원천 : 'S'=SMB, 'K'=KMB, 'E'=EBS, 'C'=CMB, 'J'=JPM
		char	exnm	[ 8];
		int		ipck	;
		MDARCH	*arch	;			// MDARCH
		INDEX	*indx	;			// INDEX
	...
	for (ii=0, excnt=0; ii<MAX_XCHG; ii++)
	{
		...
		else if (arch->xchg.excode[0] == 'J')	{ JP = (MDFOLD *)((char *)arch + sizeof(MDARCH) + (sizeof(INDEX)*arch->xchg.maxcnt)); JA = arch; }
	}
	...
	if (!JP)	{ printf("shm attach error : JPM\n");	return (-1); }
...
int main(int argc, char **argv)
{
	...
	while (1)
	{
		sp = SP;  ep = EP;  cp = CP;  kp = KP;  zp = ZP;  bp = BP;  rp = RP; jp = JP;

tot = SA->rsum + EA->rsum + CA->rsum + KA->rsum + RA->rsum + JA->rsum;

if (rnd%2 == 0)
printf("          CUST (%7d) (%7d)                   BEST (%7d)                          SMB (%7d)                       EBS  (%7d)                      \n", ZA->rsum, tot, BA->rsum, SA->rsum, EA->rsum);
else
printf("          CUST (%7d) (%7d)                   KMB  (%7d)                      CMBS(%7d)                       REUT (%7d)           JPM (%7d)           \n", ZA->rsum, tot, KA->rsum, CA->rsum, RA->rsum, JA->rsum);
...
if (rnd%2 == 0) {
			...
			if (jp) {
				sprintf(&sbuf[strlen(sbuf)], " [%s %02u:%02u.%02u %9g %9g\033[0m]",(jp->quotalarm>0?"\033[104m알람\033[0m":(jp->trdf==9?"\033[90m제외":(jp->trdf==0?"\033[91m대기\033[90m":"\033[33m가능\033[0m")))
											,jp->khms/10000000, (jp->khms/100000)%100, (jp->khms/1000)%100, jp->bidlast, jp->offerlast);
				jp++;
			}
}
			printf("%s\n", sbuf);
