...
//	memcpy(ps->fixingdate, gmast->fixingdate, sizeof(ps->fixingdate));
	...
	sprintf(stemp, "N%.6s",	gmast->symb);		MEMCPY(ps->rdcode,	stemp);
	sprintf(stemp, "%.8f",	gmast->bid);		MEMCPY(ps->pbid,	stemp);
	sprintf(stemp, "%.f",	gmast->vbid);		MEMCPY(ps->vbid,	stemp);
	sprintf(stemp, "%.8f",	gmast->ask);		MEMCPY(ps->pask,	stemp);
	sprintf(stemp, "%.f",	gmast->vask);		MEMCPY(ps->vask,	stemp);
