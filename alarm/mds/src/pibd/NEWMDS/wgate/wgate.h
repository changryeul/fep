...
#define		WG_QUEKEY			WG_SHMKEY					// 주문쓰레드에 데이터 전달 큐
...
// WGREQ.req.stype
#define		DF_FXSPT			'S'			// SPOT		(FXGO=FXSPOT,	API=SP)
#define		DF_FXFWD			'F'			// FOWARD	(FXGO=FXFWD,	API=FW)
#define		DF_FXSWP			'W'			// SWAP		(FXGO=FXSWAP,	API=SW)
#define		DF_FXSIC			'I'			// 바로환전	(FXGO=N/A,		API=IC)

// WGREQ.user.clitp
#define		DF_CLITP_ABT		'1'		// 차익API
#define		DF_CLITP_API		'2'		// 일반API
#define		DF_CLITP_FXG		'3'		// FXGO of Bloomberg

// WGREQ.req.reqtp
#define		DF_REQTP_NOR		'1'		// 일반주문요청
#define		DF_REQTP_RFS		'2'		// RFS 요청
#define		DF_REQTP_RFQ		'3'		// RFQ 요청

#define		STR_CLITP(tp)		(tp=='1' ? "ABT" : tp=='2' ? "API" : tp=='3' ? "FXGO" : "N/A")
#define		STR_REQTP(tp)		(tp=='1' ? "NOR" : tp=='2' ? "RFS" : tp=='3' ? "RFQ" : "N/A")
...
#define		IsRFS(q)			(q->req.reqtp[0] == DF_REQTP_RFS || q->req.reqtp[0] == DF_REQTP_RFQ)
...
// 채널구분자 : 차익API='D', FXGO='5', 일반API='6'
#define		GET_CHNL(c)			(c == DF_CLITP_ABT ? "D" : c == DF_CLITP_API ? "6" : c == DF_CLITP_FXG ? "5" : "9")
...
// BUSINESS DEFINE
//################################################################################################
#define		DF_W3370_CNT		5			// W3370A01 고객주문처리 쓰레드 Count
...
// --------------------------------------------------------------------------------------------------------
// ★★★ 필드값은 FIX엔진과 통신하는 구조체값을 그대로 사용하여 space로 패딩되어 있음. (비교구문 시 주의)
// --------------------------------------------------------------------------------------------------------
typedef struct {
	...
	// ◆◆◆ FXGO : Tag131(QuoteReqID) or Tag262(MDReqID)
	char	reqID			[ 64];		// 동일고객, 동일통화에 대한 요청도 가능하도록 reqID를 KEY에 추가
...
// RFS 인경우는 req 요청시 조회한 밴드정보와 마크업 값으로 계산하고, Stream 인 경우는 실시간으로 m365 조회하여 계산함.
	...
	struct	_user {		// 요청이 올 때마다 조회해야 하므로 DB에서 정보관리
		...
		char	clitp		[ 1];		// '1':차익API, '2':일반API, '3':FXGO
...
//		char	orgquot		[ 1];		// 마크업/수수료 없는 원가제공 여부
	...
	// RFS,RFQ INFO
	struct	_req {
		char	reqtp		[  1];		// '1'=일반주문, '2'=RFS, '3'=RFQ
		char	stype		[  1];		// Tag167=SecurityType (S:SPOT,F:FWD,W:SWAP,I:바로환전)
...
//################################################################################################
enum {
	MSG_LOGOUT		= 'L',		// Logout (Session Closed) FIX세션기준으로 모두 해제
...
//######################################################################################
typedef struct {
	char	msgtp		[  1];		// 'L' Logout (Session Closed)
									// 'R' Quote Request(subscribe/unsubscribe)
									// 'S' Quote Response(reject)
									// 'Q' Quote Feed
									// 'O' New/Amend/Cncl Order
									// 'E' Order Ack/Execution
	WGKEY	wgkey		;
//	char	timestamp	[ 20];		// Aggregator에서 FIX로 전송한 시간 (YYYYMMDD hhmmss SSSSSS)
	char	data		[  1];		// data pointer for : WG_LOGOUT / WG_QUOT_REQ / WG_QUOT_RES / WG_QUOT / WG_ORD / WG_ORD_EXC
} WGCOM;

// 일반API 종료 시 고객이 전송 또는 고객 FIX 세션 종료 시 FIX엔진에서 전송
// WGCOM.wgkey 값 기준으로 모두 해제, sendcompid + sendsubid 기준으로 모두 해지 (wgkey.reqID는 값없음)
typedef struct {
	char	text		[128];		// 58(Text) : 종료사유
} WG_LOGOUT;
#define		SZ_WG_LOGOUT		sizeof(WG_LOGOUT)
...
typedef struct {
	...
	char	stype		[  1];		// Tag167=SecurityType or Tag9063=Tenor Code (S:SPOT,F:FWD,W:SWAP,I:바로환전)
	char	symb		[  6];		// PK // WGKEY + SYMBOL
