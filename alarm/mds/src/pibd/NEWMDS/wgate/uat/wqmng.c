		...
		if (item[0] == 'q')		break;
			...
			if (strcmp(item, "mrgn") == 0)	preq->margin = atof(ival);
				...
				if (strlen(ival) == sizeof(preq->req.etime))
					...
					memcpy(preq->req.etime, "000000000", sizeof(preq->req.etime));
			}
		}
	}
...
void printreq(int pos, WGREQ *p)
{
	...
	printf("seq=%d] use=%d (%.15s/%.15s) (%s/%.30s) (%.1s/%.6s/%.6s) bid/ask(%.*f:%.*f) mkup(%.6s) qid(%s)\n", pos, 
	...
	printf("  RQ) id=%.30s  mrgn=%.6f  qty=%.f  side=%.1s  tm=(%.2s:%.2s:%.2s ~ %.2s:%.2s:%.2s)\n", 
