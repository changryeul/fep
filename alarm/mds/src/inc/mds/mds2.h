...
// 24.10.07) JMP의 NDF용 별도 마스트메모리
#define		DF_SHMKEY_NDF		0x90990000		// NDF마스트메모리
...
typedef struct {
	...
	char		bestspread		[ 6];		// bestspread		// 24.10.10) 호가이상발생여부[0] : '0'=정상, '1'=발생 (BEST인 경우만 사용, '1' 이면 주문 중지상태)
