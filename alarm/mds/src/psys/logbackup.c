...
int backup_log_file(int date)
{
	...
	rc = mkdir(dirpath, 0775);
	if (rc < 0)
	{
		printf("mkdir error. [%s] E:%d \n", dirpath, errno);
	}
	else
	{
		printf("mkdir ok. [%s] \n", dirpath);
	}
	sprintf(command, "/usr/bin/mv %s/*%08d.log %s", LOG_DIR, date, dirpath);
	system(command);
