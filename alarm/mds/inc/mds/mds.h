...
typedef	struct {
	int	n_strk;				// # of strike price
	struct	cnp {
		double	strk;			// strike price
		void	*call;			// folder of call option
		void	*put;			// fodler of put option
	} cnp[3000];
} OPTFOLD;
...
int		mds_optfolder(MARKET *market, WHERE *where, OPTFOLD *optfold);
...
//void	mds_leadmonth_update(MARKET *market);

//void sendfolder(MARKET *market, MDFOLD *folder, char *check);

#define     PI          3.141592653589793238462643 
#define     DaysInYear  365     // 1년은 365일
//
// 옵션 구분 (현물, 선물, 지수 옵션)
typedef enum {TypeOptFuture = 1, TypeOptSpot, TypeOptIndex} TypeOption;

// 선물/옵션 구분 (TypeSymbolClass)
typedef enum {TypeFuture, TypeCall, TypePut} TypeSymbolClass ;
...
// 내재변동성 
double getImpliedVolBS(
    TypeOption  opt_tp,
    TypeSymbolClass callPut_tp,
    double under_p,         // spot price
    double strike_p,        // Strike (exercise) price,
    double domest_rt,       // 국내이자율
    double divi_rt,         // 국외이자율
    double dividend_index,  // 배당액지수
    double time,            // 잔존일수/365
    double curr_p           // 옵션현재가
    );
...
typedef struct {
	double	mprice;			// market price
	double	uprice;			// underlying price
	double	interest;		// interest rate (CD 금리)
	double	edays;			// days until expiration (잔존일수)
	/////////////////////////////////////////////////////////////////////////////
	double	ydays;			// days of year (일년 일수 365:평년)
	double	dividend;		// dividend rate (Default : 0)
} F_PRICING_IN;

typedef struct {
	double	timeyear;		// time year
	double	tprice;			// theoretical price
	double	tbasis;			// theoretical price basis
	double	disrate;		// disparate ratio of theotetical price
	double	mbasis;			// - Future only - market price basis
} F_PRICING_OUT;

	
typedef struct {
	char	callput;		// - Option only -  'C' : call   'P' : put
	double	mprice;			// market price
	double	uprice;			// underlying price
	double	interest;		// interest rate (CD 금리)
	double	edays;			// days until expiration (잔존일수)
	double	sprice;			// - Option only - strike price 
	double	hvol;			// - Option only - historical volatility of underlyhing product price
	//////////////////////////////////////////////////////////////////////////////
	double	ydays;			// days of year (일년 일수 365:평년)
	double	dividend;		// dividend rate (Default : 0)
} O_PRICING_IN;
...
typedef struct {
	...
	double	tbasis;			// theoretical price basis
	double	disrate;		// disparate ratio of theotetical price
	double	ivol;			// - Option only - implied volatility
	double	delta;			// - Option only - GREEK - DELTA
	double	gamma;			//                       - GAMMA
	double	vega;			//		         - VEGA
	double	theta;			//	 	         - THETA
	double	rho;			//	 	         - RHO
...
typedef	struct {                        // --- Input ---
	...
	double	uprice;			// underlying price
	double	edays;			// days until expiration (잔존일수)
	double	ydays;			// days of year (일년 일수 365:평년, 366:윤년)
	double	interest;		// interest rate (CD 금리)
	double	dividend;		// dividend rate (Default : 0)
	double	sprice;			// - Option only - strike price 
	double	hvol;			// - Option only - historical volatility of underlyhing product price
...
// pricing functions
void   future_pricing(F_PRICING_IN *in, F_PRICING_OUT *out);
void   future_pricing_0(PRICING *pricing);
void   option_pricing(O_PRICING_IN *in, O_PRICING_OUT *out);
void   option_pricing_0(int callput, PRICING *pricing);
