#ifndef _KRX_TTSSDP90006_H
#define _KRX_TTSSDP90006_H

/* TTSSDP90006 협의(협의대량, EFP, FLEX)비상주문호가재송신 - KRX EXTURE 3.0 v3.24 전문 (자동생성, DATA부) */
/* (변형: 레이아웃 공유 그룹 TTSSDP20006) */
/* DATA 길이 합계 = 309 (interface-list.csv '길이(헤더제외)'와 대조) */
typedef struct {
    ...
    char Order_Rejected_Reason_Code[4];         /*   39 호가거부사유코드 (String 4) */
    char Spot_Prc[11];                          /*   40 현물가격 (Float 11) */
    char Program_Trading_Type_Code[2];          /*   41 PT구분코드 (String 2) */
} TTSSDP90006_DATA;

#endif  /* _KRX_TTSSDP90006_H */
