#ifndef _KRX_TRDESP00901_H
#define _KRX_TRDESP00901_H

/* TRDESP00901 주식파생상품 기초자산 코드정보 - KRX EXTURE 3.0 v3.24 전문 (자동생성, DATA부) */
/* DATA 길이 합계 = 58 (interface-list.csv '길이(헤더제외)'와 대조) */
typedef struct {
    char Message_Sequence[11];                  /*    1 메세지일련번호 (Long 11) */
    char Transaction_Code[11];                  /*    2 트랜잭션코드 (String 11) */
    char Market_Identification[3];              /*    3 시장ID (String 3) */
    char Transmit_Date[8];                      /*    4 전송일자 (String 8) */
    char Business_Date[8];                      /*    5 영업일자 (String 8) */
    char Issue_Code[12];                        /*    6 종목코드 (String 12) */
    char Underlying_Asset_Identification[3];    /*    7 기초자산ID (String 3) */
    char Uly_Cd[2];                             /*    8 기조자산코드 (String 2) */
} TRDESP00901_DATA;

#endif  /* _KRX_TRDESP00901_H */
