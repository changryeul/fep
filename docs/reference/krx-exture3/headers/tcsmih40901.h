#ifndef _KRX_TCSMIH40901_H
#define _KRX_TCSMIH40901_H

/* TCSMIH40901 장중추가증거금_옵션결제차금(파생시장) - KRX EXTURE 3.0 v3.24 전문 (자동생성, DATA부) */
...
typedef struct {
    ...
    char Trust_Ask_Trading_Value[22];           /*   15 위탁매도거래대금 (Float 22) */
    char Trust_Bid_Trading_Value[22];           /*   16 위탁매수거래대금 (Float 22) */
    char Principal_Ask_Trading_Value[22];       /*   17 자기매도거래대금 (Float 22) */
    char Principal_Bid_Trading_Value[22];       /*   18 자기매수거래대금 (Float 22) */
    char Filler_Value[1029];                    /*   19 필러값 (String 1029) */
} TCSMIH40901_DATA;

#endif  /* _KRX_TCSMIH40901_H */
