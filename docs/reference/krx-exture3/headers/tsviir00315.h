#ifndef _KRX_TSVIIR00315_H
#define _KRX_TSVIIR00315_H

/* TSVIIR00315 파생심리 대체입출금내역 - KRX EXTURE 3.0 v3.24 전문 (자동생성, DATA부) */
/* DATA 길이 합계 = 455 (interface-list.csv '길이(헤더제외)'와 대조) */
typedef struct {
    ...
    char Source_Type_Code[1];                   /*   20 출처구분코드 (String 1) */
    char Counterpart_Member_Number[5];          /*   21 상대회원번호 (String 5) */
    char Counterpart_Branch_Number[5];          /*   22 상대지점번호 (String 5) */
    char Counterpart_Account_Number[12];        /*   23 상대계좌번호 (String 12) */
    char Counterpart_Account_Name[80];          /*   24 상대계좌명 (String 80) */
    char Transfer_In_Out_Amount[22];            /*   25 입출금금액 (Float 22) */
    char Remark[100];                           /*   26 비고 (String 100) */
} TSVIIR00315_DATA;

#endif  /* _KRX_TSVIIR00315_H */
