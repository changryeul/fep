#ifndef _KRX_TCSMIH10101_H
#define _KRX_TCSMIH10101_H

/* TCSMIH10101 선물정산가격(파생시장) - KRX EXTURE 3.0 v3.24 전문 (자동생성, DATA부) */
...
typedef struct {
    ...
    char Setlmult[22];                          /*   11 거래승수 (Float 22) */
    char Tdd_Setl_Prc[18];                      /*   12 금일정산가격 (Float 18) */
    char Prevdd_Setl_Prc[18];                   /*   13 전일정산가격 (Float 18) */
    char Setl_Prc_Tp_Cd[2];                     /*   14 정산가격구분코드 ★변경 (String 2) */
    char Filr_Val[1084];                        /*   15 필러값 (String 1084) */
} TCSMIH10101_DATA;

#endif  /* _KRX_TCSMIH10101_H */
