#ifndef _KRX_TSVIIR00316_H
#define _KRX_TSVIIR00316_H

/* TSVIIR00316 파생심리 현금입출금내역 - KRX EXTURE 3.0 v3.24 전문 (자동생성, DATA부) */
/* DATA 길이 합계 = 382 (interface-list.csv '길이(헤더제외)'와 대조) */
typedef struct {
    ...
    char Cash_In_Out_Amount[22];                /*   20 입출금금액 (Float 22) */
    char Transactor_Number[10];                 /*   21 처리자번호 (String 10) */
    char Transactor_Name[20];                   /*   22 처리자명 (String 20) */
    char Remark[100];                           /*   23 비고 (String 100) */
} TSVIIR00316_DATA;

#endif  /* _KRX_TSVIIR00316_H */
