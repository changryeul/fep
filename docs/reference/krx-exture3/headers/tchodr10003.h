#ifndef _KRX_TCHODR10003_H
#define _KRX_TCHODR10003_H

/* TCHODR10003 호가입력 - KRX EXTURE 3.0 v3.24 전문 (자동생성, DATA부) */
/* (변형: 레이아웃 공유 그룹 TCHODR10001) */
/* DATA 길이 합계 = 294 (interface-list.csv '길이(헤더제외)'와 대조) */
typedef struct {
    ...
    char Algo_Stgy_Tp_Cd[1];                    /*   38 알고리즘전략구분코드 ★신규 (String 1) */
    char Trdr_Id[6];                            /*   39 거래자ID ★신규 (String 6) */
    char Ord_Grp_No[2];                         /*   40 호가그룹번호 ★신규 (String 2) */
    char Smp_Cd[1];                             /*   41 자전거래방지코드 ★신규 (String 1) */
    char Ord_Cond_Prc[11];                      /*   42 호가조건가격 (Float 11) */
    char Trd_Mkt_Choic_Tp_Cd[1];                /*   43 거래시장선택구분코드 (String 1) */
    char Srtsell_Id[10];                        /*   44 공매도ID (String 10) */
} TCHODR10003_DATA;

#endif  /* _KRX_TCHODR10003_H */
