...
/*--------------------------------------------------------*/
{
	...
	sleep(1);			// Client로부터 Parameter를 받을 시간을 준다.
	rt = Init_Parameters();
...
/*--------------------------------------------------------*/
{
	...
	R_Cnt = DSHM_R (TS_R1_1, (void *)R_Fmt, 1);
	Log( USR_OK, "    R_Cnt=[%d]", R_Cnt );

	R_Cnt = DSHM_R (TS_W1_1, (void *)R_Fmt, 1);
	Log( USR_OK, "    R_Cnt=[%d]", R_Cnt );
	Log( USR_OK, "    IDN(D_K,P_K,0)=[%s]", IDN(D_K,P_K,0));
	Log( USR_OK, "    file[%s,%d:%s] W[%d]R[%d]", IFN(D_K,P_K,0), SYS_NO, SYS_STR, WRITE_CNT, RD_CNT);
	if (R_Cnt < 0)
	{
...
// 공통
	...
	else	// 전략 없음 --- CDC
	{
		...
		/* 금융상품선물A0_Load - pa_7201_dd1 */
		sprintf(fifo_name, "%s/PA/pa_7201_dd1", _FEP_FIFO);
		Log( USR_OK, "    fifo_name=[%s]", fifo_name);
		...
		/* 금융상품선물시세ShmLoad - pa_7202_dd1 */
		sprintf(fifo_name, "%s/PA/pa_7202_dd1", _FEP_FIFO);
		Log( USR_OK, "    fifo_name=[%s]", fifo_name);
		Poll[InPollCnt +3].fd = open(fifo_name, O_RDWR|O_NDELAY);
		Poll[InPollCnt +3].events = POLLIN;
		if (Poll[InPollCnt +3].fd < 0)
		{
			Log(FIF_FATAL, "    cannot open i{%d] FIFO[%s][%d][%d:%s]",
		...
		/* 금융상품선물_M4Load - pa_7203_dd1 */
		sprintf(fifo_name, "%s/PA/pa_7203_dd1", _FEP_FIFO);
		Log( USR_OK, "    fifo_name=[%s]", fifo_name);
		Poll[InPollCnt +4].fd = open(fifo_name, O_RDWR|O_NDELAY);
		Poll[InPollCnt +4].events = POLLIN;
		if (Poll[InPollCnt +4].fd < 0)
		{
			Log(FIF_FATAL, "    cannot open i{%d] FIFO[%s][%d][%d:%s]",
				i, fifo_name, Poll[3].fd, SYS_NO, SYS_STR);
			return_flag = -1;
	...
	else
	{
		...
		Log( USR_OK, "    Client에 정상기동 (50021) 송신");
	}
...
*/
		...
		/* 46,47,48,49	: Process Nick Name
						  pa_50101mp => 0101, pa_50403mp => 0203				*/
		/* ******************************************************************** */
