...
#elif defined A3291
#define		MK_GBN		2	/* 2026/01/05 compile CHECK!!! */ 
#define		DATA_SIZE	450
#endif
...
{
	int		i, rt;
#if defined A2291 || A3291
	YSK_SETTLE_RESP_FMT		*sr_fmt;
#endif
	...
	for (i = 0; i < R_Cnt; i ++)
	{
		...
		*/
		...
		if (memcmp (R_Fmt[i].Data, "REJ", 3) == 0)   // CLIENT에게 주문거부 송신(REJC/REJE)
			Log (USR_OK, "SKIP DATA Write OK ResponseCode[%4.4s]", R_Fmt[i].Data);
#elif defined A2291 || A3291
		sr_fmt = (YSK_SETTLE_RESP_FMT *)R_Fmt[i].Data;
		if (memcmp (sr_fmt->Header.sRpCode, "REJ", sizeof(sr_fmt->Header.sRpCode) - 1) == 0)   // CLIENT에게 주문거부 송신(REJC/REJE)
			Log (USR_OK, "SKIP DATA Write OK ResponseCode[%4.4s]", sr_fmt->Header.sRpCode);
		else
...
/*----------------------------------------------------------------------*/
{
	...
	int		mk_gbn, item_seq, acc_seq, trad_flag, mem_area_loc;
	char	tr_code[12];
#if defined A2291 || A3291
	YSK_SETTLE_RESP_FMT		*sr_fmt;
#endif
...
#elif defined A2291 || A3291
	m_size = 20;
	sr_fmt = (YSK_SETTLE_RESP_FMT *)p_buf;
	memcpy (tr_code, sr_fmt->SettleRespData.TrCode, 11);
#endif

Log (USR_OK, "tr_code[%s] m_size[%d]", tr_code, m_size);
...
#elif defined A2291 || A3291
	if (memcmp (tr_code, "TTRODP11301", 11) == 0)	// TTRODP11301(금융파생일반확인, 318) 
#endif
	{
#if defined A1291
		...
		mem_area_loc = 0;
Log (USR_OK, "01 dat[%s]", dat);
#elif defined A2291 || A3291
		KRX_SETTLE_RESP_DATA	*dat	=
			(KRX_SETTLE_RESP_DATA*)&sr_fmt->SettleRespData;

		mem_area_loc = 10;
Log (USR_OK, "02 dat[%s]", dat);
#endif

Log (USR_OK, "MembershipItem[%20.20s] mem_area_loc[%d]", dat->MembershipItem+mem_area_loc+30, mem_area_loc+30);
		...
		*/
		...
		{
//			Log (USR_OK, "01.타매체 수신 [%30.30s] [%s]", dat->MembershipItem+mem_area_loc+30, dat->DataSeq);
			...
			(memcmp (dat->New_Modify_Cancel_gbn, "2", 1) == 0)	)
		{
			for (i = 0; i < MAX_MICHE; i++)
			{
				...
				if (Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Jan_Cnt <= 0)
				{
					...
					/* int3. 종목일련번호 */
					Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Item_Seq	= AtoIf (dat->MembershipItem+mem_area_loc+37, 5);
					/* int4. 계좌일련번호 */
					Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Acc_Seq	= AtoIf (dat->MembershipItem+mem_area_loc+42, 2);
					/* int5. 전략번호 */
					//Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Str_No	= AtoIf (dat->MembershipItem+mem_area_loc+33, 2);
...
#elif defined A2291 || A3291
					if (memcmp (dat->MembershipItem+mem_area_loc+45, "1", 1) != 0)	// !스프레드
					{
...
/* 금융파생은 실제 음수를 사용하지 않는다. 로직에서도 음수는 배제했음 */
							Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Order_Price_dv = AtoDf (&dat->Price[1], sizeof(dat->Price) - 1);
				...
				if (i >= MAX_MICHE)
				{
					Log (USR_ERROR, "미체결공간 10,000모두사용 10초후 재시도 시작[%d]", MK_GBN);
					sleep (10);
					i = -1;
				}
			...
			(memcmp (dat->New_Modify_Cancel_gbn, "3", 1) == 0)	)
		{
			...
			/* ************************************************************************ */
#if defined A2291 || A3291
			if (memcmp (dat->MembershipItem+mem_area_loc+45, "1", 1) != 0)	// !스프레드, 정정시 미체결 수량X, 차액만 수정
																//            취소시 미체결 수량 && 금액 수정
			{
#endif
			...
			else												// 스프레드, 미체결 수량만(근/원월물 각각)
			{
				if (memcmp (dat->New_Modify_Cancel_gbn, "3", 1) == 0)	// 취소, 일부취소없다(취소된만큼 뺀다)
				{
					...
					{
						// 근월물
						Shm_Risk[0].ProFit[mk_gbn][AtoIf (dat->MembershipItem+mem_area_loc+4,  5)][acc_seq].item_do_michecnt -= real_ord_cnt;
						// 원월물
						Shm_Risk[0].ProFit[mk_gbn][AtoIf (dat->MembershipItem+mem_area_loc+16, 5)][acc_seq].item_su_michecnt -= real_ord_cnt;
					}
					else													// 매도(근월물매수,원월물매도)
					{
						// 근월물
						Shm_Risk[0].ProFit[mk_gbn][AtoIf (dat->MembershipItem+mem_area_loc+4,  5)][acc_seq].item_su_michecnt -= real_ord_cnt;
						// 원월물
						Shm_Risk[0].ProFit[mk_gbn][AtoIf (dat->MembershipItem+mem_area_loc+16, 5)][acc_seq].item_do_michecnt -= real_ord_cnt;
					}
				}
			}
#endif
...
#elif defined A2291 || A3291
	if ((memcmp (tr_code, "TTRODP11321", 11) == 0)	||	// 거부
...
//			Log (USR_OK, "01.타매체 수신 [%30.30s] [%s]", dat->MembershipItem+mem_area_loc+30, dat->DataSeq);
...
#elif defined A2291 || A3291
		if (memcmp (dat->MembershipItem+mem_area_loc+45, "1", 1) != 0)	// !스프레드, 미체결 수량 && 금액 감소
		{
			/* 주문가격 */
			edt_gum = 0;
			if ((memcmp (dat->Order_Type, "1", 1) == 0)    ||	// 시장가
				(memcmp (dat->Order_Type, "X", 1) == 0))		// 최유리 지정가
			{
				...
				edt_gum = AtoDf (&dat->Price[1], sizeof(dat->Price) - 1);

			if (memcmp (tr_code, "TTRODP11321", 11) == 0)		// 거부
				edt_cnt = AtoLf (dat->OrderQuantity, 10);
			else												// 자동취소
				edt_cnt = AtoLf (&dat->Real_Modify_Cancel_Cnt[1],	sizeof(dat->Real_Modify_Cancel_Cnt) - 1);
#endif
		...
		else												// 스프레드
		{
			if (memcmp (dat->New_Modify_Cancel_gbn, "1", 1) == 0)	// 신규
			{
				...
				{
					/* 근월물(반대방향) */
					Shm_Risk[0].ProFit[mk_gbn][AtoIf (dat->MembershipItem+mem_area_loc+4,  5)][acc_seq].item_do_michecnt -= AtoIf (&dat->OrderQuantity[1], sizeof(dat->OrderQuantity) - 1);
					/* 원월물(반대방향) */
					Shm_Risk[0].ProFit[mk_gbn][AtoIf (dat->MembershipItem+mem_area_loc+16, 5)][acc_seq].item_su_michecnt -= AtoIf (&dat->OrderQuantity[1], sizeof(dat->OrderQuantity) - 1);
				}
				else
				{	
					/* 근월물(반대방향) */
					Shm_Risk[0].ProFit[mk_gbn][AtoIf (dat->MembershipItem+mem_area_loc+4,  5)][acc_seq].item_su_michecnt -= AtoIf (&dat->OrderQuantity[1], sizeof(dat->OrderQuantity) - 1);
					/* 원월물(반대방향) */
					Shm_Risk[0].ProFit[mk_gbn][AtoIf (dat->MembershipItem+mem_area_loc+16, 5)][acc_seq].item_do_michecnt -= AtoIf (&dat->OrderQuantity[1], sizeof(dat->OrderQuantity) - 1);
				}
			}
			/* 정정/취소의 거부는 무시 */ 
		}
	...
	if (memcmp (tr_code, "TTRMOP41301", 11) == 0)	// TTRMOP41301(채권LP일반확인, 295)
	{
		...
		mk_gbn		=	AtoIf (dat->MembershipItem+mem_area_loc+35, 1);	// 시장구분
		item_seq	=	AtoIf (dat->MembershipItem+mem_area_loc+37, 5);	// 종목 Seq
		acc_seq		=	AtoIf (dat->MembershipItem+mem_area_loc+42, 2);	// 계좌 seq
		...
		if ((memcmp (dat->MembershipItem+mem_area_loc+30, "A", 1) != 0)	&&
			...
			(memcmp (dat->MembershipItem+mem_area_loc+30, "T", 1) != 0)	)
		{
//			Log (USR_OK, "02.타매체 수신 [%30.30s] [%s]", dat->MembershipItem+mem_area_loc+30, dat->DataSeq);
			Log (USR_OK, "02.타매체 수신 [%30.30s] [%s]", dat->MembershipItem+mem_area_loc+30, dat);
			return (OK);
		}
			...
			(memcmp (dat->New_Modify_Cancel_gbn, "2", 1) == 0)	)
		{
			for (i = 0; i < MAX_MICHE; i++)
			{
				...
				if (Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Jan_Cnt <= 0)
				{
					...
					/* int1. 주문잔량 */
					if ((memcmp (dat->New_Modify_Cancel_gbn, "1", 1) == 0)  ||			// 신규
					    (memcmp (dat->New_Modify_Cancel_gbn, "2", 1) == 0)	)			// 정정
					{
						Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Jan_Cnt =
...
*/
					...
					/* int2. 시장구분 */
					//Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Mk_gbn	= AtoIf (dat->MembershipItem+mem_area_loc+35, 1);
...
/*
						edt_gum = AtoLf (Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Order_Cnt, 8)
								* AtoDf (Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Order_Price, 9);
*/
					}
					/* 9. 호가유형코드 */
					memcpy (Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].OrderType,		dat->Order_Type,1);
			...
			(memcmp (dat->New_Modify_Cancel_gbn, "3", 1) == 0)	)
		{
			for (i = 0; i < MAX_MICHE; i++)
			{
				...
				if (memcmp (Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].OrderNo, dat->OriginalOrderNo, 10) == 0)
				{
                    ...
                    /* 원주문번호 삭제하기(Jan_Cnt = 0) */
                    Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Jan_Cnt = 0;
                    Shm_Mk_PreMatch[0].MeChe_Cnt[mk_gbn][acc_seq]--;
Log (USR_OK, "원주문 삭제 New_Modify_Cancel_gbn[%1.1s] OrderNo[%10.10s] OriginalOrderNo[%10.10s]",
    dat->New_Modify_Cancel_gbn, Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].OrderNo, dat->OriginalOrderNo);
                }
            }
        }
...
// 2025 LP는 정정취소확인수량 항목이 없다 어떻게 처리하나?
					// 실제처리된 수량
					real_ord_cnt = AtoIf (dat->Real_Modify_Cancel_Cnt, sizeof (dat->Real_Modify_Cancel_Cnt));
...
//						org_cnt01 = AtoLf (&dat->Real_Modify_Cancel_Cnt[2], 8);
						org_gum02 = AtoDf (Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Order_Price_Lp02,	9);
						org_cnt02 = AtoLf (Shm_Mk_PreMatch[0].F_MiChe[mk_gbn][acc_seq][i].Order_Cnt_Lp02,	9);
//						org_cnt02 = AtoLf (&dat->Real_Modify_Cancel_Cnt[2], 8);
					}
...
Log (USR_OK, "03 dat[%s]", dat);
		...
		item_seq	=	AtoIf (dat->MembershipItem+mem_area_loc+37, 5);	// 종목 Seq
		acc_seq		=	AtoIf (dat->MembershipItem+mem_area_loc+42, 2);	// 계좌 seq
Log (USR_OK, "03 dat[%60.60s]", dat->MembershipItem);
