...
typedef struct {
    KRX_HEADER  Header;
    char        Data[4096];			/* 로그온:I/F프로세스정보(10)+I/F프로세스고유번호(30)+암호화 적용여부(1,Y/N) */
}   KRX_SESSION_FMT;
...
//    char        Data[116];			/* 체결수신은 ME그룹 필요 더 길다 */
    char        Data[4096];			/* 체결수신은 ME그룹 필요 더 길다 */
}   KRX_R_SESSION_FMT;
...
/*--------------------------------------------------------------------------
 * DATA HEAD STRUCTURE (100 Bytes ) 
 *--------------------------------------------------------------------------*/
typedef struct
{
    char  sLength               [ 4]; /* length field를 제외한 단일DataSize        */
    char  sSeqNo                [ 8]; /* data전문별 일련번호                       */
    char  sDate                 [ 8]; /* 주문일자                                  */
    char  sTime                 [ 6]; /* 주문시간                                  */
    char  sSndTp                [ 2]; /* 송신처+LINT구분                           */
                                		/* 송신처구분 : 72(파생)                    */
    char  sMediaType            [ 3]; /* 매체(채널)구분                            */
                                		/* 102 ( 정규장_영업사원 ) , 612 (야간장_영업사원) */
    ...
    char  sOnOffGb              [ 2]; /* Online/Offline 구분                       */
    char  sOrderType            [ 2]; /* 주문 종류                                 */
...
#define     YSK_DATA_HEAD_LEN       (sizeof (YSK_DATA_HEAD))        /* 100 */

typedef struct {
	YSK_DATA_HEAD				Header;
	KRX_JUMUN_DATA				JumunData;
}	YSK_JUMUN_FMT;			// 주문
#define     YSK_JUMUN_FMT_LEN		(sizeof (YSK_JUMUN_FMT))		/* */

typedef struct {
	YSK_DATA_HEAD				Header;
	KRX_SETTLE_RESP_DATA		SettleRespData;
}	YSK_SETTLE_RESP_FMT;
#define     YSK_SETTLE_RESP_FMT_LEN		(sizeof (YSK_SETTLE_RESP_FMT))		/* */

typedef struct {
	YSK_DATA_HEAD				Header;
	KRX_SETTLE_DATA				SettleData;
}	YSK_SETTLE_FMT;
#define     YSK_SETTLE_FMT_LEN		(sizeof (YSK_SETTLE_FMT))		/* */
...
typedef struct {
    ...
    char     proc_stus_dstcd         [  1];  // 3  처리상태구분코드
    char     start_yms               [ 14];  // 4  시작일시              
    char     end_yms                 [ 14];  // 5  종료일시              
    char     lp_start_yms            [ 14];  // 6  유동성공급시작일시    
    char     lp_end_yms              [ 14];  // 7  유동성공급종료일시    
    char     item_cd                 [ 12];  // 8  종목코드              
    char     mm_item_dstcd           [  2];  // 9  유동성공급종목구분코드
    char     sped1_prc               [ 11];  // 10 스프래드1가격         
    char     sped2_prc               [ 11];  // 11 스프래드2가격         
    char     sped3_prc               [ 11];  // 12 스프래드3가격         
    char     ord1_qanty              [ 10];  // 13 주문1수량             
    char     ord2_qanty              [ 10];  // 14 주문2수량             
    char     ord3_qanty              [ 10];  // 15 주문3수량             
    char     ord_qanty_unit          [  5];  // 16 주문수량단위          
    char     dspratio_taget_dstcd    [  2];  // 17 괴리율대상구분코드    
    char     dspratio_dstcd          [  1];  // 18 KRX괴리율구분코드         
    char     dspratio                [ 11];  // 19 괴리율                
    char     tick_unit               [ 11];  // 20 틱단위                
    char     trdr_uno                [  5];  // 21 거래원번호            
    char     trdr_no                 [  4];  // 22 CMBS트레이더번호      
    char     prv_yild                [ 11];  // 23 민간평가수익률        
    char     prv_prc                 [ 11];  // 24 민간평가가격          
    char     clsng_prc               [ 11];  // 25 종가                  
    char     clsng_yild              [ 11];  // 26 종가수익률            
