...
#if defined	A7401
		rt = conv_JPM(r_buf, rt, &r_sise);
#elif defined A7402
		rt = conv_NH(r_buf, rt, &r_sise);
#elif defined A7403
		rt = conv_SH(r_buf, rt, &r_sise);
#endif
		if (rt < 0)
		{
...
#if 1   /* HONG */
    Sise_SHM_Detach();
	Sise_SHM_Attach(FX_SHM_KEY);
	Sise_SHM_Attach(RISK_SHM_KEY);
	Sise_SHM_Attach(ITEM_SHM_KEY);
#endif
...
//	sprintf (fifo_name, "%s/PA/pa_7402_ur1", _FEP_FIFO);
	...
	for (i = 0; i < MAX_AUTO_PROC; i++)
	{
		...
		sprintf (fifo_name, "%s/PA/pa_75%1d%02dm11", _FEP_FIFO, i/10+1, i%10+1);
