...
/*----------------------------------------------------------------------*/
{
		...
		(memcmp (R_Pkt->Data, "KMA", 3) == 0)	)
	{
		...
		{
			/* Copy Data        */
			memcpy (W_Fmt[0].Data,  R_Pkt->Data+82, strlen(R_Pkt->Data) -82);
			W_Fmt[0].LineFeed[0] = '\n';
		}
		else
		{
	...
	else if (memcmp (R_Pkt->Data, "500", 3) == 0)
	{
		...
		else
		{
			...
			target = AtoIf (&Header->ApType_Cd[3], 2) + 2 + target;

			Log( USR_OK, "ApType[%5.5s] t01[%2.2s] t02[%2.2s] target[%d]",
				Header->ApType_Cd, &Header->ApType_Cd[1], &Header->ApType_Cd[3], target);
			Log( USR_OK, "W_Fmt=[%.100s]", &W_Fmt);
	...
	else if (memcmp (R_Pkt->Data, "100", 3) == 0)		// 100100 주문
	{
		...
		if (memcmp (dat->MembershipItem+35, "0", 1) == 0)
		{
			rt = DSHM_W (TS_W1_1, (void *)&W_Fmt, 1);	// 채권주문
		}
		else
		if (memcmp (dat->MembershipItem+35, "1", 1) == 0)
		{
			rt = DSHM_W (TS_W2_1, (void *)&W_Fmt, 1);	// 파생주문
		}
	}
#endif
	else
