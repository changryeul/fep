...
#define     CHK_PROC		"pa_1201_tr"					/* 체결Process가 인터페이스종료(TCHEDP99000) 수신후 종료되면 주문송신도 종료 */
#if 0
2025 미사용
#define     CHK_PROC		"pa_1601_tr"					/* 장운영 수신, 장중 */
...
/*----------------------------------------------------------------------*/
{
	...
	if (tr_code == TR_LOON)
	{
		...
		{
			Log (USR_ERROR, "LOGON response recv error TrCode[%11.11s]", &DataBuff[8+6]);
			close (Sockfd);
			return;
		}
...
#if 0
2024, 수신없음 송신임
	if (memcmp(S_Fmt.Header.MsgType, "SCHOPQ00000", 11) == 0)
	{
		Log (USR_OK, "업무개시요청(SCHOPQ00000): <%d:%d:%d>",
...
*/
	...
	{
		Log (USR_OK, "회선시험요청(SCHHER00000): [%4.4s] <%d:%d:%d> SHM_DATA[0].seq:<%d>",
			S_Fmt.Data, FirstSeq, INT_SEQ, LOAD_CNT, SHM_DATA[0].seq);
		return (RP_POLL);
	}
...
#if 0
/* 연계시 수신회원사번호 */
memcpy (S_Fmt.Header.DeliverToCompID, &R_Fmt[i].Data[, strlen (TCP_COMPANY));
/* 연계시 송신회원사번호 */
memcpy (S_Fmt.Header.DeliverToCompID, &R_Fmt[i].Data[, strlen (TCP_COMPANY));
#endif
...
// SEARCH_HEADER_LEN : sizeof (SEARCH_HEADER)) = 50 bytes
		...
		memcpy (&w_buf[sizeof(BUFF_RW_HEAD)],       "REJC00000000000",		15);
...
*/
	...
	for (i = 0; i < r_cnt; i ++)
	{
		...
		if ( (memcmp (&R_Fmt[i].Data[ADD_HEADER_SIZE+11], "TCHODR40001", 11) == 0)	||	/* 채권일반호가(254) */
			 (memcmp (&R_Fmt[i].Data[ADD_HEADER_SIZE+11], "TCHODR40002", 11) == 0)  ||
			 (memcmp (&R_Fmt[i].Data[ADD_HEADER_SIZE+11], "TCHODR40003", 11) == 0)  )
		{
			Size_Len	= sizeof (KRX_NOTE_JUMUN_DATA);					// 254
		}
		else
		if ( (memcmp (&R_Fmt[i].Data[ADD_HEADER_SIZE+11], "TCHMOR40001", 11) == 0)	||	/* 채권LP일반호가(255) */
			 (memcmp (&R_Fmt[i].Data[ADD_HEADER_SIZE+11], "TCHMOR40002", 11) == 0)  ||
			 (memcmp (&R_Fmt[i].Data[ADD_HEADER_SIZE+11], "TCHMOR40003", 11) == 0)  )
		{
...
*/
		...
		memcpy (J_Q_Fmt.JumunData[i].Data,	&R_Fmt[i].Data[ADD_HEADER_SIZE + sizeof(J_Q_Fmt.JumunData[0].DataSeq)],	// 0+11
																Size_Len - 11);
