...
SpotOrdNo *g_shm_oid_spot;    // 현물주문번호 Shm
...
/*--------------------------------------------------------*/
{
    ...
    // config
    if (load_config("/fsfxwin/fep/etc.ut/conf/config.ini") != 0) {
    ...
    G_OID_FUT->sem = Sem_Create(g_cfg.fut_oid_shm_key);
    if( G_OID_FUT->sem == NULL)
    {
        SLog(FIF_FATAL, "FutOrdNo Sem_Create error.");
        return(-6);
	}

	// 4. 현물 주문번호 공유메모리 세마포어 생성 및 attach 
	first = 0;
	shmsz = sizeof(SpotOrdNo);
    g_cfg.spot_oid_shm_size = g_cfg.spot_oid_shm_size*shmsz;

    shmid = shmget(g_cfg.spot_oid_shm_key, 0, 0666);
    if (shmid < 0)
    {
    	shmid = shmget(g_cfg.spot_oid_shm_key, sizeof(SpotOrdNo), IPC_CREAT|0666);
		if (shmid > 0) {
			first = 1;

		} else {
			SLog(FIF_FATAL, "shmget failed!!! key[%x] shmid[%d] shmsz[%d] errno[%d]\n", g_cfg.spot_oid_shm_key, shmid, sizeof(SpotOrdNo), errno);
			return(-7);
		}
    }
    ...
    SLog(USR_OK, "====> SIZE[%ld] key[%x] first[%d]", g_cfg.spot_oid_shm_size, g_cfg.spot_oid_shm_key, first);
	if (first) {
		memset(g_shm_oid_spot, 0x00, sizeof(SpotOrdNo));
	}

    G_OID_SPOT->sem = Sem_Create(g_cfg.spot_oid_shm_key);
    if( G_OID_SPOT->sem == NULL)
    {
...
/*--------------------------------------------------------*/
{
	...
	snprintf(l_dbgfile, sizeof(l_dbgfile), "%s/pa_5031_mp", g_cfg.log_file, get_today());
	...
	{
		Sem_Lock(G_OID_FUT->sem);
	    G_OID_FUT->ord_dt     = get_today();	
	    G_OID_FUT->fut_ordno  = 0;
		Sem_Unlock(G_OID_FUT->sem);
	}

	// 당일 New 주문번호 채번
	if (G_OID_SPOT->ord_dt == 0 || G_OID_SPOT->ord_dt != today)
	{
		Sem_Lock(G_OID_SPOT->sem);
	    G_OID_SPOT->ord_dt     = get_today();	
		G_OID_SPOT->spot_ordno = 0;
		Sem_Unlock(G_OID_SPOT->sem);
	}
...
//   G_ORDNO->spot_ordno_last = g_cfg.spotoidlast;                               // 현물주문번호 최종
	...
	l_dbg(L_ERR, "STRAT->fut_sym[%.12s] fut_idx[%d]", key.expcode, G_STRAT->fut_idx);
	succ_flag = 1;
	return(0);
