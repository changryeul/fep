...
/* 파생종목정보 사이즈 1318 (A006F), 7291(재처리) */
/*
#	A001F(10301/11301) : 1318, 종목정보
#	H001F(10301/11301) : 63, 정산가
*/
#elif defined A7201 || A7291 || A7301 || A7391
...
*/
		...
		if (memcmp (r_buf, "I2", 2) == 0)
		{
#if		1		/* polling data log */
				SLog (USR_OK, "I2 [%.5s]", r_buf);
#endif
				continue;
		}
#if defined A7201
		else if ((memcmp(r_buf, "A006F", 5) == 0)		||		// 종목정보
				(memcmp (r_buf, "H306F",   5) == 0))			// 정산가
		{
			W_Flag = 1;
		}
#elif defined A7291
		else if (memcmp(&r_buf[6], "A006F", 5) == 0)			// 종목정보
		{
			W_Flag = 1;
			if (memcmp(&r_buf[20], "99999", 5) == 0)
			{
				START_S = JOB_END;
...
#elif defined A7301
		else if (memcmp(r_buf, "A006V", 5) == 0)				// 종목정보
//				(memcmp (r_buf, "H306V",   5) == 0)				// 정산가
		{
			W_Flag = 1;
		}
#elif defined A7391
		else if (memcmp(&r_buf[6], "A006V", 5) == 0)			// 종목정보
		{
			W_Flag = 1;
			if (memcmp(&r_buf[20], "99999", 5) == 0)
			{
...
//				PROC(D_K,P_K).process_status = 9;
...
#endif
#if defined A7202
		else if ((memcmp (r_buf, "A306F",   5) == 0)    ||      // 체결, port 11305, size 173
				 (memcmp (r_buf, "G706F",   5) == 0)    ||      // 체결+우선호가 port 11305, size 431
				 (memcmp (r_buf, "B606F",   5) == 0)	||		// 우선호가 port 11305, size 324
				 (memcmp (r_buf, "A606F",   5) == 0)	||		// 종목마감, port 11305
				 (memcmp (r_buf, "A706F",   5) == 0)	||		// 장운영TS(종목장운영정보) port 11305
				 (memcmp (r_buf, "R106F",   5) == 0))			// 장운영TS+호가 port 11305
		{
			W_Flag = 2;
		}
#elif defined A7302
		else if ((memcmp (r_buf, "A306V",   5) == 0)    ||      // 체결, port 11305, size 173
				 (memcmp (r_buf, "G706V",   5) == 0)    ||      // 체결+우선호가 port 11305, size 431
				 (memcmp (r_buf, "B606V",   5) == 0)	||		// 우선호가 port 11305, size 324
				 (memcmp (r_buf, "A606V",   5) == 0)	||		// 종목마감, port 11305
				 (memcmp (r_buf, "A706V",   5) == 0)	||		// 장운영TS(종목장운영정보) port 11305
				 (memcmp (r_buf, "R106V",   5) == 0))			// 장운영TS+호가 port 11305
		{
			W_Flag = 2;
		}
#endif
#if defined A7203
		else if (memcmp (r_buf, "M406F",   5) == 0)				// 장운영스케줄 port 11315
		{
			W_Flag = 3;
		}
#elif defined A7303
		else if (memcmp (r_buf, "M406V",   5) == 0)				// 장운영스케줄 port 11315
		{
			W_Flag = 3;
		}
#endif
		...
		{
//			Log (USR_OK, "Else Case Receive RecvPkt[%5.5s]", r_buf);
			continue;
		}
...
#else
			ItoAf (IDW_CNT(0,0) + 1, W_Fmt.If_Seq,	sizeof (W_Fmt.If_Seq));
#endif
...
#if defined A7291 || A7391
			memcpy (W_Fmt.Data, &r_buf[6], len);
#else
...
#ifdef	SAM_USE
				rt = F_W (TS_W1_2, (void *)&W_Fmt, 1);
#else
				rt = DSHM_W (TS_W1_1, (void *)&W_Fmt, 1);
#endif
			else if (W_Flag == 3)
				rt = F_W (TS_W1_3, (void *)&W_Fmt, 1);

			if (rt != 1)
			{
#ifdef	SAM_USE
				SLog (SAM_FATAL, "file write fail[%s] rt[%d]", OFN(D_K,P_K,W_Flag-1), rt);
#else
				SLog (SAM_FATAL, "file write fail[%s] rt[%d]", ODN(D_K,P_K,W_Flag-1), rt);
#endif
				close (Sockfd);
				return;
			}
...
/*----------------------------------------------------------------------*/
{
	...
	memset(if_ip_addr, 0x00, sizeof(if_ip_addr));
	sprintf(if_ip_addr, "%d.%d.%d.%d", UDP_IF_IP1(D_K,P_K), UDP_IF_IP2(D_K,P_K),
									UDP_IF_IP3(D_K,P_K), UDP_IF_IP4(D_K,P_K));

	SLog(USR_OK, "Ip_Addr[%s] if_ip_addr[%s]", ip_addr, if_ip_addr);
...
*/
    ...
    {
		strcpy( if_ip_addr, "192.178.10.194");  
    }
	else if (memcmp ((char *)getenv ("_FEP_DIV"), "REAL2", 5) == 0)
    {
		strcpy( if_ip_addr, "192.168.10.204");  

    }
	else
    {
		strcpy( if_ip_addr, "192.168.10.203");  
    }
	mreq.imr_interface.s_addr   = inet_addr(if_ip_addr);
#endif
...
#if 0     // by lcr 추가 if_ip_addr 에 interface명칭을 넣어 보자  권한 문제로 막음
    if (memcmp ((char *)getenv ("_FEP_DIV"), "TEST", 4) == 0)
    {
		strcpy( ifname, "eno12419");  
    }
	else
    {
		strcpy( ifname, "bondS");  
	...
	rt = setsockopt (Sockfd, SOL_SOCKET, SO_BINDTODEVICE, ifname, strlen(ifname));
	if (rt == -1)
	{
		SLog (UDP_WARN, "setsockopt (SO_BINDTODEVICE) fail {%d:%s}", SYS_NO, SYS_STR);
		return (NOTOK);
	}
#endif
