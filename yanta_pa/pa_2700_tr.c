...
/*----------------------------------------------------------------------*/
{
	...
	*** */
	W_Flag = 0;
#if defined A7702			// 리커버리용
	if (memcmp (&RecvPkt[10],	"LOGOK",	5) == 0)				// LOG ON, OK
	{
		PktType = T_RSND;		// T_RSND 요청
        Send_Packet ();
	}
#else						// 리얼용
	if (memcmp (&RecvPkt[10],	"LOGOK",	5) == 0)				// LOG ON, OK
...
Log (USR_OK, "OK 01");
    ...
    else if ( PktType == T_RSND ) 			// 재전송요구
	{
		memcpy (&SendPkt[0],	"0000000040",	10);			// No Body
		if (memcmp (PROC(D_K,P_K).curr_tr, "REQJM", 	5) == 0	||
			memcmp (PROC(D_K,P_K).curr_tr, "REQIF",     5) == 0)
			memcpy (&SendPkt[10],	PROC(D_K,P_K).curr_tr,	5);		// REQJM(A0), REQIF(A3,G7)
		else
			memcpy (&SendPkt[10],	"REQJM",	5);		// REQJM(A0), REQIF(A3,G7)
	}
