...
//	sprintf(fifo_name, "%s/PA/pa_55%5.5s1", _FEP_FIFO, _Exe_Name+5);  // 선물시세수신
	...
	if (Poll[2].fd < 0)
	{
		/* P4-3: optional low-latency doorbell FIFO absent is non-fatal */
		/* strategy still runs on 100ms poll timeout; fills via DSHM */
		SLog(SYS_ERROR, "arb: optional latency FIFO absent[%s][%d:%s] - fallback to 100ms poll",
...
//	sprintf(fifo_name, "%s/PA/pa_7402_ur1", _FEP_FIFO);  // 현물시세수신
	/* P4-full(d): FEP-native doorbell keyed by slot; OD_SEQ=slot(0..39) computed first */
	OD_SEQ = (AtoIf (_Exe_Name+4, 2) - 1) * 10;
	OD_SEQ =  AtoIf (_Exe_Name+6, 2) + OD_SEQ - 1;
	sprintf(fifo_name, "%s/PO/pf_dbell_%02d", _FEP_FIFO, OD_SEQ);
