...
#if defined I1100 || I2100 || I3100 || I8100					/* 시세	*/
	sprintf (ApType, "WA%-5.5s%c",
		_Exe_Name+4, _Exe_Name[9] == 's' ? 'R' : 'S');
#else
	sprintf (ApType, "%c%-6.6s%c",
		_Exe_Name[0], _Exe_Name+3, _Exe_Name[9] == 's' ? 'R' : 'S');
#endif
...
/*----------------------------------------------------------------------*/
{
	...
	{
#if defined SK010 || SK011 || SK012 || SK013 || SK014 ||\
	SK210 || SK211 || SK212 || SK213 || SK214 ||\
    R3010 || R3011 || R3210 || R3211 ||\
    SE010 || SE210					/* SE:e-HR SK:HTS R:CTI	*/
		INT_SEQ = AtoIf (R_Pkt->Head.SeqNo, sizeof (R_Pkt->Head.SeqNo));
#endif
...
Log (USR_OK, "next[%d] data_len[%d] ", next, data_len);
...
/*----------------------------------------------------------------------*/
{
	...
	Log (USR_OK, "file write[%s:%d:%d]",
		OFN(D_K,P_K,0), OFW(D_K,P_K,0,0), d_cnt);
