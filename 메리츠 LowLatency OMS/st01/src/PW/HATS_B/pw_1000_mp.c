...
/*----------------------------------------------------------------------*/
{
	...
	while (START_S < JOB_END)
	{
		...
		for (i = 0; i < ThdCnt; i ++)
		{
			...
			{
				rt = pthread_create (&ThdCtx[i].Id, NULL,
					(void *(*)())Sync_Thread, &ThdCtx[i]);
...
/*----------------------------------------------------------------------*/
{
	int				i, fd, fifo_fd, rec_size, rt, sk, tk, status, len, pt, cnt;
	int				rcnt, wcnt, data_cnt, max_cnt, poll_cnt;
	long			offset;
	char			r_buf[4096], w_buf[4096], sub[4], poll_flag;
	char			fifo_name[128], f_name[128], tmp[128];
	struct pollfd	Poll;
	char			*cq;
	...
	if (fifo_fd == -1)
	{
		Log (FIF_FATAL, "cannot open FIFO [%s] {%d:%s}",
	...
	{
		close (fifo_fd);
		Log (SAM_FATAL, "open fail[%s] {%d:%s}", f_name, SYS_NO, SYS_STR);
		ThdCtx[tk].RunFlag = OFF;
		pthread_exit ((void *)&status);
	}
		...
		if (poll_flag == ON && data_cnt == 0)
		{
			while (1)
			{
				...
				else if (rt == -1)
				{
					...
					Log (FIF_ERROR, "[%s] cannot read FIFO {%d:%s}",
...
#if 0	/* test	*/
			Log (USR_OK, "[%s] W[%d] R[%d] cnt[%d]",
				DSHM(D_K,sk).data_name, wcnt, rcnt, cnt);
#endif
