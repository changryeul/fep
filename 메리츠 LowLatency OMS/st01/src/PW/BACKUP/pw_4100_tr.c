...
#if defined A1201
#define	BIZ_TYPE	"A303"
#elif defined A1202
#define	BIZ_TYPE	"A313"
#elif defined A2201
#define	BIZ_TYPE	"B303"
#elif defined A2202
#define	BIZ_TYPE	"B313"
#endif
/*------------------------------------------------------------------------
	Global Variables
------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------*/
{
	...
	if ( (memcmp (R_Pkt->Head.DataCnt, "91", 2) == 0)	||		// T_LIOK(91)
		((memcmp (R_Pkt->Head.DataCnt, "01", 2) >= 0) &&		// T_DATA(01~08)
		 (memcmp (R_Pkt->Head.DataCnt, "08", 2) <= 0))	)
	{
		return (OK);
	}
	else if (memcmp (R_Pkt->Head.DataCnt, "92", 2) == 0)		// T_RSOK(92,개시응답)
	{
...
/*----------------------------------------------------------------------*/
{
    ...
    /* 3. 업무식별코드 */
#if defined A1201
    memcpy (S_Pkt->ApType, "A303", sizeof (S_Pkt->ApType));
#elif defined A1202
    memcpy (S_Pkt->ApType, "A313", sizeof (S_Pkt->ApType));
#elif defined A2201
    memcpy (S_Pkt->ApType, "B303", sizeof (S_Pkt->ApType));
#elif defined A2202
    memcpy (S_Pkt->ApType, "B313", sizeof (S_Pkt->ApType));
#endif
...
/*----------------------------------------------------------------------*/
{
	...
	for (i = 0, next = 0; i < d_cnt; i ++)
	{
		f_gbn = 0;
		/* ************************ */
		/* File Write용 Header 생성 */
		ItoAf (INT_SEQ + i + 1,	f_head.Seq, 	sizeof (f_head.Seq));
		ItoAf (INT_SEQ + i + 1,	f_head.If_Seq,	sizeof (f_head.If_Seq));
		memcpy (f_head.ApType, BIZ_TYPE, 		strlen (ApType));
		...
		ItoAf (DataSize, f_head.DataHeader, 4);
		memset (&f_head.DataHeader[4], 0x20, HEAD_SIZE-4);	// 20-4=16 byte
		/* ************************ */
		...
		/* ********************************************* */
		/* Data 부 TrCode체크(가변이라 Size체크해야한다) */
		/* 데이터헤더(70)+11, TRCODE CHECK				 */
		memset (&w_buf[rec_size*i+f_size], 0x20, DataSize);
		if (memcmp (data+TCP_DATA_HEAD_LEN+11+next, "MERITZREJEC", 11) == 0)	// FEP/KRX거부
		{
			f_gbn = 1;
			len = TCP_DATA_HEAD_LEN+TCP_REJE_HEAD_LEN+sizeof(KRX_JUMUN_DATA);	// 70+80+261=>411
		}
		else
		if (memcmp (data+TCP_DATA_HEAD_LEN+11+next, "TTRTDP21301", 11) == 0)	// TTRTDP21301(체결)
		{
			f_gbn = 2;
			len = TCP_DATA_HEAD_LEN+sizeof(KRX_SETTLE_DATA);					// 70+242=>312
		}
		else
		if ((memcmp (data+TCP_DATA_HEAD_LEN+11+next, "TTRODP11301", 11) == 0)	||	// TTRODP11301(정상확인), TTRODP11321(거부)
			(memcmp (data+TCP_DATA_HEAD_LEN+11+next, "TTRODP11321", 11) == 0)	)	
		{
			f_gbn = 1;
			len = TCP_DATA_HEAD_LEN+sizeof(KRX_SETTLE_RESP_DATA);				// 70+287=>357
		}
		else
		if ((memcmp (data+TCP_DATA_HEAD_LEN+11+next, "TCHORD10001", 11) == 0)	||	// 신규/정정/취소 주문응답
			(memcmp (data+TCP_DATA_HEAD_LEN+11+next, "TCHORD10002", 11) == 0)	||	
			(memcmp (data+TCP_DATA_HEAD_LEN+11+next, "TCHORD10003", 11) == 0)	)
		{
			f_gbn = 1;
			len = TCP_DATA_HEAD_LEN+sizeof(KRX_JUMUN_DATA);						// 70+261=>411
		}
		else
		{
			Log (USR_ERROR, "Data Type Error [%11.11s] [%200.200s]", 
				data+TCP_DATA_HEAD_LEN+11+next, data+TCP_DATA_HEAD_LEN+next);
			return (NOTOK);
		}
		/* ********************************************* */

		if (len <= HEAD_SIZE || len > DataSize)
		{
		...
		memcpy (&w_buf[rec_size*i+f_size], data+next, len);
	...
	if (f_gbn == 1)
		...
		rt = F_W (TS_W2_1, w_buf, d_cnt);
#else
	if (f_gbn == 1)
		rt = DSHM_W (TS_W1_1, w_buf, d_cnt);
	else if (f_gbn == 2)
		rt = DSHM_W (TS_W2_1, w_buf, d_cnt);
#endif
