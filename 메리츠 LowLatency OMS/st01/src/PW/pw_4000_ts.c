...
#if defined A6610||A6620||A6630||A6640||A6650||A6660||A6670||A6680||A6690||A6700
				/* Data 통신부의 Filler값(20번째)에 값 set함. */
				/* 응답:1, 체결:2, 조회:3, 주문후처리:F	*/
				if (memcmp (&R_Buf[(f_len+LegSize+1)*i+f_len+34],
							"TCHODR", 6) == 0)
					memcpy (&S_Pkt->Data[next+19], "1",	1);
				else
				if (memcmp (&R_Buf[(f_len+LegSize+1)*i+f_len+34],
							"TCHTDP", 6) == 0)
					memcpy (&S_Pkt->Data[next+19], "2",	1);
				else
				if (memcmp (&R_Buf[(f_len+LegSize+1)*i+f_len+16],
							"FFFF", 4) == 0)
					memcpy (&S_Pkt->Data[next+19], "F", 1);
				else
					memcpy (&S_Pkt->Data[next+19], "3", 1);
#endif
