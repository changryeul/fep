...
/*----------------------------------------------------------------------*/
void	PW_3010_TR (int argc, char **argv)
/*----------------------------------------------------------------------*/
{
	int		rt;
...
/*----------------------------------------------------------------------*/
{
	...
	for (i = 0; i < PROC_MAX; i ++)
	{
        ...
        {
#if defined CHK_TS
            sprintf(chk_proc, "%.10s", ODN(D_K,P_K,0));
#elif defined CHK_TS1
            if ((memcmp ((char *)getenv ("host_name"), "ap68", 4) == 0) && (memcmp (_Exe_Name+3, "3", 1) == 0))
            {
                memcpy(chk_proc,    OFN(D_K,P_K,0),     4);
                memcpy(chk_proc+4,  "2",                1);
                memcpy(chk_proc+5,  OFN(D_K,P_K,0)+5,   5);
            }
            else
            {
                sprintf(chk_proc, "%.10s", ODN(D_K,P_K,0));
...
#elif defined CHK_PR
            sprintf (chk_proc, "%-7.7s_pr", _Exe_Name);
#else
            sprintf (chk_proc, "%-5.5s01_ps", _Exe_Name, _Exe_Name[6]);
#endif
        }
        else
        {
...
#elif defined CHK_PR
            sprintf (chk_proc, "%-6.6s%d_pr", _Exe_Name, i + 1);
#else
            sprintf (chk_proc, "%-6.6s%d_ps", _Exe_Name, i + 1);
#endif
        }
...
#if defined CHK_TS1
                Log (USR_ERROR, "check tcp status[%.4s]", S_Pkt->ResponseCode);
#else
...
/*----------------------------------------------------------------------*/
{
	int				i, next, seq, d_cnt, rt, loop, len, f_size, rec_size;
	...
	for (i = 0, next = 0; i < d_cnt; i ++)
	{
		...
		if (rt < 0)
		{
			...
			INT_SEQ += 1;
			Set_TR_Time ();
			Log (USR_OK, "DSHM write 01[%s:%d:%d]", ODN(D_K,P_K,DELAY_TIME - 1), ODW(D_K,P_K,DELAY_TIME - 1,0), 1);
	...
	INT_SEQ += 1;
	Set_TR_Time ();
	Log (USR_OK, "File write[%s:%d:%d]", OFN(D_K,P_K,0), OFW(D_K,P_K,0,0), 1);
	ItoAf (1, S_Pkt->DataCnt, sizeof (S_Pkt->DataCnt));
#endif
...
/*----------------------------------------------------------------------*/
{
	...
	for (i = 0; i < PROC_MAX; i ++)
	{
		...
		nstat1 = ON;
#elif defined CHK_TS1
        nstat1 = TCP1_NSTAT(D_K,Pk[i]);
#else
        nstat = X25_NSTAT(D_K,Pk[i],X25_LINE_GUBUN(D_K,Pk[i]));
        lstat = X25_LSTAT(D_K,Pk[i],X25_LINE_GUBUN(D_K,Pk[i]));
#endif

		if ((nstat == ON && lstat == ON) || (nstat1 == ON))
			return (OK);
		else if (nstat == OFF)
			off_f ++; 
		else if (nstat == END)
	...
	if (nstat1 != ON)
		memcpy (S_Pkt->ResponseCode, ERR_TCP_STATUS, strlen (ERR_TCP_STATUS));
