...
/* ELW는 JA다 */
#if defined A1566||A1567||A1569
	sprintf (ApType, "JA%-4.4s%c%c",
	...
	while (START_S < JOB_END)
	{
		...
		else
		{
			while (START_S < JOB_END)
			{
				if (WR_CNT > RD_CNT)
				{
#if defined A1536||A1537||A1539||A1566||A1567||A1569
					if (MAX_POLL < (WR_CNT - RD_CNT))
					{
						Log (SAM_ERROR, "01 WR_CNT[%d] RD_CNT[%d]",
							WR_CNT, RD_CNT);
						sleep(60);
						Exit_Process ();
					}
			...
			case	FILE_EVENT:
				...
				while (START_S < JOB_END)
				{
					if (WR_CNT > RD_CNT)
					{
						...
						if (MAX_POLL < (WR_CNT - RD_CNT))
						{
							Log (SAM_ERROR, "02 WR_CNT[%d] RD_CNT[%d]",
								WR_CNT, RD_CNT);
							sleep(60);
							Exit_Process ();
						}
...
/*----------------------------------------------------------------------*/
{
	...
	if ( PktType == T_DATA )
	{
		...
		for (i = 0, next = 0; i < DataCnt; i ++)
		{
			...
			/* 취소, 청산주문만 처리	*/
#if defined A1537 || B1532
			ACCNO(D_K, 4).js_order_no_band ++;
			ItoAf (ACCNO(D_K, 4).js_order_no_band,
					&S_Pkt->Data[next+20+82+33], 6);
#endif
#if defined A1539
			/* aptype 50번 전용 채널(MC처리) */
			ACCNO(D_K, 19).acc_order_no ++;
			ItoAf (ACCNO(D_K, 19).acc_order_no,
					&S_Pkt->Data[next+20+82+33], 6);
#endif
#if defined A1567
			/* aptype 65번 전용 채널 */
			ACCNO(D_K, 24).acc_order_no ++;
			ItoAf (ACCNO(D_K, 24).acc_order_no,
					&S_Pkt->Data[next+20+82+33], 6);
#endif
