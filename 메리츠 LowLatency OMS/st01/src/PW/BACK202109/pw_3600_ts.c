...
/*------------------------------------------------------------------------
	Global Variables
------------------------------------------------------------------------*/
int		Sockfd, PortNo, DataCnt, PollCnt, PktType, MaxCnt, LegSize, RetryCnt;
char	ApType[10], RecvPkt[TCP_BUFF_MAX_LEN], SendPkt[TCP_BUFF_MAX_LEN];
char	IpAddr[20], DeviceSendFlag, DataSendFlag, R_Buf[FILE_BUF_LEN];
...
/*----------------------------------------------------------------------*/
{
	...
	if (rt == OK)
	{
		...
		else if (memcmp (R_Pkt->MsgType, TCP_STOK_CD,
...
/*----------------------------------------------------------------------*/
{
	int		data_len, f_len, i, next, rt, val;
	...
	if (AtoIf (R_Pkt->ResponseCode, sizeof (R_Pkt->ResponseCode)) != 0 &&
		PktType != T_POLL)
	{
		if (PktType == T_DATA && R_Pkt->ResponseCode[0] == 'K')
			return (FAIL);
		else
		{
...
/*----------------------------------------------------------------------*/
{
	...
	{
		TCP2_CON_STA = OFF;
		Log (SAM_FATAL, "cannot read file[%s]", IFN(D_K,P_K,0));
		sleep (1);
		close (Sockfd);
		Exit_Process ();
	}
...
#if (0)
	Log (SAM_OK, "F_R: IFN[%s] IFR[%d]", IFN(D_K,P_K,0), RD_CNT);
	Log (SAM_OK, "SAM RD [%s](%d)", R_Buf, strlen (R_Buf));
#endif
