...
*/
	...
	while (1)
	{
...
#if 0
		// 영업일 테스트 START 2021.07.09
		Log (USR_OK, "INFO(D_K).date:[%s] DAEMON(D_K).date:[%s] Shm_Lg[0].business_day[%.8s]", 
				INFO(D_K).date, DAEMON(D_K).date, Shm_Lg[0].business_day);
		
		if (strlen(Shm_Lg[0].business_day) < 8)
		{
			Log(USR_OK, "business_date is NULL");
		}

		if (memcmp(DAEMON(D_K).date, Shm_Lg[0].business_day, sizeof(DAEMON(D_K).date)) != 0 &&
			strlen(Shm_Lg[0].business_day) == 8 &&
			StopProc == 0 )
		{
			Log(USR_OK, "Check Date Stop Process");
			StopProc = 1;
			Stop_Process();
		}
		...
		if (memcmp(DAEMON(D_K).date, Shm_Lg[0].business_day, sizeof(DAEMON(D_K).date)) == 0 &&
			...
			StopProc == 1 )
		{
			Log(USR_OK, "Check Date Start Process");
			StopProc = 2;
			Start_Process();
			StopProc = 0;
		}
	...
	switch (process_no)
	{
		...
		case	OK:
			...
			Log (SYS_ERROR, "execl(%s,%s,%s) failure[%s] {%d:%s}",
				...
				"unlink the expired data, log and FIFO", SYS_NO, SYS_STR);
			exit (FAIL);
		default:
			wait (&status);
	}
...
/*----------------------------------------------------------------------*/
{
	...
	switch (process_no)
	{
		...
		case	OK:
			...
			Log (SYS_ERROR, "execl(%s,%s,%s,%s) failure[%s] {%d:%s}",
...
/**************************************************************************
   Functions      : . SHM Backup Process 기동
   Parameters IN  : .
   Parameters OUT : .
   Return Code    : . void
**************************************************************************/
...
/*----------------------------------------------------------------------*/
{
	...
	{
		case	SIGUSR1:
			Log (SYS_OK, "signal caught [%d:SIGUSR1], pa_daemon_proc", p_signo);
			signal (SIGUSR1, Sig_Handler);
			break;
		case	SIGUSR2:
			Log (SYS_OK, "signal caught [%d:SIGUSR2], pa_daemon_proc", p_signo);
			signal (SIGUSR2, Sig_Handler);
			break;
		default:
			Log (SYS_OK, "signal caught [%d:XXXXXXX], pa_daemon_proc", p_signo);
