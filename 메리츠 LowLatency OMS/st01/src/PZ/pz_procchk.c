...
typedef	struct Mem_Read_Data {
	char	daemon_id[20];	/* 부문 daemon의 process ID					*/
	char	start_time[6];	/* 부문 기동 시각 (hhmm)					*/
	char	end_time[6];	/* 부문 종료 시각 (hhmm)					*/
	char	pname[1000][20];/* 업무 process ID							*/
	u_char	pstat[1000];	/* process status (1:run 2:stop 9:not run)	*/
	short	count;			/* 실행 대상 process 개수					*/
	char	chk_flag;		/* 부문 daemon 기동 여부 (0:미기동 1:기동)	*/
}	Data_Info;
		...
		for (k = 0; k < Data[i].count; k ++) 
		{
			...
			else 
			{
				...
				if (Data[i].pstat[k] == 2)                      /* stop */
                    ...
                    Log (PRO_ERROR, "process(%d,%s) not run",
                        not_cnt, Data[i].pname[k]);
				...
				{
					// 20210929
					Log (USR_OK, "DAEMON(i).date:[%s], Shm_Lg[0].business_day:[%s][%d]", 
							DAEMON(i).date, Shm_Lg[0].business_day, strlen(Shm_Lg[0].business_day));
					if (memcmp(DAEMON(i).date, Shm_Lg[0].business_day, sizeof(DAEMON(i).date)) == 0 && 
				...
				else											/* run	*/
				{
					if (NR > 0)
					{
						/* add */
						rt = 0;
						sprintf (path, "ps -ef|grep %s|egrep -v 'grep|ps|sh|ls|vi|view|tail|ls'|wc -l > /tmp/msp1", Data[i].pname[k]);
						system(path);
						if ((fp_b = fopen ("/tmp/msp1", "r")) == NULL)
						{
							Log (USR_ERROR, "cannot open:[/tmp/msp1]");
						}
						else
						{
...
/*----------------------------------------------------------------------*/
{
	...
	for (dk = 0; dk < Process_Count; dk ++) 
	{
		...
		Data[dk].count = k;		/* 0부터 k-1까지 Proc_Id (pname) Save	*/
	}
...
/*----------------------------------------------------------------------*/
{
	int				i, b;
	float			rt;
#if defined __hpux || sun
	struct statvfs	mbuf;
#elif defined _AIX || __linux
	struct statfs	mbuf;
#endif
