...
------------------------------------------------------------------------*/
extern int	Rows;
int			Sk, Curr_Arry, In_Arry, FO_Flag;
char		ItemCd[15];
...
/*----------------------------------------------------------------------*/
{
	...
	attroff (A_BOLD);
	Draw_Line (1, 0, 79);
	mvaddstr (2, 0, "상품   (1:선물 2:옵션 3:ELW)    종목코드");
	Draw_Underline (2, 5, 1);
	Draw_Underline (2, 41, 12);
	mvaddstr (3, 0, "순번");
...
/*----------------------------------------------------------------------*/
{
	...
	while (1)
	{
		...
		else
		{
			...
			if (strlen (buf) != 1 || (buf[0] != '1' && buf[0] != '2' && buf[0] != '3'))
			{
	...
	while (1)
	{
		...
		{
			Draw_Underline (2, 41, 12);
			rt = Get_String (2, 41, 12, buf);
		}
		else
		{
			Draw_Underline (2, 41, 8);
			rt = Get_String (2, 41, 8, buf);
		}
		...
		else
		{
			...
			{
				/* 종목 찾기 */
				if ((memcmp (Shm_Elw[Shm_Elw[0].total_elw_cnt-1].Elw_Master.stock_code,
							buf, 12) == -1)	||
					(memcmp (Shm_Elw[0].Elw_Master.stock_code, buf, 12) == 1))
				{
					Disp_Msg ("ELW_01 종목코드 미존재 (Esc:Cancel)");
					continue;
				}
				else
				{
					i = jy = left = 0;
					right = Shm_Elw[0].total_elw_cnt - 1;

					while( left <= right )
					{
						mid = (left + right) / 2;

						if (memcmp (Shm_Elw[mid].Elw_A1.stock_code, buf, 12) == 0)
						{
							jy = 1;
							break;
						}
						/* 1 왼쪽이 크다 */
						else if (memcmp (Shm_Elw[mid].Elw_A1.stock_code, buf, 12) == 1)
							right = mid-1;
						else if (memcmp (Shm_Elw[mid].Elw_A1.stock_code, buf, 12) == -1)
							left = mid+1;
						else
						{
							Disp_Msg ("A0 Serch NotOk");
							continue;
						}
						i ++;
					}
					...
					if (jy == 0)
					{
						Disp_Msg ("ELW_02 종목코드 미존재 (Esc:Cancel)");
						continue;
					}
...
/*----------------------------------------------------------------------*/
{
	...
	{
		mvaddstr (4, 0, "ELW 현재가 (ELW_CURR)");
		Curr_Arry = Shm_Elw[Sk].CURR_Arry_Key;
...
*/
	...
	{
        MvAddStr (6, 9, 9, Shm_Elw[Sk].Elw_Master.compress_code);
        MvAddStr (6, 33, 30, Shm_Elw[Sk].Elw_Master.name);
        MvAddStr (8, 7, 7, Shm_Elw[Sk].Elw_Master.high_limit_price+2);
        MvAddStr (8, 25, 7, Shm_Elw[Sk].Elw_Master.low_limit_price+2);
        MvAddStr (9, 7, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].opening_price+3);
        MvAddStr (9, 23, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].highest_price+3);
        MvAddStr (9, 41, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].lowest_price+3);
		MvAddStr (10, 9, 7, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].trade_qty+3);
        MvAddStr (10, 29, 8, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].accum_trade_qty+4);
        MvAddStr (10, 56, 18, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].accum_trade_amt);

        /* 현재가 */
        MvAddStr (12, 7, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].current_price+3);
        /* 우선호가 */
        MvAddStr (13, 14, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[0].offer+3);
        MvAddStr (13, 40, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[0].bid+3);
        MvAddStr (14, 14, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[1].offer+3);
        MvAddStr (14, 40, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[1].bid+3);
        MvAddStr (15, 14, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[2].offer+3);
        MvAddStr (15, 40, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[2].bid+3);
        MvAddStr (16, 14, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[3].offer+3);
        MvAddStr (16, 40, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[3].bid+3);
        MvAddStr (17, 14, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[4].offer+3);
        MvAddStr (17, 40, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[4].bid+3);

		mvaddstr (18, 0, "합산");
        /* 총 호가수량 */
        MvAddStr (19, 15, 7, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].tot_offer_qty+5);
        MvAddStr (19, 41, 7, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].tot_bid_qty+5);
        /* 호가수량 */
        MvAddStr (20, 18, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[0].offer_qty+6);
        MvAddStr (20, 44, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[0].bid_qty+6);
        MvAddStr (21, 18, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[1].offer_qty+6);
        MvAddStr (21, 44, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[1].bid_qty+6);
        MvAddStr (22, 18, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[2].offer_qty+6);
        MvAddStr (22, 44, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[2].bid_qty+6);
        MvAddStr (23, 18, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[3].offer_qty+6);
        MvAddStr (23, 44, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[3].bid_qty+6);
        MvAddStr (24, 18, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[4].offer_qty+6);
        MvAddStr (24, 44, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[4].bid_qty+6);

		mvaddstr (25, 0, "LP");
        /* 호가수량(LP) */
        MvAddStr (27, 18, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[0].LP_offer_qty+6);
        MvAddStr (27, 44, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[0].LP_bid_qty+6);
        MvAddStr (28, 18, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[1].LP_offer_qty+6);
        MvAddStr (28, 44, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[1].LP_bid_qty+6);
        MvAddStr (29, 18, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[2].LP_offer_qty+6);
        MvAddStr (29, 44, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[2].LP_bid_qty+6);
        MvAddStr (30, 18, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[3].LP_offer_qty+6);
        MvAddStr (30, 44, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[3].LP_bid_qty+6);
        MvAddStr (31, 18, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[4].LP_offer_qty+6);
        MvAddStr (31, 44, 6, Shm_Elw[Sk].Elw_Curr_Arry[Curr_Arry].hoga[4].LP_bid_qty+6);
	}
