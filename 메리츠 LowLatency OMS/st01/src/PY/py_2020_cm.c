...
/*----------------------------------------------------------------------*/
{
	Disp_Title ();
	attron (A_BOLD);
	mvaddstr (0, 18, "[2020] 시세 - 체결 (G7)");
	attroff (A_BOLD);
	Draw_Line (1, 0, 79);
	mvaddstr (2, 0, "상품   (1:선물 2:옵션)    종목코드");
	mvaddstr (3, 0, "횟수:     , 현재번호:");
...
/*----------------------------------------------------------------------*/
{
	...
	if (FO_Flag == '1')											/* 선물	*/
	{
		MvAddNum (3, 6, 3, Shm_Futures[Sk].CHE_Cnt);
		MvAddNum (3, 22, 3, Shm_Futures[Sk].CHE_Arry_Key);
		MvAddNum (3, 26, 3, Shm_Futures[Sk].Befor_CHE_Arry_Key);
		mvaddstr (4, 0, "지수선물 체결가 (Futures_G7)");
		memcpy (fms_c0.tr_gbn, Shm_Futures[Sk].Futures_G7.tr_gbn,
			sizeof (SIF_G7) - 8);
	}
	else														/* 옵션	*/
	{
		MvAddNum (3, 6, 3, Shm_Options[Sk].CHE_Cnt);
		MvAddNum (3, 22, 3, Shm_Options[Sk].CHE_Arry_Key);
		MvAddNum (3, 26, 3, Shm_Options[Sk].Befor_CHE_Arry_Key);
		mvaddstr (4, 0, "지수옵션 체결가 (Options_G7)");
		memcpy (opt_c0.tr_gbn, Shm_Options[Sk].Options_G7.tr_gbn,
			sizeof (SIO_G7) - 8);
	}
	...
	if (FO_Flag == '1')											/* 선물	*/
	{
		...
		MvAddStr (7, 9, 8, fms_c0.item_code+3);
		MvAddStr (7, 33, 30, Shm_Futures[Sk].Futures_A0.kor_nm);
	...
	else														/* 옵션	*/
	{
        ...
        MvAddStr (7, 9, 8, opt_c0.item_code+3);
        MvAddStr (7, 33, 30, Shm_Options[Sk].Options_A0.kor_nm);
