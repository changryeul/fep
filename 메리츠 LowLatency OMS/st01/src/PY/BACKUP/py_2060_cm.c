...
/*----------------------------------------------------------------------*/
{
	...
	if (FO_Flag == '1')										/* 선물	*/
	{
		s_k = Shm_Futures[0].total_stock_cnt;
	}
	else
	{
		s_k = Shm_Options[0].total_stock_cnt;
	}
	...
	if (FO_Flag == '1')                                 	/* 선물	*/
	{
   		MvAddStr (5, 9,  8, Shm_Futures[Arry].Futures_FB.item_cd);
   		MvAddStr (5, 33,  30, Shm_Futures[Arry].Futures_FB.kor_nm);
   		MvAddNum (7, 10,  2, Shm_Futures[Arry].Futures_CURR.check_cnt);
		pr = Shm_Futures[Arry].Futures_CURR.middle_price;
   		MvAddNum (7, 25,  3, pr / 1000);
   		MvAddNum (7, 29,  3, pr % 1000);
		pr = Shm_Futures[Arry].Futures_CURR.af_middle_price;
   		MvAddNum (7, 49,  3, pr / 1000);
   		MvAddNum (7, 53,  3, pr % 1000);
	}
	else
	{
   		MvAddStr (5, 9,  8, Shm_Options[Arry].Options_OB.item_cd+3);
   		MvAddStr (5, 33,  30, Shm_Options[Arry].Options_OB.kor_nm);
   		MvAddNum (7, 10,  2, Shm_Options[Arry].Options_CURR.check_cnt);
		pr = Shm_Options[Arry].Options_CURR.middle_price;
   		MvAddNum (7, 25,  3, pr / 1000);
   		MvAddNum (7, 29,  3, pr % 1000);
		pr = Shm_Options[Arry].Options_CURR.af_middle_price;
   		MvAddNum (7, 49,  3, pr / 1000);
   		MvAddNum (7, 53,  3, pr % 1000);
	}
