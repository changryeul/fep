...
/*----------------------------------------------------------------------*/
{
	...
	if (FO_Flag == '1')										/* 선물	*/
	{
		...
		memcpy (fms_curr.item_cd, 
				Shm_Futures[Sk].Futures_CURR_Arry[Curr_Arry].item_cd, 
				sizeof (FMS_C0) - 4);

		mid_pr = 
		Shm_Futures[Sk].Futures_CURR.middle_price;
	}
	else													/* 옵션	*/
	{
		...
		memcpy (opt_curr.item_cd, 
				Shm_Options[Sk].Options_CURR_Arry[Curr_Arry].item_cd,
				sizeof (OPT_C0) - 4);

		mid_pr = 
		Shm_Options[Sk].Options_CURR.middle_price;
	}
	...
	if (FO_Flag == '1')											/* 선물	*/
	{
		...
		MvAddStr (6, 9, 8, fms_curr.item_cd);
		...
		MvAddStr (10, 13, 7, fms_curr.tot_con_qty);
		MvAddStr (10, 45, 11, fms_curr.tot_con_amt);

		/* 현재가 */
		MvAddStr (12, 7, 3, fms_curr.crprc);
		MvAddStr (12, 11, 2, fms_curr.crprc+3);
		MvAddNum (12, 40, 3, mid_pr / 1000);
		MvAddNum (12, 44, 3, mid_pr % 1000);
		/* 우선호가 */
	...
	else														/* 옵션	*/
	{
        ...
        MvAddStr (6, 9, 8, opt_curr.item_cd);
        ...
        MvAddStr (10, 13, 8, opt_curr.tot_con_qty);
        MvAddStr (10, 45, 11, opt_curr.tot_con_amt);
