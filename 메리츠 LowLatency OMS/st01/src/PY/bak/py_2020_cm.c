...
/*----------------------------------------------------------------------*/
{
	...
	if (FO_Flag == '1')											/* 선물	*/
	{
		mvaddstr (3, 0, "지수선물 체결가 (Futures_G7)");
		memcpy (fms_c0.item_code, Shm_Futures[Sk].Futures_G7.item_code,
			sizeof (SIF_G7));
	}
	else														/* 옵션	*/
	{
		mvaddstr (3, 0, "지수옵션 체결가 (Options_G7)");
		memcpy (opt_c0.item_code, Shm_Options[Sk].Options_G7.item_code,
			sizeof (SIO_G7));
	}
