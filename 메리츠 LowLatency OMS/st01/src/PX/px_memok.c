...
/*************************************************************************
	Function		: . read accno.ini and check the account SHM (ACCNO)
	Parameters IN	: .
	Parameters OUT	: .
	Return Code		: . void
*************************************************************************/
...
/*-----------------------------------------------------------------------*/
{
	...
	while ((sp = fgets (buf, sizeof (buf), fp)) != NULL)
	{
		...
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			...
			if (DAEMON(p_cnt).a_count != atoi (tmp2))
				printf ("%s [%d:%s]\n", tmp3, DAEMON(p_cnt).a_count, tmp2); 
		...
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			if (memcmp (ACCNO(p_cnt,cnt-1).info, sp1+1, strlen (sp1+1)) != 0)
				printf ("%s [%s:%s]\n", tmp3, ACCNO(p_cnt,cnt-1).info, sp1+1); 
		...
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			if (memcmp (ACCNO(p_cnt,cnt-1).aptype_code, tmp2,
				strlen (tmp2)) != 0)
				printf ("%s [%s:%s]\n", tmp3, ACCNO(p_cnt,cnt-1).aptype_code,
					tmp2); 
		...
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			if (memcmp (ACCNO(p_cnt,cnt-1).acc_no, tmp2, strlen (tmp2)) != 0)
				printf ("%s [%s:%s]\n", tmp3, ACCNO(p_cnt,cnt-1).acc_no, tmp2); 
		...
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			if (memcmp (ACCNO(p_cnt,cnt-1).login_pwd, tmp2,
				strlen (tmp2)) != 0)
				printf ("%s [%s:%s]\n", tmp3, ACCNO(p_cnt,cnt-1).login_pwd,
					tmp2); 
		...
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			if (memcmp (ACCNO(p_cnt,cnt-1).athrty, tmp2,
				strlen (tmp2)) != 0)
				printf ("%s [%s:%s]\n", tmp3, ACCNO(p_cnt,cnt-1).athrty, tmp2); 
		...
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			if (memcmp (ACCNO(p_cnt,cnt-1).ip_addr, tmp2, strlen (tmp2)) != 0)
				printf ("%s [%s:%s]\n", tmp3, ACCNO(p_cnt,cnt-1).ip_addr, tmp2); 
			continue;
		}
		...
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			if (memcmp (ACCNO(p_cnt,cnt-1).auto_man_cnt, tmp2, strlen (tmp2)) != 0)
				printf ("%s [%s:%s]\n", tmp3, ACCNO(p_cnt,cnt-1).auto_man_cnt, tmp2); 
			continue;
		}
		...
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			if (ACCNO(p_cnt,cnt-1).js_order_no_band != atoi (tmp2))
				printf ("%s [%d:%s]\n", tmp3,
					ACCNO(p_cnt,cnt-1).js_order_no_band, tmp2); 

			continue;
		}

#if 0	// 202109 삭제
		sprintf (tmp3, "ACCNO_%d_JS_BAND_B", cnt);
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			if (ACCNO(p_cnt,cnt-1).js_order_no_band_b != atoi (tmp2))
				printf ("%s [%d:%s]\n", tmp3,
					ACCNO(p_cnt,cnt-1).js_order_no_band_b, tmp2); 

			continue;
		}

		sprintf (tmp3, "ACCNO_%d_CLI_BAND", cnt);
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			if (ACCNO(p_cnt,cnt-1).acc_order_no != atoi (tmp2))
				printf ("%s [%d:%s]\n", tmp3,
					ACCNO(p_cnt,cnt-1).acc_order_no, tmp2); 
		...
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			if (ACCNO(p_cnt,cnt-1).acc_risk_max != atoi (tmp2))
				printf ("%s [%d:%s]\n", tmp3,
					ACCNO(p_cnt,cnt-1).acc_risk_max, tmp2); 
		...
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			if (ACCNO(p_cnt,cnt-1).acc_f_order_maxcnt != atoi (tmp2))
				printf ("%s [%d:%s]\n", tmp3,
					ACCNO(p_cnt,cnt-1).acc_f_order_maxcnt, tmp2); 
		...
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			if (ACCNO(p_cnt,cnt-1).acc_f_get_maxcnt != atoi (tmp2))
				printf ("%s [%d:%s]\n", tmp3,
					ACCNO(p_cnt,cnt-1).acc_f_get_maxcnt, tmp2); 
		...
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			if (ACCNO(p_cnt,cnt-1).acc_so_order_maxcnt != atoi (tmp2))
				printf ("%s [%d:%s]\n", tmp3,
					ACCNO(p_cnt,cnt-1).acc_so_order_maxcnt, tmp2); 
		...
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			if (ACCNO(p_cnt,cnt-1).acc_so_get_maxcnt != atoi (tmp2))
				printf ("%s [%d:%s]\n", tmp3,
					ACCNO(p_cnt,cnt-1).acc_so_get_maxcnt, tmp2); 
