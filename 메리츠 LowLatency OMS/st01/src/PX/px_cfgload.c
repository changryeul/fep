...
/*----------------------------------------------------------------------*/
{
	...
	while (1)
	{
		...
		if (memcmp (buf+2, tmp3, strlen (tmp3)) == 0)
		{
			fprintf(w_fp, "%s\n", buf);
			if (cnt - 1 != itemcnt)
				printf ("%2.2s accno count changed: %d -> %d\n",
					buf, itemcnt, cnt - 1);
			continue;
		}
		...
		{
			itemcnt = atoi (tmp2);
			sprintf (buf, "Accno_Count=%s", tmp2);
			fprintf (w_fp, "%s\n", buf);
			continue;
		}

		sprintf (tmp3, "%s", "ACCNO_0000_COMMENT");
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			sprintf (buf, "Accno_%d_Comment=%s", cnt, sp1 + 1);
			fprintf (w_fp, "%s\n", buf);
			continue;
		}

		sprintf (tmp3, "%s", "ACCNO_0000_APTYPE");
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			sprintf (buf, "Accno_%d_Aptype=%s", cnt, tmp2);
			fprintf (w_fp, "%s\n", buf);
			continue;
		}

		sprintf (tmp3, "%s", "ACCNO_0000_ACC_NO");
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			sprintf (buf, "Accno_%d_Acc_No=%s", cnt, tmp2);
			fprintf (w_fp, "%s\n", buf);
			continue;
		}

		sprintf (tmp3, "%s", "ACCNO_0000_PWD");
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			sprintf (buf, "Accno_%d_Pwd=%s", cnt, tmp2);
			fprintf (w_fp, "%s\n", buf);
			continue;
		}

		sprintf (tmp3, "%s", "ACCNO_0000_ATHRTY");
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			sprintf (buf, "Accno_%d_Athrty=%s", cnt, tmp2);
			fprintf (w_fp, "%s\n", buf);
			continue;
		}

		sprintf (tmp3, "%s", "ACCNO_0000_IP");
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			sprintf (buf, "Accno_%d_Ip=%s", cnt, tmp2);
			fprintf (w_fp, "%s\n", buf);
			continue;
		}

		sprintf (tmp3, "%s", "ACCNO_0000_AUTO_MAN_CNT");
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			sprintf (buf, "Accno_%d_Auto_Man_Cnt=%s", cnt, tmp2);
			fprintf (w_fp, "%s\n", buf);
			continue;
		}

		sprintf (tmp3, "%s", "ACCNO_0000_JS_BAND");
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			sprintf (buf, "Accno_%d_Js_Band=%s", cnt, tmp2);
			fprintf (w_fp, "%s\n", buf);
			continue;
		}

		sprintf (tmp3, "%s", "ACCNO_0000_RISK_MAX");
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			sprintf (buf, "Accno_%d_Risk_Max=%s", cnt, tmp2);
			fprintf (w_fp, "%s\n", buf);
			continue;
		}

		sprintf (tmp3, "%s", "ACCNO_0000_ACC_F_ORDER_MAXCNT");
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			sprintf (buf, "Accno_%d_Acc_F_Order_MaxCnt=%s", cnt, tmp2);
			fprintf (w_fp, "%s\n", buf);
			continue;
		}

		sprintf (tmp3, "%s", "ACCNO_0000_ACC_F_GET_MAXCNT");
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			sprintf (buf, "Accno_%d_Acc_F_Get_MaxCnt=%s", cnt, tmp2);
			fprintf (w_fp, "%s\n", buf);
			continue;
		}

		sprintf (tmp3, "%s", "ACCNO_0000_ACC_SO_ORDER_MAXCNT");
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			sprintf (buf, "Accno_%d_Acc_SO_Order_MaxCnt=%s", cnt, tmp2);
			fprintf (w_fp, "%s\n", buf);
			continue;
		}

		sprintf (tmp3, "%s", "ACCNO_0000_ACC_SO_GET_MAXCNT");
		if (memcmp (tmp1, tmp3, Max(strlen(tmp1),strlen(tmp3))) == 0)
		{
			sprintf (buf, "Accno_%d_Acc_SO_Get_MaxCnt=%s", cnt, tmp2);
			fprintf (w_fp, "%s\n", buf);
			continue;
		}
