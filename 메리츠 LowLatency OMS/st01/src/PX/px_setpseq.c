...
{
	int		dk, pk;
	char	proc_id[12], if_seq[12], sub[4], buf[128];
	FILE	*fp;
	...
	if (argc != 1 && argc != 3)
	{
		...
		printf ("Usage: %s <process ID> <interface sequence>\n\n", argv[0]);
	...
	memset (if_seq, 0, sizeof (if_seq));

	if (argc == 3)
	{
		memcpy (proc_id, argv[1], strlen (argv[1]));
		memcpy (if_seq, argv[2], strlen (argv[2]));
	}
	else
	{
		...
		printf ("\033[1minterface sequence ?\033[0m ");
		fflush (stdout);
        fgets (if_seq, sizeof(if_seq), stdin);
		if (if_seq[0] == '\0')
			exit (FAIL);
	}
	...
	Log (USR_OK, "[%s: %s, %s]", buf, proc_id, if_seq);

	exit (OK);
}	/* End of main ()	*/

/*************************************************************************
	End of Program (px_setpseq.c)
*************************************************************************/
