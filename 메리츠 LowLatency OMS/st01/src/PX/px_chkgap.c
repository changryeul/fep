...
/*----------------------------------------------------------------------*/
{
	...
	else
	{
		memcpy (name, "all", 3);
		memcpy (day, "00000000", 8);
		memcpy (chk_from, "090004", 6);
		memcpy (chk_to, "150500",	6);
	}
	...
	printf ("================================================================================\n");
	printf ("Line   Seq Cnt      Order Recv    Confirm Send  MECHE  CAL_TIME     TOTAL        KRX        FEP\n");
	printf ("--------------------------------------------------------------------------------\n");
...
/* park */
		...
		printf ("%2.2s %8.8s %2.2s  %.2s:%.2s:%.2s.%.6s %.15s   %.3s %07.06f %07.06f %07.06f %07.06f\n",
			buf+163, buf+114, buf+122, buf+50, buf+52, buf+54, buf+56,
			buf+17, tot_tm, krx_tm, fep_tm);
	...
	printf ("min: TOT = %010.06f, KRX = %010.06f, FEP = %010.06f\n",
		min_tot, min_krx, min_fep);
	printf ("max: TOT = %010.06f, KRX = %010.06f, FEP = %010.06f\n",
		max_tot, max_krx, max_fep);
	printf ("avg: TOT = %010.06f, KRX = %010.06f, FEP = %010.06f\n",
		avg_tot, avg_krx, avg_fep);
	printf ("\n[%s]\n", path);
