...
/* **************************************************************************************************** */
		...
		switch (i)
		{
			...
			case 2:			// 첫번째 시장신호(ex, 유가증권)
			case 3:			// 두번째 시장신호(ex, 주식선물)
			case 4: 		// 세번째 시장신호(ex, 주식옵션), 2개까지만 쓰지 3개이상은 안 쓸듯.
				rt = Auto_Logic(i-2);
				if (rt == NOTOK)
					return;
				break;
...
/*--------------------------------------------------------*/
{
	...
	if (memcmp(R_Fmt[0].Data, "500200", 6) == 0)				// 자동기동 Setting
		Set_In_Param();
...
/*--------------------------------------------------------*/
{
	...
	/* 2. 샘플전략은 지수선물의 체결수량이 50계약 이상 && 체결틱이 상승 => Call매수	*/
	/* 				   체결수량이 50계약 이상 && 체결틱이 하락 => Put매수			*/
	/* 3. 주문은 1회만 처리하고 종료한다. 											*/
	/* 4. 전략설정시 3개의 종목을 받는제 첫번째는 조건이 되는 선물이고				*/
	/*						 두번째는 주문대상이 되는 Call옵션이고					*/
	/*						 세번째는 주문대상이 되는 Put옵션으로 전략을 협의하였다.*/
	/* 5. 또한 화면에서도 3종목은 순서대로 받기로 하였다.							*/
	/* **************************************************************************** */
	
	if (AtoIf(Shm_Futures[0].G7.tot_con_qty, 7) <= 0)	// 장개시전에는 주문처리 안함.
		return (0);
	
	/* 선물의 체결수량(50이상)과 UpTick or DownTic 체크 */
	if (Shm_Futures[Shm_Strrg[0].SamPle_St01[OD_SEQ].Condition[0].item_seq].HogaLastGbn == 0)	// A3
	{
		/* 체결수량 체크 */
		if (memcmp(Shm_Futures[Shm_Strrg[0].SamPle_St01[OD_SEQ].Condition[0].item_seq].A3.chekyul_qty, "000050", 6) > 0)
		{
			/* Up or Down 체크 */
			if (memcmp(Shm_Futures[Shm_Strrg[0].SamPle_St01[OD_SEQ].Condition[0].item_seq].A3.crprc,
					  Shm_Futures[Shm_Strrg[0].SamPle_St01[OD_SEQ].Condition[0].item_seq].A3.before_crprc, 5) > 0)	//UP
			{
				rt = 1;			// 첫번째종목 매수
			}
			else
			if (memcmp(Shm_Futures[Shm_Strrg[0].SamPle_St01[OD_SEQ].Condition[0].item_seq].A3.crprc,
					  Shm_Futures[Shm_Strrg[0].SamPle_St01[OD_SEQ].Condition[0].item_seq].A3.before_crprc, 5) < 0)	//Down
			{
				rt = 2;			// 두번째종목 매수
			}
		}
	...
	if (Shm_Futures[Shm_Strrg[0].SamPle_St01[OD_SEQ].Condition[0].item_seq].HogaLastGbn == 1)	//G7
	{
		/* 체결수량 체크 */
		if (memcmp(Shm_Futures[Shm_Strrg[0].SamPle_St01[OD_SEQ].Condition[0].item_seq].G7.chekyul_qty, "000050", 6) > 0)
		{
			/* Up or Down 체크 */
			if (memcmp(Shm_Futures[Shm_Strrg[0].SamPle_St01[OD_SEQ].Condition[0].item_seq].G7.crprc,
					  Shm_Futures[Shm_Strrg[0].SamPle_St01[OD_SEQ].Condition[0].item_seq].G7.before_crprc, 5) > 0)	//UP
			{
				rt = 1;			// 첫번째종목 매수
			}
			else
			if (memcmp(Shm_Futures[Shm_Strrg[0].SamPle_St01[OD_SEQ].Condition[0].item_seq].G7.crprc,
					  Shm_Futures[Shm_Strrg[0].SamPle_St01[OD_SEQ].Condition[0].item_seq].G7.before_crprc, 5) < 0)	//Down
			{
				rt = 2;			// 두번째종목 매수
			}
		}
...
/*--------------------------------------------------------*/
{
	...
	/* Daemon에게 즞각적인 종료요청을 위해 signal 주기 */
...
/***********************************************************
	...
	Parameters IN 	: . mk_gbn : 시장구분 (1~10)
...
/*--------------------------------------------------------*/
{
	...
	if (mk_gbn == 5|| mk_gbn == 6)		// 현물시장 5:유가증권, 6:코스닥
	{
		/* ************************************************************************ */
		/* 변수값 넣기 */
		...
		/* 호가수량, 80 */
		memset (tmp, 0, sizeof (tmp));
		sprintf (tmp, "%010ld", Shm_Strrg[0].SamPle_St01[OD_SEQ].Condition[arry].order_cnt);
	...
	if (mk_gbn == 5 || mk_gbn == 7)			// 5:유가증권, 7:ELW/ETN/ETF
	...
	{
		if (mk_gbn == 5 || mk_gbn == 6)		// 현물시장 5:유가증권, 6:코스닥
			Log (SAM_FATAL, "shm write fail [%s]", ODN(D_K,P_K,0));
		else
			Log (SAM_FATAL, "shm write fail [%s]", ODN(D_K,P_K,1));
		return (NOTOK);
	}

	if (mk_gbn == 5 || mk_gbn == 7)			// 5:유가증권, 7:ELW/ETN/ETF
		Log (SAM_OK, "file write [%s:%d:%d]", ODN(D_K,P_K,0), ODW(D_K,P_K,0,0), rt);
	else if (mk_gbn == 6)					// 6:코스닥
		Log (SAM_OK, "file write [%s:%d:%d]", ODN(D_K,P_K,1), ODW(D_K,P_K,1,0), rt);
	else
		Log (SAM_OK, "file write [%s:%d:%d]", ODN(D_K,P_K,2), ODW(D_K,P_K,2,0), rt);
