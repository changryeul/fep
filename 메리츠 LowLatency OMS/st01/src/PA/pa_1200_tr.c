...
/*----------------------------------------------------------------------*/
{
	...
	{
		Log	(USR_ERROR, "TH.ResponseCode [%.3s:%03d]",
...
/*----------------------------------------------------------------------*/
{
	...
	for (i = 0, next = 0; i < d_cnt; i ++)
	{
		...
		if	(memcmp (data+11+next, "MERITZREJEC", 11) == 0) // FEP/KRX거부
		{
			f_gbn = 1;
			len = TCP_REJE_HEAD_LEN+sizeof(KRX_JUMUN_DATA); // 80+261=>341
		}
		else
		{
			if	(memcmp (data+11+next, "TTRTDP21301", 11) == 0) // TTRTDP21301(체결)
			{
				f_gbn = 2;
				len = sizeof(KRX_SETTLE_DATA);     		// 242
			}
			else
			if	((memcmp (data+11+next, "TTRODP11301", 11) == 0) || // TTRODP11301(정상확인), TTRODP11321(거부), TTRODP11303(자동취소)
				 (memcmp (data+11+next, "TTRODP11321", 11) == 0) || 
				 (memcmp (data+11+next, "TTRODP11303", 11) == 0) ) 
			{
				f_gbn = 1;
				len = sizeof(KRX_SETTLE_RESP_DATA);		// 287
			}
			else
			if	((memcmp (data+11+next, "TCHORD10001", 11) == 0) || // 신규/정정/취소 주문응답
				(memcmp (data+11+next, "TCHORD10002", 11) == 0) || 
				(memcmp (data+11+next, "TCHORD10003", 11) == 0) )
			{
				f_gbn = 1;
				len = sizeof(KRX_JUMUN_DATA);			// 261
			}
			else
			{
				Log (USR_ERROR, "Data Type Error [%11.11s] [%261.261s]",
				 data+next+11, data+next);
				return (NOTOK);
			}
		}
		/*	********************************************* */
		...
		if (f_gbn == 1)
			memcpy	(&w_buf[rec_size*i+f_size], "100200", 6);
		else
			memcpy	(&w_buf[rec_size*i+f_size], "100400", 6);
		memcpy	(&w_buf[rec_size*i+f_size+SEARCH_HEADER_LEN], data+next, len);
	...
	Log (USR_OK, "Write ok[%s][%d]", w_buf, strlen(w_buf));
	Set_TR_Time ();
