...
/*----------------------------------------------------------------------*/
{
	...
	while (START_S != END)
	{
		...
		if (Sockfd < 0)
		{
			TCP2_CON_STA = OFF;
			TCP2_LINE_ST = OFF;
			SLog (TCP_ERROR, "socket fail:Sockfd[%d] {%d:%s}", Sockfd, SYS_NO, SYS_STR);
			sleep (5);
			continue;
		}
		...
		else
		{
			while (START_S < JOB_END)
			{
				if (WR_CNT > RD_CNT)
				{
					...
					{
						SLog (SAM_ERROR, "01 WR_CNT[%d] RD_CNT[%d]", WR_CNT, RD_CNT);
						sleep(60);
						Exit_Process ();
					}
					File_Event_Rtn ();
		...
		if (rt > 0)
		{
			for (i = 0; i < PollCnt; i ++)
			{
				...
				if (Poll[i].revents & POLLHUP)
				{
					if (i == SOCKET_EVENT)
					{
						SLog (TCP_ERROR, "socket disconnected[%#06x]", Poll[i].revents);
						return;
					}
			...
			case FILE_EVENT:
				...
				while (START_S < JOB_END)
				{
					if (WR_CNT > RD_CNT)
					{
						if (MAX_POLL < (WR_CNT - RD_CNT))
						{
							SLog (SAM_ERROR, "02 WR_CNT[%d] RD_CNT[%d]", WR_CNT, RD_CNT);
							sleep(60);
							Exit_Process ();
						}
						File_Event_Rtn ();
...
/*----------------------------------------------------------------------*/
{
	...
	if ( PktType == T_DATA )
	{
		...
		//memcpy (S_Pkt->Data,    &R_Buf[0], DataSize);
...
/*
	...
	if (rt <= 0)
	{
		SLog (TCP_ERROR, "Select_Send:send failure[%d] {%d:%s}", rt, SYS_NO, SYS_STR);
		TCP2_CON_STA = OFF;
		close (Sockfd);
		Exit_Process ();
	}
