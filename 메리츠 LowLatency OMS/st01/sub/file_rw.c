...
/*------------------------------------------------------------------------
	Function Prototypes
------------------------------------------------------------------------*/
int		F_R (int, char *, int);
int		F_R_Proc (int, char *, int, int);
int		F_W (int, char *, int);
int		F_W_Proc (int, char *, int, int);
void	Add_Count (int, int);
int		F_R2 (char *, int);
void	F_W2 (char *, char *);
int		F_W2_Proc (int, char *, int, int);
int		F_R3 (int, int, char *);
int		F_W3 (int, int, char *);
int		F_WB (int, int, char *, int);
int		SF_W (int, char *);
int		SF_W_Proc (int, char *, int);
...
/*----------------------------------------------------------------------*/
{
	...
	if (f_name == '\0' && strlen (f_name) == 0)
	{
		Log (SAM_ERROR, "F_W2:file name is NULL");
		Exit_Process ();
	}

	if (p_buf == '\0' && strlen (p_buf) == 0)
	{
...
/*----------------------------------------------------------------------*/
{
	int				rt, cnt, i, FIFO_fd, len, pt, f_size, null_cnt;
	char			buf[FILE_BUF_LEN], sub[4], fifo_name[256], null_flag;
	FILE_RW_HEAD	file_rw;
	BUFF_RW_HEAD	buff_rw;
