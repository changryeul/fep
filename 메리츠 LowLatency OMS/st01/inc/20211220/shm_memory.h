...
/* 자동주문 Process 수  */
#define		MAX_AUTO_PROC	  20						/* 자동주문 Process 수	*/
...
typedef struct
{
	long	item_getcnt;						/* 계좌별 보유종목수량(부호있음)	*/
							/* 최초 원장에서 받고 매매시 업데이트한다. */
	long	item_getmoney;						/* 장부금액(수량*평단)				*/
							/* 최초 원장에서 받고 매매시 업데이트한다. */
	long	item_tot_sugum;						/* 당일 매수누적금액	*/
	long	item_cha_sugum;						/* 당일 매수차감금액	*/
