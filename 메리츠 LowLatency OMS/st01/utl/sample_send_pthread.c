...
typedef struct
{
	int		R_cnt;
	int		W_cnt;
	char	Data[MAX_CNT][MAX_DATA_SIZE];
}	R_DATA;
...
/* Send Data */
	...
	sprintf (fifo_name, "%s/st01/utl/sample_send_pthread1", (char *)getenv("_FEP_HOME"));
	mknod (fifo_name, S_IFIFO|0777, 0);
	chmod (fifo_name, 0777);
	Poll[0].fd = open(fifo_name, O_RDWR|O_NDELAY);
	Poll[0].events = POLLIN;
	if (Poll[0].fd < 0)
	{
		printf ("cannot open FIFO[%s][%d][%d:%s]\n",
			fifo_name, Poll[0].fd, errno, strerror(errno));
		exit (1);
	}

	memset (R_Data[0].Data, 0, sizeof (R_DATA) - sizeof(int) * 2);
	R_Data[0].W_cnt = 0;
	R_Data[0].R_cnt = 0;

	pthread_create(&Read_t, NULL, Thread_Q_RW, NULL);
	pthread_create(&Send_t, NULL, Thread_Wait_Send, NULL);
...
void	*Thread_Q_RW ()
{
	...
	while(1)
	{
		...
		/* Q에서 읽은것 R_Data에 넣기 */
		memset (R_Data[0].Data[(R_Data[0].W_cnt%MAX_CNT)], 0x20, DATA_SIZE);
		memcpy (R_Data[0].Data[(R_Data[0].W_cnt%MAX_CNT)], R_Buff, strlen(R_Buff));
		R_Data[0].W_cnt++;

		/* fifo write로 Q왔다는 신호주기 */
		rt = write (Poll[0].fd, "1", 1);
		if (rt < 0)
			printf ("cannot write FIFO[%d][%d:%s]\n",
					Poll[0].fd, errno, strerror(errno));

		while (1)
		{
			rt = read (Poll[0].fd, tmp, sizeof(tmp));
#if defined __linux
...
void *Thread_Wait_Send ()
{
	...
	while (1)
	{
		if (R_Data[0].W_cnt > R_Data[0].R_cnt)
			Tcp_Send(1);		/* Data */
		else
		{
			rt = poll (Poll, PollCnt, 30*1000);
			if (rt > 0)
			{
				for (i = 0; i < PollCnt; i++)
				{
			...
			else
			{
				...
				{
					printf ("poll timeout\n");
					Tcp_Send (0);		/* Poll Send */
					continue;
				}
				else
				{
					if (errno == EINTR)
						printf ("poll interrupted {%d:%s}\n", errno, strerror(errno));
					else
						printf ("poll failure {%d:%s}\n", errno, strerror(errno));
					continue;
				}
			}
			...
			switch (i)
			{
				...
				default:
					printf ("event case error [%d,%d]\n", i, PollCnt);
					return;
					break;
			}
		}	/* End of else */
	}	/* End of while */
}	/* End of Thread_Wait_Send */

				
/* ******************************************************** */
/* Poll_Wait && Tcp_Send									*/
/* ******************************************************** */
void Tcp_Send (int d_gbn)
{
	int		rt;
	...
	if (d_gbn == 1)		/* Data */
	{
		memset(SendPkt, 0x20, 50+DATA_SIZE);
		/* 송신할 Packet Size는 500으로 고정임, 앞에 사이즈정보는 총사이즈(50+500) - 4 = 546 */
		sprintf(tmp, "0546DATA0000%8.8s%08d                      ", Date, INT_SEQ+1);
		memcpy(SendPkt, tmp, strlen(tmp));
		memcpy(&SendPkt[50], R_Data[0].Data[(R_Data[0].R_cnt%MAX_CNT)], DATA_SIZE);
	}
	else				/* Poll */
	{
		memset(SendPkt, 0x20, 50);
		/* 송신할 Packet Size는 50으로 고정임, 앞에 사이즈정보는 총사이즈(50) - 4 = 46 */
		sprintf(tmp, "0046POLL0000%8.8s%08d                      ", Date, INT_SEQ);
		memcpy(SendPkt, tmp, strlen(tmp));
	}
	rt = send(Sockfd, SendPkt, strlen(SendPkt), 0);
	if(rt <= 0)
