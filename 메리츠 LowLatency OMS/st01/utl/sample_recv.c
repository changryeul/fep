...
/* Send Data */
	...
	int		msqid;
	key_t	new_key;
	new_key = 0x33000004;	/* <= 사용하실 Q주소로 변경해서 쓰세요*/
	Msgbuf	sbuf;
	...
	while(1)
	{
		...
		if ((memcmp(&R_Buff[4], "DATA", 4) != 0)	&&
			(memcmp(&R_Buff[4], "POLL", 4) != 0))
		{
			printf("Data Recv TR CODE Error [%50.50s]\n", R_Buff);
			close(Sockfd);
			exit(1);
		}
		/* ******** */
		...
		if (memcmp(&R_Buff[4], "DATA", 4) == 0)
			sprintf(tmp, "0046DAOK0000%8.8s%08d                      ", Date, INT_SEQ+1);
		else
			sprintf(tmp, "0046POOK0000%8.8s%08d                      ", Date, INT_SEQ);
		memcpy(SendPkt, tmp, strlen(tmp));
printf("Befor SendPkt [%s]\n", SendPkt);
		rt = send(Sockfd, SendPkt, strlen(SendPkt), 0);
		if(rt <= 0)
		{
		...
		if (memcmp (&R_Buff[4], "DATA", 4) == 0)
		{
			...
			memcpy(sbuf.mtext, &R_Buff[50], strlen(R_Buff) - 50);
			
			rt = msgsnd(msqid, &sbuf, buf_length, 0);
			if(rt < 0)
			{
				printf("ERROR msgid[%d] mtype[%d] mtext[%s] len[%d]\n",
